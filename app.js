...
// ─────────────────────────────────────────────────────────────────────────────
// Rexyキャラクター（画像は rexy.js が window.REXY_IMAGES を定義。未ロード時は非表示）
// ─────────────────────────────────────────────────────────────────────────────
const REXY_IMAGES = typeof window !== "undefined" && window.REXY_IMAGES || {};
// 雨・雪のラウンドでは雨ポーズ(R)に切替。それ以外（晴/曇/未設定）は晴れポーズ。
function isWetWeather(weather) {
  return weather === "rainy" || weather === "snowy";
}
// 17時以降を夜とみなす（ホーム/起動あいさつで使用）
function isNightNow() {
  return new Date().getHours() >= 17;
}
// 晴れポーズ→雨ポーズの対応表（天気連動する用途のみ）
const REXY_RAIN_MAP = {
  basic3: "rain11",
  // アルバ・イーグル
  basic1: "rain9",
...
// Rexy（左）＋セリフ吹き出し（右）。childrenが吹き出しの中身。
function RexyBubble({
  costume = "basic",
  size = 64,
  children
...
// 分析ページ見出し下のRexy＋ランダムセリフ。マウントの度（タブを開く度）にセリフを抽選。
...
// クラブIDの表記揺れを正規化（旧表記 "50w" → 現マスター "W50" など）
// 集計・表示で同一クラブを統合するために使用。ストレージは変更しない。
function normalizeClubId(id) {
  if (!id) return id;
  // 旧ウェッジ表記 "{数字}w"（小文字w・数字先頭）→ "W{数字}"
  const m = /^(\d{2})w$/i.exec(id);
  if (m) return "W" + m[1];
  return id;
}

// クラブIDから表示ラベルを取得（マスター準拠。未登録IDはそのまま返す）
...
// ─────────────────────────────────────────────────────────────────────────────
// SHOT DEFINITIONS（新フォーマット）
// カテゴリ: tee / approach / putt の3種のみ
// ─────────────────────────────────────────────────────────────────────────────
...
// × サブ選択肢（カテゴリ別でOBのpenaltyCountが異なる）
// ⚠️ 保存規約：recordShot では shotCount = (定義値 shotCount) + penaltyCount として保存される。
//    定義値の shotCount はベース打数（ペナ除く）であり、保存後の値とは異なる。
//    外部データを生成する際は保存後の値を使うこと:
//      ティ OB       → shotCount: 3, penaltyCount: 2
//      ティ 1ペナ    → shotCount: 2, penaltyCount: 1
//      アプローチ OB → shotCount: 2, penaltyCount: 1  （penaltyCount が tee と異なる点に注意）
//      バンカー      → shotCount: 1, penaltyCount: 0
...
// 残り距離ラベル → 代表値（ヤード）変換（ドライバー飛距離推定用）
...
// メートル数をpinDistラベルに変換（パット距離スライダー用）
...
// 詳細モード: ゴール達成ホール数を集計
// パーオン = 非パットショット数 ≤ par−2 かつ パット有り
// 2パット以内 = パット有り（グリーン外直接カップイン除く）かつ ≤2パット
// Par4/5ティショット成功 = quality "○" のティショット（par≥4）
function calcGoalProgress(holeData, holePars, goal) {
  ...
  return Object.entries(holeData).filter(([, hd]) => hd.done).filter(([hKey, hd]) => {
    ...
    switch (goal.type) {
      ...
      case "gir":
        {
          // カップインアプローチ（パットなし）もグリーンオン扱いでGIRに含める
...
// コースデータ定義 & VENUESマスター
// ─────────────────────────────────────────────────────────────────────────────
// ⚠️  このセクションは venues.js に移動しました。
//     HTML で以下の順序で読み込んでください:
//       1. <script src="venues.js"></script>          ← VENUES グローバル変数
//       2. <script type="text/babel" src="golf-tracker.jsx"></script>
//
// VENUES はグローバル変数として参照可能です。
// ─────────────────────────────────────────────────────────────────────────────

// コース名からホール開始番号を判定（VENUESマスタのholeデータを参照）
...
// helper: 現在のラウンドから全18ホール情報を取得
function getRoundHoles(round) {
  if (!round) return [];
  const venue = VENUES.find(v => v.id === round.venueId);
  if (!venue) return [];
  const front = venue.subCourses.find(c => c.name === round.frontCourse)?.holes || [];
  // 北コースなど1コースの場合は後半も同じホールを使う
  const back = round.backCourse && round.backCourse !== round.frontCourse ? venue.subCourses.find(c => c.name === round.backCourse)?.holes || [] : venue.subCourses.length === 1 ? front : [];
...
const calcIdealGIR = (hcp, holeCount) => {
  ...
  return Math.min(holeCount, Math.max(0, (0.00476 * h * h - 0.429 * h + 10.12) * holeCount / 18));
};
const rnd2 = v => Math.round(v * 100) / 100;
const LONG_DIST_SET = new Set(["150Y以内", "200Y以内", "250Y以内", "250Y超"]);
const SHORT_DIST_SET = new Set(["10Y以内", "20Y以内", "30Y以内", "40Y以内", "50Y以内", "100Y以内"]);
...
function calcSimpleAnalytics(simpleHoleData, holePars, hcp = null, teeRates = null) {
  ...
  const holes = Object.entries(simpleHoleData).map(([h, d]) => ({
    ...d,
    hole: parseInt(h),
  ...
  const teeBadHoles = teeHoles45.filter(h => h.teeEval === "×");
  ...
  // 固定係数（A=0.5, B=0.25, C=0.75）
  const COEF_A = 0.3,
    COEF_B = 0,
    COEF_C = 0.9; // tGG×0.3, tFG廃止, tFB×0.9
  const bunkRate = teeRates?.bunkerFailRate ?? 0.81;
  ...
  // (2) ロングゲーム：GIR実績 − 理想GIR − ロングゲームOB×2 − ペナ − バンカー×失敗率 + ペナありパーオン
  // GIR近似：score - putts <= par - 2 のホール数
  const girCount = holes.filter(h => (h.score || 0) - (h.putts || 0) <= h.par - 2).length;
  // ペナありパーオン：ティ× かつ OB/ペナあり かつ score-putts ≤ par+ob*2+pen-2
  const penParOnCount = holes.filter(h => {
    if (h.teeEval !== "×") return false;
    const ob = h.ob || 0,
      pen = h.penalty || 0;
    if (ob + pen === 0) return false; // バンカーのみは除外
    const nonPutts = (h.score || 0) - (h.putts || 0);
    const regularGIR = nonPutts <= h.par - 2;
    return !regularGIR && nonPutts <= h.par + ob * 2 + pen - 2;
  }).length;
  // ロングゲームOB/バンカー近似（Q2-A：ティ評価×のホールをティ分として除外）
  // teeEval が × のホールのOB/バンカーはティ分とみなし除外、それ以外をロングゲーム分
  const longGameOB = holes.filter(h => h.teeEval !== "×" && (h.ob || 0) > 0).reduce((a, h) => a + (h.ob || 0), 0);
  const longGamePen = holes.filter(h => h.teeEval !== "×").reduce((a, h) => a + (h.penalty || 0), 0);
  const longGameBunk = holes.filter(h => h.teeEval !== "×" && (h.bunker || 0) > 0).reduce((a, h) => a + (h.bunker || 0), 0);
  let longScore;
  if (hcp !== null && hcp !== undefined) {
    const idealGIR = calcIdealGIR(hcp, holes.length);
    const longBunkRate = teeRates?.longBunkerFailRate ?? 0.81;
    longScore = r(girCount - idealGIR - longGameOB * 2 - longGamePen - longGameBunk * longBunkRate + penParOnCount);
  } else {
    const longShots = [...holes.filter(h => h.par === 3).map(h => h.teeEval), ...holes.map(h => h.secondEval)].filter(Boolean);
    const longBadRate = longShots.filter(v => v === "×").length / Math.max(longShots.length, 1);
    const longFairRate = longShots.filter(v => v === "△").length / Math.max(longShots.length, 1);
    longScore = r(longFairRate * -0.5 + longBadRate * -1);
  }

  // (3)(5) ショートゲーム
  // アプローチをしたホール（評価「無」以外）のパット数合計
  ...
  // スコア = HC + ホール数 - 理想パーオン数 - アプローチしたホールのパット数合計
  // ハンディキャップ未算出の場合は従来方式にフォールバック
  // 寄せホール（アプローチ評価○△×）が少なすぎる場合は短期ゲームを評価対象外(null)にする。
  //   → 寄せをしていない(全「無」/未記録)と avgPutt=0 で式が縮退し、HCPが悪いほど高評価になる逆転を防ぐ。
  //   閾値: 18H=3件以上 / 9H=2件以上
  const shortMinSample = holes.length <= 9 ? 2 : 3;
  let shortScore;
  if (approachHoles.length < shortMinSample) {
    shortScore = null; // データ不足：評価対象外
  } else if (hcp !== null && hcp !== undefined) {
    // 方針B（確定）：そのラウンドで「寄せ周りに失った打数」を表す指標。
    //   成功(○)は当たり前=加点なし／失敗で打数を失う(基本マイナス)／HCPが少し緩和。
    //   loss   = ×*1.0 + 3パット以上*1.0 + △*0.5
    //   bonus  = (○ かつ 0パット=カップイン)*1.0 + (○ かつ 1パット=寄せワン)*0.5
    //   relief = min(hcp,36)*0.15  （HCPは分析時36でキャップ。常に全額加点）
    //   shortScore = bonus - loss + relief
    //   approachEval は既に normEval 済み（○/△/×）
    let loss = 0,
      bonus = 0;
    for (const h of approachHoles) {
      if (h.approachEval === "×") loss += 1.0;
      if (h.approachEval === "△") loss += 0.5;
      if ((h.putts || 0) >= 3) loss += 1.0;
      if (h.approachEval === "○" && (h.putts || 0) === 0) bonus += 1.0;
    ...
    const appBad = approachShots.filter(v => v === "×").length / Math.max(approachShots.length, 1);
  ...
  // (4) パット：Par合計×0.4 + HC×0.15 - パット数合計 - 3パット超過合計 + 1パット×0.5
  //   超過合計 = Σ max(0, putts-2)（3パット=1/4パット=2/5パット=3）
  //   1パット加点 = 1パット回数 × 0.5（パットが強い人を加点強調）
  //   ハンディキャップ未算出の場合は従来方式（3パット割合×-1）にフォールバック
  const threePuttCount = holes.filter(h => (h.putts || 0) >= 3).length; // 文言・課題判定用の「3パット以上の回数」
  const puttOverSum = holes.reduce((a, h) => a + Math.max(0, (h.putts || 0) - 2), 0); // 3パット超過合計
  const onePuttCount = holes.filter(h => (h.putts || 0) === 1).length; // 1パット回数
  ...
  // (5) バンカースコア：バンカーに入ったホールの対Par差合計 ÷ ホール数 × 18 - HCP
  // バンカーホールなし or HCPなし → null
  ...
  if (bunkerHoles.length > 0) {
    ...
    if (hcp !== null && hcp !== undefined) {
      // 期待差分 = HC × バンカーホール数 ÷ 18（スケールを揃える）
      const expectedDiff = hcp * bunkerHoles.length / 18;
      bunkerScore = r(expectedDiff - bunkerParDiffSum);
    } else {
      bunkerScore = r(-bunkerParDiffSum); // 負 = 悪い方向に統一
    }
  }

  // AIインサイト用：ロング/ショート品質配列
  // ロング = Par3のティ評価 + 全ホールのsecondEval（2nd shot）
  // ショート = thirdEval（Par5の3rd shot）+ approachEval（最後の寄せ）
  //   → 詳細モードの shortShotQuals（100Y以内ショット全部）と同等のサンプル数を確保
  const longShotQuals = [...holes.filter(h => h.par === 3 && h.teeEval).map(h => h.teeEval), ...holes.map(h => h.secondEval).filter(Boolean)];
  const shortShotQuals = [...holes.map(h => h.thirdEval).filter(Boolean), ...holes.filter(h => h.approachEval && h.approachEval !== "無" && h.approachEval !== "none").map(h => h.approachEval)];
  return {
    teeScore,
    longScore,
    shortScore,
    ...
    idealGIR: hcp !== null && hcp !== undefined ? calcIdealGIR(hcp, holes.length) : null,
    longShotQuals,
    shortShotQuals
  };
}
...
// ─────────────────────────────────────────────────────────────────────────────
// 詳細モード専用アナリティクス（remainDist で正確分類）
// ─────────────────────────────────────────────────────────────────────────────
function calcDetailAnalytics(holeData, holePars, hcp = null, teeRates = null) {
  ...
  const isLongDist = d => !d || LONG_DIST_SET.has(d); // null → ロング（Par3ティfallback）
  const isShortDist = d => d && SHORT_DIST_SET.has(d);
  const holes = Object.entries(holeData).filter(([, hi]) => hi?.done).map(([hStr, hi]) => {
    ...
    // ロングゲームショット：Par3ティ全て ＋ ティ以外・パット以外で100Y超
    const longShotQuals = [
    // Par3ティ: 100Y超(isLongDist, null→long扱い)のみlong
    ...(par === 3 ? teeShots.filter(s => isLongDist(s.remainDist)).map(s => normQ(s.quality)) : []), ...shots.filter(s => s.categoryKey !== "tee" && s.categoryKey !== "putt").filter(s => isLongDist(s.remainDist)).map(s => normQ(s.quality))].filter(Boolean);

    // ショートゲームショット：Par3ティ100Y以内 ＋ ティ以外・パット以外で100Y以内
    const shortShotQuals = [
    // Par3ティ: 100Y以内(isShortDist)のみshort
    ...(par === 3 ? teeShots.filter(s => isShortDist(s.remainDist)).map(s => normQ(s.quality)) : []), ...shots.filter(s => s.categoryKey !== "tee" && s.categoryKey !== "putt").filter(s => isShortDist(s.remainDist)).map(s => normQ(s.quality))].filter(Boolean);
    const hasShortShot = shortShotQuals.length > 0;
    // 初めてショートゲームショットをした以降の全打数（パット含む）
    const firstShortIdx = shots.findIndex(s => s.categoryKey !== "tee" && s.categoryKey !== "putt" && isShortDist(s.remainDist) || par === 3 && s.categoryKey === "tee" && isShortDist(s.remainDist));
  ...
  const teeBadHoles = teeHoles.filter(h => h.teeQuality === "×");
  const teeFairHoles = teeHoles.filter(h => h.teeQuality === "△");
  const teeBadOB = teeBadHoles.reduce((a, h) => a + h.ob, 0);
  // teeScore: teeRatesデータあり→距離別レート方式、それ以外→固定係数方式
  const COEF_A = 0.3,
    COEF_B = 0,
    COEF_C = 0.9;
  const bunkerPenalty = holes.reduce((a, h) => a + h.bunker, 0) * (teeRates?.bunkerFailRate ?? 0.81);
  const hcpBase = hcp ?? 0;
  let teeScore;
  if (hcp !== null && teeRates?.hasData) {
    let bonus = 0;
    for (const hole of holes) {
      if (hole.par < 4) continue;
      const teeQ = hole.teeQuality;
      if (teeQ !== "○" && teeQ !== "△") continue;
      const nShot = (holeData[hole.h]?.shots || []).find(s => s.categoryKey !== "tee" && s.categoryKey !== "putt");
      const dist = nShot?.remainDist;
    ...
    // 固定係数方式（HC有無で hcpBase * 0.3 を加算するかどうかだけ異なる）
    let tGSecG = 0,
      tFSecG = 0,
      tFSecB = 0;
    for (const hole of holes) {
  ...
  // ペナありパーオン（詳細モード）
  // ティ× かつ teeShot.subType が ob/1pen（バンカーは除外）かつ 補正後GIR達成
  const penParOnCount = holes.filter(hole => {
    if (hole.teeQuality !== "×") return false;
    const teeShots = (holeData[hole.h]?.shots || []).filter(s => s.categoryKey === "tee");
    ...
    if (teeOB + teePen === 0) return false; // バンカーのみは除外
  ...
  // ③-HC shortScore（詳細モード・HC設定あり）
  // 判定:
  //   ① 距離OK（距離閾値以内）                         → +1.0
  //   ② 距離NG + 3m以内 + 1パット                      → +1.0（救済成功）
  //   ③ 2パット（30Y/40Y/50Y/100Y以内）                 → 0（中立）
  //   ③ 2パット（10Y/20Y以内）                          → 失敗（-1）
  //   ④ 3パット以上                                     → 失敗（-1）
  // shortScore = HC×0.15 + successPts×0.3 − failureCount
  const SHORT_STRICT_SET = new Set(["10Y以内", "20Y以内"]); // 2パット=失敗
  const WITHIN_3M_SET = new Set(["1m以内", "2m以内", "3m以内"]);
  const CUP_SUCCESS_THRESH = {
    "10Y以内": new Set(["1m以内"]),
    "20Y以内": new Set(["1m以内", "2m以内"]),
    "30Y以内": new Set(["1m以内", "2m以内", "3m以内"]),
    "40Y以内": new Set(["1m以内", "2m以内", "3m以内", "4m以内"]),
    "50Y以内": new Set(["1m以内", "2m以内", "3m以内", "4m以内", "5m以内"]),
    "100Y以内": new Set(["1m以内", "2m以内", "3m以内", "4m以内", "5m以内", "10m以内", "10Y以内"])
  };
  const isShortSuccess = (shotDist, pinDist) => !!CUP_SUCCESS_THRESH[shotDist]?.has(pinDist);
  let shortScore;
  if (hcp !== null && hcp !== undefined) {
    const idealGIR = calcIdealGIR(hcp, holes.length);
    if (shortHoles.length > 0) {
      ...
      for (const hole of shortHoles) {
        const shots = holeData[hole.h]?.shots || [];
        const pinDist = holeData[hole.h]?.pinDist;
        // 最後のショートゲームショットを取得
        let lastSGIdx = -1;
        ...
        if (pinDist && isShortSuccess(lastSG.remainDist, pinDist)) {
          successPts += 1.0;
          // ② 距離NG + 3m以内 + 1パット
        } else if (pinDist && WITHIN_3M_SET.has(pinDist) && putt?.shotCount === 1) {
          successPts += 1.0;
          // ③ 10Y/20Y以内 + 2パット → 失敗
        } else if (putt?.shotCount === 2 && SHORT_STRICT_SET.has(lastSG.remainDist)) {
          failureCount++;
          // ③ 30Y〜100Y以内 + 2パット → 0（中立）
        } else if (putt?.shotCount === 2) {
          // 何も加算しない
          // ④ 3パット以上 → 失敗
  ...
  // ④ puttScore：Par合計×0.4 + HC×0.15 - パット数合計 - 3パット超過合計 + 1パット×0.5
  let puttScore;
  if (hcp !== null && hcp !== undefined) {
    puttScore = r(totalParSum * 0.4 + hcp * 0.15 - totalPutts - puttOverSum + onePuttCount * 0.5);
  } else {
...
// ─────────────────────────────────────────────────────────────────────────────
// ラッパー：inputMode に応じて適切な分析関数を呼び分ける
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// 簡易ドライビングディスタンス推定
// 条件: Par4/5 + ティショットがドライバー（1W）+ 評価○ + 2打目の残り距離あり
// 推定距離 = ホール距離 − 2打目残り距離（代表値）
// ─────────────────────────────────────────────────────────────────────────────
function calcDriverDistStats(rounds, maxRounds = 5) {
  ...
  for (const r of detailRounds) {
    ...
    for (const [hKey, hd] of Object.entries(r.holeData)) {
      ...
      if (dist > 100 && dist < 420) samples.push(dist); // 妥当範囲チェック
    }
    if (samples.length > 0) {
  ...
  return result; // [{ date, avg, count }, ...]
}

// ─────────────────────────────────────────────────────────────────────────────
// 3ホール区間分析（18Hラウンドのみ・直近n件の平均）
// 6区間（1-3H/4-6H/7-9H/10-12H/13-15H/16-18H）× 5指標
// ─────────────────────────────────────────────────────────────────────────────
function calcSegmentAnalysis(rounds, n) {
  // 18H完了ラウンドのみ（simpleHoleData が17ホール以上）、日付降順、直近n件
  ...
  const segments = SEGMENTS.map(holes => {
    const roundParDiffs = []; // 各ラウンドの区間対パー合計
    const roundPutts = []; // 各ラウンドの区間パット合計
    let girHoles = 0,
      totalHoles = 0; // パーオン率（全ホールプール）
    let recOk = 0,
      recTotal = 0; // リカバリー率
    let teeBad = 0,
      tee45 = 0; // ティ×率（Par4/5のみ）
    ...
    target.forEach(r => {
      ...
      holes.forEach(h => {
        ...
        // パーオン: (score - putts) <= par - 2
        totalHoles++;
        const isGir = score - putts <= par - 2;
        if (isGir) {
          girHoles++;
        } else {
          // リカバリー: パーオン失敗ホールで Par以下
          recTotal++;
          if (score <= par) recOk++;
        }
        ...
        if (par >= 4) {
          tee45++;
          if (normQuality(hd.teeEval) === "×") teeBad++;
        }
      });
      if (hasData) {
...
function calcAnalytics(r, hcp, teeRates = null) {
  if (r.inputMode === "detail" && r.holeData && Object.keys(r.holeData).length > 0) {
    const sa = calcDetailAnalytics(r.holeData, r.holePars, hcp, teeRates);
    return sa ? {
      ...sa,
      isDetailMode: true
    } : null;
...
function calcHistoricalTeeRates(rounds) {
  ...
  let longBunkTotal = 0,
    longBunkFail = 0;
  // 距離別カウント: { dist: { goodTee_nextGood, goodTee_total, fairTee_nextGood, fairTee_nextBad, fairTee_total } }
  const distMap = {};
  for (const dk of DIST_KEYS) distMap[dk] = {
    gNG: 0,
    gT: 0,
    fNG: 0,
    fNB: 0,
  ...
  for (const r of targets) {
    for (const [hKey, hi] of Object.entries(r.holeData || {})) {
      ...
      if (!teeSh || par < 4) continue; // Par3除外

      // ティバンカー失敗率
      if (teeSh.subType === "bunker") {
        bunkTotal++;
        const next = shots.find(s => s.categoryKey !== "tee" && s.categoryKey !== "putt");
        if (!next || next.quality !== "○") bunkFail++;
      }

      // ロングゲームバンカー失敗率（Par3ティ含む100Y超、非ティ）
      for (let i = 0; i < shots.length; i++) {
        ...
        longBunkTotal++;
        const next = shots.slice(i + 1).find(ns => ns.categoryKey !== "putt");
        if (!next || next.quality !== "○") longBunkFail++;
      }
...
function generateDiagnosis(sa, shd, hcp, rounds, roundId, roundCount = 1, scoreOverride = null) {
  ...
  const band = hcpBands.find(b => hcp <= b.max) ?? hcpBands[hcpBands.length - 1];

  // --- ギャップ判定（z-score方式：HCに応じた標準偏差で正規化） ---
  // 標準偏差推定: σ = max(2.5, HC×0.25 + 2.5)
  //   HC0 →2.5打, HC10→5.0打, HC20→7.5打, HC30→10.0打
  // 9Hは標本数が半分なのでSDが√(1/2)倍 ≒ 0.707
  const sdBase = Math.max(2.5, (hcp ?? 0) * 0.25 + 2.5);
  const sd = sdBase * (is9H ? 0.707 : 1.0);
  const z = gap / sd;
  const gapLevels = [{
    maxZ: -0.5,
    label: "絶好調",
    color: "#f59e0b",
    comment: "実力以上のラウンド。コンディションが非常に良かったです！"
  }, {
    maxZ: 0.5,
    label: "好調",
    color: "#16a34a",
    comment: "実力をしっかり発揮できたラウンドです。"
  }, {
    maxZ: 1.0,
    label: "標準",
    color: "#0ea5e9",
    comment: "実力相応のスコアです。弱点の改善で次のレベルへ。"
  }, {
    maxZ: 2.0,
    label: "やや不調",
    color: "#fbbf24",
    comment: "実力より少し落ちたラウンド。何が原因か振り返りましょう。"
  }, {
    maxZ: Infinity,
    label: "不調",
    color: "#dc2626",
    comment: "課題が重なったラウンドです。特定の弱点に集中して対処を。"
  }];
  const lvl = gapLevels.find(l => z <= l.maxZ) ?? gapLevels[gapLevels.length - 1];
  ...
  const gradeOf = (score, hcpForTee = null, idealGIRForLong = null, isShort = false, isDetailShort = false, isPutt = false) => {
    ...
    // パット専用グレード（新式 Par×0.4+HC×0.15−パット−超過+1パット×0.5 に対応した帯）
    if (isPutt) {
      if (score >= 1.5) return {
        label: "S",
        color: "#D4A017"
    ...
    if (idealGIRForLong !== null) {
      ...
      if (score > -ig * 0.3) return {
        label: "A",
        color: "#3FA34D"
      };
      if (score > -ig * 0.5) return {
        label: "B",
        color: "#4DA8DA"
      };
      if (score > -ig * 0.7) return {
        label: "C",
        color: "#C2B280"
      };
      return {
    ...
    if (hcpForTee !== null) {
      ...
      if (score > h * 0.3) return {
        label: "S",
        color: "#D4A017"
      };
      if (score > h * 0.1) return {
        label: "A",
        color: "#3FA34D"
      };
      if (score > 0) return {
    ...
    // ショートゲーム専用：簡易モードと詳細モードで閾値が異なる
    if (isShort) {
      if (isDetailShort) {
        // 詳細モード（距離+パット方式: 値域 ~-4〜+3）
        if (score > 2.0) return {
          label: "S",
          color: "#D4A017"
        };
      ...
      // 簡易モード（方針B確定式：bonus−loss+relief、値域 ~-8〜+6 に合わせた帯）
      if (score > 3.0) return {
        label: "S",
        color: "#D4A017"
      };
  ...
  // --- カテゴリ別インサイト（HC帯考慮・ドライバー練習言及なし） ---
  const teeGood = count("teeEval", "○"),
    teeBad = count("teeEval", "×"),
    teeFair = count("teeEval", "△");
  // ロング/ショート品質配列（analytics から正確な集計を使用）
  // - 詳細モード: shot-level の100Y超 / 100Y以内 ショットを全部反映（複数ショット・null距離も含む）
  // - 簡易モード: Par3ティ + secondEval（ロング）、thirdEval + approachEval（ショート）
  const longQuals = sa.longShotQuals || [];
  const longGood = longQuals.filter(v => v === "○").length / rCount;
  const longBad = longQuals.filter(v => v === "×").length / rCount;
  ...
  const shortBad = shortQuals.filter(v => v === "×").length / rCount;
  const shortTotal = shortQuals.length / rCount;
  const diagIdealGIR = sa.idealGIR ?? null;
  const insights = {
    tee: (() => {
      ...
      // C / D
      return sa.totalOB + sa.totalPen >= 4 ? `OB/ペナルティ${fmtN(sa.totalOB + sa.totalPen)}回。罰打がスコアを直撃しています` : teeBad >= 5 ? `ティショット×が${fmtN(teeBad)}回。インパクト精度の安定が最優先課題` : teeBad >= 3 ? `ティショットミス${fmtN(teeBad)}回。スイングのインパクトゾーンを確認しよう` : band.id === "S" || band.id === "A+" || band.id === "A" ? "△が多め。フェース管理とインパクトゾーンの精度向上で○率を高めよう" : "ティショット△が多め。コンパクトなスイングで芯当て率を上げよう";
    })(),
    long: (() => {
      ...
      if (g.label === "B") {
        return longBad >= 2 ? `ロングゲーム×${fmtN(longBad)}回。番手選びと方向性を見直そう` : "ロングゲームは標準的。パーオン率の向上でスコアが変わります";
      }
      // C / D
      return longBad >= 4 ? `ロングゲーム×${fmtN(longBad)}回。番手選びと方向性を見直そう` : "パーオンが期待を下回っています。グリーンを捉える精度向上が最優先課題";
    })(),
    short: (() => {
      const g = gradeOf(sa.shortScore, null, null, true, sa.isDetailMode);
      if (g.label === "D" || g.label === "C") {
        return shortBad >= 4 ? `ショートゲーム×${fmtN(shortBad)}回。グリーン周りの精度が最重要課題` : "寄せ後に1パットで上がるためのエリアを意識しましょう";
      }
      return shortBad >= 4 ? `ショートゲーム×${fmtN(shortBad)}回。グリーン周りの精度が最重要課題` : sa.avgAfterPutts > 2.3 ? band.id === "S" || band.id === "A+" || band.id === "A" ? `寄せ後の平均${sa.avgAfterPutts}パット。ピン2m以内の寄せ率を意識しよう` : `寄せ後の平均${sa.avgAfterPutts}パット。寄せ精度を高めてパットを楽にしよう` : shortGood >= shortTotal * 0.6 ? `ショートゲーム○${fmtN(shortGood)}回。グリーン周りが強みです` : "ショートゲームの安定感をもう少し上げましょう";
    })(),
    putt: (() => {
  ...
  const categories = [{
    ...
    score: sa.teeScore,
    hcpForGrade: hcp,
    idealGIRForGrade: null
  }, {
    key: "long",
    label: "ロングゲーム",
    score: sa.longScore,
    hcpForGrade: null,
    idealGIRForGrade: diagIdealGIR
  }, ...(shortTotal > 0 && sa.shortScore != null ? [{
    key: "short",
    label: "ショートゲーム",
    score: sa.shortScore,
    hcpForGrade: null,
  ...
  // 最低スコア要素（横ばい時の補足用）：displayScore で比較可能に
  const sortedCats = [...categories].sort((a, b) => (a.displayScore ?? a.score) - (b.displayScore ?? b.score));
  const weakestCat = sortedCats[0];

  // グレードがC/D かどうかを判定するヘルパー
  const isWeak = cat => {
  ...
  // rest：グレードC/D かつ強制フラグ未使用、displayScoreの昇順（悪い順）
  ...
  const prevRounds = (rounds || []).filter(r => r.isComplete && r.id !== roundId && Object.keys(r.simpleHoleData || {}).length > 0).filter(r => {
    ...
    if (!(is9H ? rCount <= 12 : rCount > 12)) return false;
    return dateToNum(r.date) < currentDateNum; // 現在ラウンドより前の日付のみ
  }).sort((a, b) => dateToNum(b.date) - dateToNum(a.date));
  let trend = null;
  if (prevRounds.length > 0) {
    const prev = prevRounds[0];
    const prevSA = calcAnalytics(prev, hcp, null); // トレンド比較はfallback（teeRatesなし）
    if (prevSA) {
...
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// スコア差分シンボル（HOME・ANALYTICSで共用）
// ─────────────────────────────────────────────────────────────────────────────
// 日付文字列 (YYYY/M/D) を数値に変換（ソート用）
...
function AiDiagnosisPanel({
  ...
  let d;
  try {
    d = generateDiagnosis(sa, shd, hcp, rounds, roundId, roundCount, sa5?.totalScore ?? null);
  } catch (e) {
    return null;
  }
  if (!d || !d.lvl || !d.band) return null;
  // Rexyの衣装：ラウンド別診断はそのラウンドが雨/雪なら rain5、それ以外と集計(roundId無)は basic6
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, label), roundCount != null && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "9px",
      color: "#64748b",
      background: "#f1f5f9",
  ...
  }, roundCount, "\u30E9\u30A6\u30F3\u30C9\u5E73\u5747"), dateRange && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "9px",
      color: "#94a3b8"
    }
  ...
  }, "\u7DCF\u5408\u8A55\u4FA1"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "17px",
      fontWeight: "900",
      color: d.lvl.color
    }
  }, d.lvl.label), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "9px",
      color: "#64748b",
      marginTop: "2px"
    }
  }, "HC ", hcp, " (", d.band.label, ")")), /*#__PURE__*/React.createElement("div", {
    style: {
      marginLeft: "auto",
      textAlign: "right"
    }
    ...
    style: {
      fontSize: "22px",
      fontWeight: "800",
      color: d.gap >= 0 ? "#f97316" : "#16a34a",
      lineHeight: 1
    }
  }, d.gap >= 0 ? "+" : "", d.gap), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "9px",
      color: "#64748b"
    }
  }, sa5 ? "直近5R平均／" : "", "\u671F\u5F85", d.expectedScore, "\u6BD4"))), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      lineHeight: 1.65
    }
  }, d.lvl.comment)), radarSlot, /*#__PURE__*/React.createElement("div", {
    className: "dag2",
    style: {
      background: "#ffffff",
      border: "1px solid #e2e8f0",
  ...
  }, /*#__PURE__*/React.createElement("span", null, "5\u8981\u7D20\u5206\u6790")), /*#__PURE__*/React.createElement("button", {
  ...
  }, "\u2139 \u7528\u8A9E")), sa5 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
  ...
  }, "\u76F4\u8FD15R"), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#0ea5e9",
      fontWeight: "700",
      minWidth: "32px",
  ...
  }, "\u76F4\u8FD120R")), showDefs && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "9px",
      color: "#64748b",
      background: "#f8fafc",
  ...
  }, "\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8"), ": Par4/5\u306E\u6700\u521D\u306E\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\uFF08Par3\u306F\u9664\u5916\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: "3px"
    }
  }, /*#__PURE__*/React.createElement("b", {
  ...
  }, "\u30ED\u30F3\u30B0\u30B2\u30FC\u30E0"), ": 100Y\u8D85\u306E\u30B7\u30E7\u30C3\u30C8\uFF08Par3\u3067100Y\u8D85\u306E\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u542B\u3080\uFF09"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", {
    style: {
      color: "#1e293b"
    }
  }, "\u30B7\u30E7\u30FC\u30C8\u30B2\u30FC\u30E0"), ": 100Y\u4EE5\u5185\u306E\u30B7\u30E7\u30C3\u30C8\uFF08Par3\u3067100Y\u4EE5\u5185\u306E\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u542B\u3080\uFF09")), d.categories.map(cat => {
    // 20Rグレード（右列用）
    const g20 = d.gradeOf(cat.score, cat.hcpForGrade, cat.idealGIRForGrade, cat.isShort, cat.isDetailMode, cat.isPutt);
    // 直近5Rデータがあれば5Rのスコアでバー・グレードを表示
    const score5 = sa5 ? (() => {
      const keyMap = {
        tee: "teeScore",
        long: "longScore",
      ...
      return sa5[keyMap[cat.key]] ?? null;
    })() : null;
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        height: "100%",
        width: d.barW(computeDs5),
        background: d.barC(computeDs5),
        borderRadius: "3px"
      }
    })), /*#__PURE__*/React.createElement("span", {
      style: {
    ...
    }, (useScore >= 0 ? "+" : "") + (useScore ?? 0).toFixed(1)), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "12px",
        fontWeight: "900",
        color: g5.color,
    ...
    }, g5.label), sa5 && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "700",
        color: g20.color,
    ...
    }, g20.label)), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#64748b",
        paddingLeft: "24px",
  ...
  }), d.weaknesses.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "dag3",
    style: {
      marginTop: "14px",
      paddingTop: "14px",
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u26A0\uFE0F"), /*#__PURE__*/React.createElement("span", null, "\u6539\u5584\u8AB2\u984C\uFF08\u512A\u5148\u9806\uFF09")), d.weaknesses.map((w, i) => /*#__PURE__*/React.createElement("div", {
    key: w.key,
    style: {
      display: "flex",
      ...
      gap: "10px",
      padding: "10px 4px",
      borderBottom: i < d.weaknesses.length - 1 ? "1px solid #f1f5f9" : "none"
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "18px",
      fontWeight: "900",
      color: ["#dc2626", "#fbbf24", "#94a3b8"][i],
      flexShrink: 0,
      lineHeight: 1.2,
      minWidth: "16px",
      textAlign: "center"
  ...
  }, w.insight))))), /*#__PURE__*/React.createElement("div", {
    className: "dag4",
    style: {
      marginTop: "14px",
      paddingTop: "14px",
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u6539\u5584\u30A2\u30C9\u30D0\u30A4\u30B9")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "10px",
      alignItems: "flex-start"
    }
  }, REXY_IMAGES[rexyCostume] && /*#__PURE__*/React.createElement(RexyIcon, {
    costume: rexyCostume,
    size: 64,
    alt: "",
    style: {
  ...
  }, d.advice.map((a, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      display: "flex",
      gap: "9px",
  ...
  }, a.priority), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#475569",
      lineHeight: 1.6
    }
  }, a.text)))))))), showTrend && d.trend && /*#__PURE__*/React.createElement("div", {
    className: "dag5",
    style: {
      padding: "10px 12px",
      background: "#f8fafc",
  ...
  }, "\u30C8\u30EC\u30F3\u30C9"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "12px",
      fontWeight: "800",
      color: d.trend.color
    }
  }, d.trend.label), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "9px",
      color: "#94a3b8",
      marginLeft: "auto"
    }
  }, "\u524D\u56DE: ", d.trend.prevDate)), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      lineHeight: 1.6
...
function deriveSimpleHoleData(holeData, holePars) {
  ...
  Object.entries(holeData).forEach(([hStr, hInfo]) => {
    if (!hInfo?.done) return;
    const h = parseInt(hStr);
    const par = pars[h - 1] || 4;
    const shots = hInfo.shots || [];

    // 新フォーマット判定（quality フィールドがあれば新フォーマット）
    const isNewFmt = shots.some(s => s.quality !== undefined);
    ...
    if (isNewFmt) {
      ...
      // ロング: 最初の100Y超アプローチショット（remainDist が LONG_DIST_SET に一致）
      ...
      // ショート: 最初の100Y以内アプローチショット（remainDist が SHORT_DIST_SET に一致）
...
// ─────────────────────────────────────────────────────────────────────────────
// エラーバウンダリー：RoundCard 内のクラッシュを捕捉して黒画面を防ぐ
class RoundCardErrorBoundary extends React.Component {
  ...
  componentDidCatch(err) {
    console.warn("[RoundCard] render error caught:", err?.message);
  }
  render() {
    if (this.state.hasError) {
      ...
      }, "\u26A0\uFE0F \u3053\u306E\u30E9\u30A6\u30F3\u30C9\u306E\u8868\u793A\u4E2D\u306B\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\uFF08\u30C7\u30FC\u30BF\u5F62\u5F0F\u3092\u3054\u78BA\u8A8D\u304F\u3060\u3055\u3044\uFF09");
    }
...
function GolfTracker() {
  ...
  // 簡易モード用ホールデータ
  // { [hole]: { score, putts, ob, penalty, bunker, teeEval, secondEval, thirdEval, approachEval } }
  const [simpleHoleData, setSimpleHoleData] = useState({});
  const [rounds, setRounds] = useState([]);
  const [importedTestData, setImportedTestData] = useState([]); // window.storageから動的読み込み
  const [dataMgmtOpen, setDataMgmtOpen] = useState(false); // データ管理(書き出し/インポート)アコーディオン：既定で閉じる
  const [storageLoaded, setStorageLoaded] = useState(false); // ロード完了前の上書き防止フラグ
  const [currentRound, setCurrentRound] = useState(null);
  const [currentHole, setCurrentHole] = useState(1);
  const [holePars, setHolePars] = useState(Array(18).fill(4));

  // holeData: { [hole]: { shots: [...], state: string, done: bool, extraPenalty?: number } }
  ...
  const [showHcpWarning, setShowHcpWarning] = useState(false); // HCP未設定警告

  // ══ 課金テスト設定（フリーミアム需要検証） ════════════════════════════════
  // ★公開前に差し替え必須：note記事URL（購入）
  const BUY_LINK_URL = "https://note.com/yostvt/n/n1b365d1d69da"; // note有料記事（詳細モード解除コード販売）
  const DETAIL_FREE_LIMIT = 3; // 詳細モード無料登録上限（ラウンド数）
  const DETAIL_PRICE_LABEL = "¥500"; // 表示価格（買い切り・仮）
  // 解除コード（note有料エリアに記載して配布）。複数可・大小文字は無視。
  const UNLOCK_CODES = ["SCRX-EZ6U-98QV"];
  const [detailUnlocked, setDetailUnlocked] = useState(false); // 詳細モード解除済みフラグ
  const [showDetailPaywall, setShowDetailPaywall] = useState(false); // 需要プローブ画面
  ...
  const [scoreDisplayMode, setScoreDisplayMode] = useState("number"); // "number" | "symbol"（ホールバイホール「スコア」行の表記切替）
  const [courseName, setCourseName] = useState("");
  // ── ラウンド設定 state ──────────────────────────────────────────────────────
  const [showYardage, setShowYardage] = useState(false);
  // 新ラウンド設定フォーム
  const [setupStep, setSetupStep] = useState(0); // 0=場所 1=コース 2=設定
  const [selectedVenue, setSelectedVenue] = useState(null); // venue id
  const [venueDropdownOpen, setVenueDropdownOpen] = useState(false); // 候補リスト開閉
  const [venueTab, setVenueTab] = useState("fav"); // "fav" | "aiueo"
  const [selectedCourseA, setSelectedCourseA] = useState(null); // 前半サブコース index
  const [selectedCourseB, setSelectedCourseB] = useState(null); // 後半サブコース index
  const [selectedGreen, setSelectedGreen] = useState(null); // green id
  const [selectedTee, setSelectedTee] = useState(null); // tee id
  const [selectedWeather, setSelectedWeather] = useState("sunny"); // 天気
  const [selectedWind, setSelectedWind] = useState(0); // 風 0-5
  const [selectedDate, setSelectedDate] = useState(() => {
    const d = new Date();
    return `${d.getFullYear()}/${String(d.getMonth() + 1).padStart(2, '0')}/${String(d.getDate()).padStart(2, '0')}`;
  });
  const [editingRoundId, setEditingRoundId] = useState(null); // 日付・天気・風編集中のラウンドID
  const [editDate, setEditDate] = useState("");
  const [editWeather, setEditWeather] = useState("sunny");
  const [editWind, setEditWind] = useState(0);
  const [savedClubs, setSavedClubs] = useState([]); // 永続クラブセット
  const [showHcpHint, setShowHcpHint] = useState(false); // ヘッダー👤のHCP未設定ヒント（起動毎1回・数秒）
  ...
  const [profileEdit, setProfileEdit] = useState(null); // 編集中の一時値
  ...
  const announcedRef = React.useRef(new Set()); // detail mode: 挨拶済みホール
  const prevHcpRef = React.useRef(undefined); // ハンディキャップ目標監視用
  const hcpInitializedRef = React.useRef(false);

  // showToast(message, pose): pose はトーストに出すRexyのポーズID。
  //  - 省略時は null（描画側で従来どおり天気連動の汎用ポーズ）
  //  - 天気連動させたい場合は呼び出し側で rexyPoseWeather(晴れポーズ, 天気) を渡す
  const showToast = (message, pose = null) => {
    ...
    toastTimerRef.current = setTimeout(() => setToast({
      message: "",
      visible: false,
      pose: null
    }), 2800);
  ...
  // スコア種別トースト（イーグル〜パー）。雨/雪ラウンドでは雨ポーズに連動。
  const scoreToast = (score, par) => {
    const diff = score - par;
    const w = currentRound ? currentRound.weather : null;
    if (diff <= -3) showToast("アルバトロス！！まさかこの目で見られるとは思わなかったよ～", rexyPoseWeather("basic3", w));else if (diff === -2) showToast("イーグル！！Rexy感激！", rexyPoseWeather("basic3", w));else if (diff === -1) showToast("やったーーー！ナイスバーディ！すごいよ～", rexyPoseWeather("basic1", w));else if (diff === 0) showToast("ナイスパー！この調子！", rexyPoseWeather("basic4", w));
  };
  const saveProfile = p => {
    setProfile(p);
    // useEffectでの自動保存をトリガー（setProfileによる state 変化で発火）
  };
  ...
  useEffect(() => {
    ...
    // window.storage（Claudeアーティファクト環境）にも保存
    if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
      window.storage.set('golf_profile', JSON.stringify(profile)).catch(() => {});
    }
  }, [profile, storageLoaded]);

  // ── 永続化ロード（起動時） ────────────────────────────────────────────────────
  useEffect(() => {
    // 同期的にlocalStorageから全データを読み込む（非同期ラッパー不要）
    let localStorageHadProfile = false; // localStorageにプロフィールがあったかフラグ
    try {
        ...
        if (Array.isArray(parsed)) {
          ...
          const migrated = parsed.map(r => {
            ...
            if (base.inputMode === "detail" && Object.keys(base.simpleHoleData || {}).length === 0 && base.holeData) {
              return {
                ...base,
                simpleHoleData: deriveSimpleHoleData(base.holeData, base.holePars)
              };
            }
            return base;
          });
    ...
    try {
      ...
      if (raw) {
        ...
        // window.storageが使えない場合のみここでトースト表示（使える場合はasync側で表示）
    ...
    // window.storage からのロード（Claudeアーティファクト環境 / localStorageが使えない場合の永続化）
    if (typeof window !== 'undefined' && window.storage && typeof window.storage.get === 'function') {
      (async () => {
        // プロフィール（localStorageで読めなかった場合のみ上書き）
        try {
          const result = await window.storage.get("golf_profile");
          if (!localStorageHadProfile) {
            // localStorage に無かった場合のみ window.storage から読む
        ...
        // クラブ設定（localStorageで読めなかった場合のみ）
        try {
          const result = await window.storage.get("golf_clubs");
          if (result && result.value) {
            const parsed = JSON.parse(result.value);
            if (Array.isArray(parsed)) {
        ...
        // ラウンドデータ（localStorageで読めなかった場合のみ）
        if (!localStorageHadRounds) {
          try {
            const result = await window.storage.get("golf_rounds");
            if (result && result.value) {
              const parsed = JSON.parse(result.value);
              if (Array.isArray(parsed)) {
                const migrated = parsed.map(r => {
        ...
        // テストデータ読み込み
        try {
          const result = await window.storage.get("golf_test_data");
          if (result && result.value) {
            const parsed = JSON.parse(result.value);
            if (Array.isArray(parsed)) {
              const migrated = parsed.map(r => {
    ...
    setStorageLoaded(true); // 同期ロード完了 → 以降の変更は保存を許可
  }, []);
  const [roundMemo, setRoundMemo] = useState(""); // ラウンドメモ（20文字以内）
  const [selectedOpt, setSelectedOpt] = useState(null);
  const [selectedClub, setSelectedClub] = useState(null);
  const [shotNote, setShotNote] = useState("");
  const [pendingOptId, setPendingOptId] = useState(null); // 選択済みoptionId（クラブ選択ステップ用）
  const [pendingBadSub, setPendingBadSub] = useState(false); // × サブ選択ステップ表示フラグ
  const [pendingClubId, setPendingClubId] = useState(null); // クラブ選択後・距離入力前の保持
  const [pendingDistStep, setPendingDistStep] = useState(false); // 残り距離入力ステップ表示フラグ
  // ── 新UI用state ──────────────────────────────────────────────────
  const [newResultId, setNewResultId] = useState(null); // 結果選択
  const [newBadSubId, setNewBadSubId] = useState(null); // ×サブ選択
  const [approachDistVal, setApproachDistVal] = useState(100); // アプローチ距離スライダー値(Y)
  const [puttDistVal, setPuttDistVal] = useState(5); // パット距離スライダー値(m)
  const [teeDistOverride, setTeeDistOverride] = useState(null); // ティ距離上書き値(null=自動)
  const [calcOpen, setCalcOpen] = useState(false); // 電卓展開中フラグ
  const [calcInput, setCalcInput] = useState(""); // 電卓入力バッファ
  const [showPinDistStep, setShowPinDistStep] = useState(false); // パット後カップ距離選択

  const par = holePars[currentHole - 1];

  // Current hole state
  const hd = holeData[currentHole] || {
    shots: [],
    state: initialCategory(par),
    done: false
  };
  ...
  // All shots flat for score totals
  ...
  // Open form for current hole's next shot
  const openShotForm = () => {
    setSelectedOpt(null);
    setSelectedClub(null);
  ...
  const recordShot = (optId, clubId = null, remainDist = null, pinDistLabel = null, remainDistRaw = null) => {
    ...
    if (SUB_IDS.includes(optId)) {
      ...
      opt = currentCatDef?.options.find(o => o.id === optId);
    }
    if (!opt) return;
    ...
    setHoleData(prev => ({
      ...prev,
      [currentHole]: {
        shots: [...(prev[currentHole]?.shots || []), shot],
        state: nextState,
        done: nextState === "done",
        pinDist: pinDistLabel ?? prev[currentHole]?.pinDist,
        extraPenalty: prev[currentHole]?.extraPenalty
      }
    }));
    ...
    if (nextState === "done") {
      ...
      if (currentCategory === "putt") {
        if (pinDistLabel) {
          // pinDistは既にholeDataに保存済み → 直接次ホールへ
  ...
  const undoLastShot = () => {
    ...
    for (const sh of prev) {
      let opt = CATEGORIES[sh.categoryKey]?.options.find(o => o.id === sh.optionId);
      if (!opt) {
        // × サブオプションを探す
        const subList = BAD_SUB_OPTIONS[sh.categoryKey] || BAD_SUB_OPTIONS.approach;
        opt = subList?.find(o => o.id === sh.optionId);
      }
      if (!opt) continue;
      if (opt.nextState === "done") {
        state = "done";
    ...
    setHoleData(p => ({
      ...
      [currentHole]: {
        ...
        pinDist: undefined,
        extraPenalty: p[currentHole]?.extraPenalty
      }
    }));
  };
  ...
  const handleRecordNew = () => {
    ...
    // optId解決（×サブ）
    const finalOptId = newBadSubId ?? newResultId;
    // 距離ラベルと生数値
    let distLabel = null;
    let distRaw = null;
    if (currentCategory === "tee") {
      const venue = currentRound?.venueId ? VENUES.find(v => v.id === currentRound.venueId) : null;
  ...
  const applyUnlockCode = () => {
    ...
    if (UNLOCK_CODES.map(c => c.toUpperCase()).includes(code)) {
      ...
      if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
        window.storage.set('golf_detail_unlocked', '1').catch(() => {});
      }
  ...
  const doStartRound = () => {
    ...
    const label = is9H ? `${venue.name}（${frontSC.name}・9H）` : venue.subCourses.length === 1 ? venue.name : frontSC.name === backSC.name ? `${venue.name}（${frontSC.name}×2）` : `${venue.name}（${frontSC.name}→${backSC.name}）`;

    // 表示用ホール番号（マスタのhole値）を保存
    ...
    setCurrentRound({
      ...
      goal: roundGoal.type && inputMode === "detail" ? {
        type: roundGoal.type,
        target: roundGoal.target
      } : null
    });
    setHoleData({});
    setSimpleHoleData({});
    setCurrentHole(1); // 常に1番からスタート
    setHolePars(pars);
    setView("round");
  ...
  // ── roundGoal はuseEffectの依存配列で参照するため、useEffect群より前に宣言 ──
  ...
  useEffect(() => {
    ...
    // window.storage（Claudeアーティファクト環境）にも保存
    if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
      window.storage.set('golf_clubs', JSON.stringify(savedClubs)).catch(() => {});
    }
  }, [savedClubs, storageLoaded]);

  // ── 完了ラウンドの永続化 ───────────────────────────────────────────────────
  useEffect(() => {
    if (!storageLoaded) return; // 起動時の空上書きを防ぐ
    (() => {
      try {
        localStorage.setItem('golf_rounds', JSON.stringify(rounds));
      } catch (_) {}
    })();
    // window.storage にも保存（アーティファクト更新時のデータ引継ぎ用）
    if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
      window.storage.set('golf_rounds', JSON.stringify(rounds)).catch(() => {});
    }
  ...
  const finishRound = () => {
    ...
    const courseName = currentRound?.course || "";
    const roundWeather = currentRound?.weather || null;
    if (inputMode === "simple") {
      ...
      totalScore = Object.values(simpleHoleData).reduce((a, h) => a + (h.score || 0), 0);
      setRounds(prev => [{
        ...currentRound,
        shots: [],
        holePars: [...holePars],
      ...
      totalScore = Object.values(holeData).reduce((a, h) => a + h.shots.reduce((s, sh) => s + sh.shotCount, 0) + (h.extraPenalty || 0), 0);
      // 詳細モードでも simpleHoleData を生成して統合分析を可能にする
    ...
    // ベストスコア更新チェック（18H完了かつスコアあり）
    if (isComplete && totalScore > 0 && (profile.bestScore === null || totalScore < profile.bestScore)) {
      saveProfile({
        ...profile,
  ...
  const resumeRound = r => {
    ...
    const firstIncomplete = Array.from({
      length: resumeHoleCount
    }, (_, i) => i + 1).find(h => r.inputMode === "simple" ? !r.simpleHoleData?.[h] : !r.holeData?.[h]?.done);
  ...
  const [skipConfirm, setSkipConfirm] = useState(null); // 簡易モード：ショット評価未入力で「次へ」した時の確認

  const handleDetailFinishClick = () => {
    const allHoles = Array.from({
      length: holePars.length
    }, (_, i) => i + 1);
    const incomplete = allHoles.filter(h => !holeData[h]?.done);
    if (incomplete.length > 0) {
      setFinishConfirm({
        type: "incomplete",
        incompleteHoles: incomplete
      });
    } else {
      setFinishConfirm({
        type: "ok"
  ...
  const handleImport = e => {
    ...
    reader.onload = ev => {
      try {
        ...
        const imported = parsed.rounds ?? (Array.isArray(parsed) ? parsed : null);
        if (!Array.isArray(imported)) throw new Error("フォーマット不正");
        const migrated = imported.map(r => {
          // shots フィールドがない場合は空配列で補完
          const base = {
            ...r,
            shots: r.shots || []
          };
        ...
        setImportMsg({
          type: "ok",
          text: `${imported.length}件を読み込みました（重複は除外）`
        });
      } catch (err) {
        setImportMsg({
          type: "error",
          text: "読み込み失敗：正しいバックアップファイルか確認してください"
        });
      }
      e.target.value = "";
  ...
  const handlePasteImport = () => {
    try {
      ...
      setImportMsg({
        type: "error",
        text: "読み込み失敗：正しいJSONか確認してください"
      });
    }
    setTimeout(() => setImportMsg(null), 4000);
  };

  // 内部キー(1〜)→マスタのホール表示番号に変換
  const getDisplayHoleNum = holeKey => {
    ...
    if (holeKey >= 1 && holeKey <= fLen) return fNums[holeKey - 1] ?? holeKey;
    if (holeKey > fLen) return bNums[holeKey - fLen - 1] ?? holeKey;
    return holeKey;
  };

  // When par changes, reset this hole's data
  const setPar = (h, p) => {
    setHolePars(prev => {
      const a = [...prev];
      a[h - 1] = p;
  ...
  const handicap = useMemo(() => {
    // 18H完了ラウンドのみ対象（9Hラウンドはハンディキャップ算出に含めない）
    const completed = [...rounds.filter(r => r.isComplete && (r.holePars?.length ?? 18) >= 18)].sort((a, b) => dateToNum(b.date) - dateToNum(a.date));
    if (completed.length < 3) return null; // 最低3ラウンド必要

    // 直近20ラウンド
    const target = completed.slice(0, 20);

    // 各ラウンドの差分（スコア - コースPar）を算出
    const diffs = target.map(r => {
      let totalScore = 0;
      let totalPar = 0;
      // simpleHoleData を優先（detail/simple 両モード共通・常に存在）
      const shd = r.simpleHoleData || {};
      const shdKeys = Object.keys(shd);
      if (shdKeys.length > 0) {
        totalScore = Object.values(shd).reduce((a, h) => a + (h.score || 0), 0);
        totalPar = shdKeys.reduce((a, k) => a + ((r.holePars || [])[parseInt(k) - 1] || 4), 0);
      } else if (r.inputMode !== "simple") {
        // fallback: detail mode の shots 配列から算出
        const shots = r.shots || [];
        totalScore = shots.reduce((a, s) => a + s.shotCount, 0);
        totalPar = (r.holePars || Array(18).fill(4)).reduce((a, p) => a + p, 0);
      }
      if (!totalPar || !totalScore) return null;
      return {
  ...
  // ── ハンディキャップ目標達成の監視（handicap 定義後に配置） ─────────────────────
  // 分析に使用する実効ハンディキャップ
  // hcpMode === "auto"   → 過去ラウンドから算出した値（なければ手入力）
  // hcpMode === "manual" → 手入力値を優先
  const effectiveHcp = useMemo(() => {
    const m = profile.manualHcp !== null && profile.manualHcp !== undefined ? profile.manualHcp : null;
    const mode = profile.hcpMode ?? "auto";
    if (mode === "manual") {
      // 手入力優先：手入力があればそれを、なければ自動算出
      return m !== null ? m : handicap !== null ? handicap.hcp : null;
    }
    // auto: 自動算出があればそれを、なければ手入力
    if (handicap !== null) return handicap.hcp;
    return m;
  }, [handicap, profile.manualHcp, profile.hcpMode]);

  // HCP未設定なら起動毎に1回、ヘッダー👤のヒントを数秒表示（storageロード後に判定）
  const hcpHintShownRef = useRef(false);
  useEffect(() => {
    if (!storageLoaded || hcpHintShownRef.current) return;
    hcpHintShownRef.current = true; // 起動中は1回だけ
    if (effectiveHcp === null) {
  ...
  useEffect(() => {
    ...
    if (curr !== null && curr !== prevHcpRef.current) {
      ...
      // (b) 改善した時だけ：前回より小さくなった(curr<prev) かつ 目標HCP以下。初回(prev=null)は発火しない
      if (target != null && prev != null && curr < prev && curr <= target) {
        setTimeout(() => showToast(`ハンディキャップが${curr}になったよ！おめでとう！！`, "basic1"), 1000);
      }
    }
    prevHcpRef.current = curr;
  }, [handicap?.hcp]);
  ...
  const S = {
    ...
    navBtn: a => ({
      ...
      fontSize: "12px",
      fontWeight: "600",
      background: a ? "rgba(22,163,74,0.12)" : "transparent",
      color: a ? "#16a34a" : "#94a3b8"
    }),
    page: {
      padding: "16px 14px",
      maxWidth: "480px",
    ...
    optBtn: (sel, col) => {
      ...
      return {
        ...
        padding: "12px 13px",
        borderRadius: "9px",
        border: sel ? `1.5px solid ${c.border}` : "1px solid #e2e8f0",
  ...
  const RoundSetupForm = () => {
    ...
    const selRow = (items, val, setter, colorSel = "#16a34a") => /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "6px",
        flexWrap: "wrap"
      ...
      const active = val === (it.id !== undefined ? it.id : i);
      return /*#__PURE__*/React.createElement("button", {
        key: key,
        onClick: () => setter(it.id !== undefined ? it.id : i),
        style: {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A"), /*#__PURE__*/React.createElement("button", {
      style: S.btn("ghost"),
      onClick: onClose
    }, "\u2715")), /*#__PURE__*/React.createElement("div", {
      style: {
      ...
      style: S.lbl
    }, "\u5165\u529B\u30E2\u30FC\u30C9"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "8px"
      }
    ...
    }, m.desc))))), inputMode === "detail" && (detailUnlocked ? /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: "-6px",
        marginBottom: "14px",
        fontSize: "11px",
      ...
      return /*#__PURE__*/React.createElement("div", {
        style: {
          ...
          fontSize: "11px",
          fontWeight: "700",
          color: remain > 0 ? "#0ea5e9" : "#d97706"
        }
      }, remain > 0 ? `詳細モードは無料であと ${remain} ラウンド登録できます` : `無料お試し（${DETAIL_FREE_LIMIT}ラウンド）は終了 ・ ${DETAIL_PRICE_LABEL}で詳細モードが使えます（お試し販売中）`);
    })()), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
      ...
      const venueRow = v => {
        ...
        return map[c] !== undefined ? map[c] : 10;
      };
      ...
      }, v.id === selectedVenue ? /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#16a34a",
          fontSize: "12px",
          width: "16px"
      ...
      }, [["fav", `お気に入り（${favVenues.length}）`], ["aiueo", "五十音順"]].map(([key, label]) => /*#__PURE__*/React.createElement("button", {
        key: key,
      ...
      }, venueTab === "fav" ? favVenues.length === 0 ? /*#__PURE__*/React.createElement("div", {
        style: {
          padding: "24px 16px",
          textAlign: "center",
          color: "#94a3b8",
      ...
      }, "\u304A\u6C17\u306B\u5165\u308A\u304C\u672A\u767B\u9332\u3067\u3059", /*#__PURE__*/React.createElement("br", null), "\u30DE\u30A4\u30DA\u30FC\u30B8\u3067\u767B\u9332\u3057\u3066\u304F\u3060\u3055\u3044") : favVenues.map(v => /*#__PURE__*/React.createElement(VenueItem, {
        key: v.id,
        v: v
      ...
      }, existingRows.map(label => /*#__PURE__*/React.createElement("button", {
        key: label,
        onClick: () => {
      ...
      }, g.label), g.venues.map(v => /*#__PURE__*/React.createElement(VenueItem, {
        key: v.id,
        v: v
      })))))));
      ...
      style: S.lbl
    }, venue.subCourses.length === 1 ? "コース" : "前半コース"), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "10px"
      }
    }, selRow(venue.subCourses.map((c, i) => ({
      id: i,
      label: c.name
    })), selectedCourseA, idx => {
      setSelectedCourseA(idx);
      // 後半が未選択(null)の時だけペア相方を自動セット（案B）。
      // 一度後半が決まれば、前半を変えても後半は固定。27H(3コース)は自動選択しない。
    ...
    }, "#34d399")), venue.subCourses.length > 1 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u5F8C\u534A\u30B3\u30FC\u30B9"), selRow([...venue.subCourses.map((c, i) => ({
      id: i,
      label: c.name
    })), {
      id: "none",
      ...
      // グリーンが未選択なら先頭を自動選択
      if (!selectedGreen && venue.greens.length > 0) setSelectedGreen(venue.greens[0].id);
    }, "#60a5fa"))), venue && selectedCourseA !== null && selectedCourseB !== null && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u4F7F\u7528\u30B0\u30EA\u30FC\u30F3"), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "10px"
      }
    }, selRow(venue.greens, selectedGreen, setSelectedGreen, "#34d399")), /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u30C6\u30A3\u30FC"), selRow(venue.tees, selectedTee, setSelectedTee, "#fbbf24")), venue && selectedGreen && selectedTee && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement("label", {
    ...
    })), venue && selectedGreen && selectedTee && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u5929\u6C17"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "6px",
        marginBottom: "12px"
      ...
      style: {
        ...
        fontWeight: "600",
        fontSize: "12px",
        border: selectedWeather === w.id ? "2px solid #16a34a" : "1px solid #e2e8f0",
        background: selectedWeather === w.id ? "rgba(14,165,233,0.12)" : "#f8fafc",
        color: selectedWeather === w.id ? "#0ea5e9" : "#64748b"
      }
    }, w.label))), /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u98A8\u306E\u5F37\u3055"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "5px"
      }
      ...
      style: {
        ...
        fontWeight: "700",
        fontSize: "13px",
        border: selectedWind === w.val ? "2px solid #fbbf24" : "1px solid #e2e8f0",
        background: selectedWind === w.val ? "rgba(251,191,36,0.15)" : "#f8fafc",
    ...
    }, /*#__PURE__*/React.createElement("span", null, w.val), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "600",
        opacity: 0.75,
    ...
    }, w.sub, /*#__PURE__*/React.createElement("br", null), "m/s"))))), venue && selectedGreen && selectedTee && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u30E9\u30A6\u30F3\u30C9\u30E1\u30E2\uFF08\u4EFB\u610F\u30FB20\u6587\u5B57\u4EE5\u5185\uFF09"), /*#__PURE__*/React.createElement("input", {
      type: "text",
      inputMode: "text",
      enterKeyHint: "done",
      style: {
      ...
      style: {
        textAlign: "right",
        fontSize: "11px",
        color: roundMemo.length >= 20 ? "#ef4444" : "#475569",
        marginTop: "3px"
      }
    }, roundMemo.length, "/20")), inputMode === "detail" && venue && selectedGreen && selectedTee && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u4ECA\u65E5\u306E\u76EE\u6A19\uFF08\u4EFB\u610F\u30FB1\u3064\u9078\u629E\uFF09"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setRoundGoal({
        type: null,
        target: 1
      }),
      style: {
      ...
      return GOAL_TYPES.map(g => {
        ...
        }, /*#__PURE__*/React.createElement("span", null, g.label)), sel && /*#__PURE__*/React.createElement("div", {
          style: {
            display: "flex",
            alignItems: "center",
            gap: "4px",
        ...
        }, roundGoal.target), /*#__PURE__*/React.createElement("button", {
          onClick: () => setRoundGoal(prev => ({
            ...prev,
            target: Math.min(maxVal, prev.target + 1)
          })),
          style: {
            width: "30px",
            height: "30px",
    ...
    })()), canStart && venue && /*#__PURE__*/React.createElement("div", {
      style: {
    ...
    }, venue.name), /*#__PURE__*/React.createElement("div", null, "\u524D\u534A\uFF1A", venue.subCourses[selectedCourseA].name, selectedCourseB === "none" ? " / 後半：なし（9H）" : ` / 後半：${venue.subCourses[selectedCourseB].name}`), /*#__PURE__*/React.createElement("div", null, "\u30B0\u30EA\u30FC\u30F3\uFF1A", venue.greens.length > 1 ? venue.greens.find(g => g.id === selectedGreen)?.label : venue.greens[0].label, " / \u30C6\u30A3\u30FC\uFF1A", venue.tees.find(t => t.id === selectedTee)?.label), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: "3px"
      }
    }, "\u65E5\u4ED8\uFF1A", selectedDate.replace(/(\d{4})\/(\d{2})\/(\d{2})/, (_, y, m, d) => `${y}/${+m}/${+d}`), "\u3000 \u5929\u6C17\uFF1A", {
      "sunny": "☀️晴れ",
      "cloudy": "☁️曇り",
      "rainy": "🌧️雨",
      "snowy": "❄️雪"
    }[selectedWeather], "\u3000 \u98A8\uFF1A", ["0〜1m/s", "2〜3m/s", "4〜5m/s", "6〜8m/s", "9〜12m/s", "13m/s〜"][selectedWind], "\uFF08", selectedWind, "\uFF09")), /*#__PURE__*/React.createElement("button", {
      style: {
        ...S.btn("primary"),
        width: "100%",
  ...
  const RoundCard = ({
    ...
    const ScoreRow = ({
    ...
    }, "P", par), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "15px",
        color: symColor,
        lineHeight: 1,
    ...
    }, totalS), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "8px",
        color: "#94a3b8",
        lineHeight: 1
      }
    }, putts > 0 ? `${putts}P` : ""))), halfStrk > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        marginLeft: "4px",
        padding: "2px 7px",
        background: "#f8fafc",
      ...
      style: {
        fontSize: "11px",
        fontWeight: "800",
        color: halfStrk - halfPar >= 0 ? "#f97316" : "#34d399"
      }
    }, halfStrk, halfStrk - halfPar >= 0 ? `(+${halfStrk - halfPar})` : `(${halfStrk - halfPar})`), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "8px",
        color: "#475569"
      }
    ...
    // sa を IIFE の外（RoundCard スコープ）で定義 → 展開時にも参照可能
    const hasSimpleData_ = Object.keys(r.simpleHoleData || {}).length > 0;
    const sa = hasSimpleData_ ? (() => {
      try {
        return calcAnalytics(r, r.hcp ?? null, teeRates);
      } catch (e) {
        return null;
      }
    })() : null;
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          padding: "1px 6px",
          borderRadius: "10px",
          background: isSimple ? "rgba(251,191,36,0.15)" : "rgba(96,165,250,0.15)",
          color: isSimple ? "#fbbf24" : "#60a5fa",
          fontWeight: "700"
        }
      }, isSimple ? "簡易" : "詳細"), /*#__PURE__*/React.createElement("span", {
        style: {
          fontSize: "10px",
          color: "#f59e0b",
          marginLeft: "2px",
      ...
      }, /*#__PURE__*/React.createElement("span", null, r.date), r.weather && /*#__PURE__*/React.createElement("span", null, {
        "sunny": "☀️",
        "cloudy": "☁️",
        "rainy": "🌧️",
        "snowy": "❄️"
      }[r.weather]), r.wind !== undefined && /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, ["0〜1m/s", "2〜3m/s", "4〜5m/s", "6〜8m/s", "9〜12m/s", "13m/s〜"][r.wind], "\uFF08", r.wind, "\uFF09"), /*#__PURE__*/React.createElement("span", {
        style: {
          fontSize: "9px",
          color: "#94a3b8"
        }
      }, "\u270E")), r.memo && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "10px",
          color: "#94a3b8",
          marginTop: "3px"
        }
      }, r.memo), r.goal && r.inputMode === "detail" && r.holeData && (() => {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          style: {
            fontSize: "10px",
            fontWeight: "700",
            color: achieved ? "#16a34a" : "#0ea5e9"
          }
        }, goalDef.label, " ", progress, "/", r.goal.target, "H"), achieved && /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: "10px",
            color: "#16a34a",
            fontWeight: "800"
      ...
      }, !r.isComplete ? /*#__PURE__*/React.createElement("button", {
      ...
      }, "\u270F\uFE0F \u5165\u529B\u4E2D") : /*#__PURE__*/React.createElement("button", {
        onClick: () => resumeRound(r),
        style: {
          padding: "5px 12px",
          borderRadius: "20px",
      ...
      }, "\uD83D\uDD8A \u4FEE\u6B63")), sa ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontWeight: "800",
          fontSize: "20px",
          color: sa.totalScore - sa.totalPar >= 0 ? "#f97316" : "#16a34a"
        }
      }, sa.totalScore), /*#__PURE__*/React.createElement("div", {
        style: {
          color: "#94a3b8",
          fontSize: "9px"
        }
      }, "(", sa.totalScore - sa.totalPar >= 0 ? "+" : "", sa.totalScore - sa.totalPar, ")")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        style: {
          fontWeight: "800",
          fontSize: "20px",
          color: scoreColor(tot)
        }
      }, fmt(tot)), /*#__PURE__*/React.createElement("div", {
        style: {
          color: "#94a3b8",
          fontSize: "9px"
        }
      }, "\u8A55\u4FA1\u30B9\u30B3\u30A2")))), !isCollapsed && /*#__PURE__*/React.createElement(React.Fragment, null, sa && /*#__PURE__*/React.createElement("div", {
        style: {
          borderTop: "1px solid #e2e8f0",
          paddingTop: "10px"
        }
        ...
        const shd = r.simpleHoleData || {};
        const frontHoles = Array.from({
          length: 9
        }, (_, i) => i + 1).filter(h => shd[h]);
        const backHoles = Array.from({
          length: 9
        }, (_, i) => i + 10).filter(h => shd[h]);
        const frontScore = frontHoles.reduce((a, h) => a + (shd[h]?.score || 0), 0);
        const backScore = backHoles.reduce((a, h) => a + (shd[h]?.score || 0), 0);
        const frontPar = frontHoles.reduce((a, h) => a + (r.holePars?.[h - 1] || 4), 0);
        const backPar = backHoles.reduce((a, h) => a + (r.holePars?.[h - 1] || 4), 0);
        const frontLabel = r.frontCourse || "前半";
        const backLabel = r.backCourse || "後半";
        const sign = d => (d >= 0 ? "+" : "") + d;
        const items = [];
        if (frontHoles.length) items.push(`${frontLabel}：${frontScore}（${sign(frontScore - frontPar)}）`);
        if (backHoles.length) items.push(`${backLabel}：${backScore}（${sign(backScore - backPar)}）`);
        ...
        const shd = r.simpleHoleData || {};
        const pars = r.holePars || Array(18).fill(4);
        const norm = v => v === "good" ? "○" : v === "fair" ? "△" : v === "bad" ? "×" : v;
        const teeRateByPar = par => {
          const target = Object.entries(shd).filter(([h, d]) => (pars[parseInt(h) - 1] || 4) === par && d?.teeEval);
          if (!target.length) return null;
          const ok = target.filter(([, d]) => norm(d.teeEval) === "○").length;
          return Math.round(ok / target.length * 100);
        };
        const p5 = teeRateByPar(5),
          p4 = teeRateByPar(4),
          p3 = teeRateByPar(3);
        const allHoles = Object.entries(shd).filter(([, d]) => d?.score != null);
        ...
        const cell = (label, val) => /*#__PURE__*/React.createElement("span", {
          style: {
            color: "#475569"
          }
        ...
        }, "\u30C6\u30A3\u6210\u529F\u7387"), cell("Par5", p5), cell("Par4", p4), cell("Par3", p3), /*#__PURE__*/React.createElement("span", {
          style: {
            color: "#475569",
            marginLeft: "4px",
            fontWeight: "600"
          }
        }, "\u30D1\u30FC\u30AA\u30F3\u7387"), /*#__PURE__*/React.createElement("span", {
          style: {
            color: rateColor(girRate),
            fontWeight: "700"
          }
        }, girRate != null ? `${girRate}%` : "−"));
      })(), /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          justifyContent: "space-between",
      ...
      }, "\u30D1\u30C3\u30C8 ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#1e293b",
          fontWeight: "700"
        }
      }, sa.totalPutts)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "OB ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: sa.totalOB > 0 ? "#b91c1c" : "#e2e8f0",
          fontWeight: "700"
        }
      }, sa.totalOB)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "\u30DA\u30CA\u30EB\u30C6\u30A3 ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: sa.totalPen > 0 ? "#b91c1c" : "#e2e8f0",
          fontWeight: "700"
        }
      }, sa.totalPen)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "\u30D0\u30F3\u30AB\u30FC ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: sa.totalBunker > 0 ? "#fbbf24" : "#e2e8f0",
          fontWeight: "700"
        }
      }, sa.totalBunker))), /*#__PURE__*/React.createElement("button", {
        onClick: () => setShotDetailRid(shotDetailRid === r.rid ? null : r.rid),
        style: {
          padding: "3px 10px",
          borderRadius: "20px",
          border: "1px solid rgba(96,165,250,0.4)",
          background: shotDetailRid === r.rid ? "rgba(96,165,250,0.2)" : "rgba(96,165,250,0.08)",
          color: "#0ea5e9",
          fontSize: "10px",
          fontWeight: "700",
          cursor: "pointer",
        ...
        const evalColor = v => v === "○" || v === "good" ? "#15803d" : v === "△" || v === "fair" ? "#a16207" : v === "×" || v === "bad" ? "#475569" : "#94a3b8";
        const successRate = (holes, key) => {
          const valid = holes.filter(h => shd[h]?.[key] && shd[h]?.[key] !== "無" && shd[h]?.[key] !== "none");
          if (!valid.length) return "－";
          const ok = valid.filter(h => shd[h]?.[key] === "○" || shd[h]?.[key] === "good").length;
        ...
        const rows = [{
          ...
          totalFn: holes => holes.reduce((a, h) => a + (shd[h]?.score || 0), 0)
        }, {
          label: "パット数",
          key: "putts",
          isEval: false,
          render: h => shd[h]?.putts ?? "－",
          color: h => (shd[h]?.putts || 0) >= 3 ? "#dc2626" : "#e2e8f0",
          totalFn: holes => holes.reduce((a, h) => a + (shd[h]?.putts || 0), 0)
        }, {
          label: "OB",
          key: "ob",
          isEval: false,
          render: h => shd[h]?.ob || "－",
          color: h => (shd[h]?.ob || 0) > 0 ? "#dc2626" : "#475569",
          totalFn: holes => holes.reduce((a, h) => a + (shd[h]?.ob || 0), 0)
        }, {
          label: "ペナルティ",
          key: "penalty",
          isEval: false,
          render: h => shd[h]?.penalty || "－",
          color: h => (shd[h]?.penalty || 0) > 0 ? "#dc2626" : "#475569",
          totalFn: holes => holes.reduce((a, h) => a + (shd[h]?.penalty || 0), 0)
        }, {
          label: "バンカー",
          key: "bunker",
          isEval: false,
          render: h => shd[h]?.bunker || "－",
          color: h => (shd[h]?.bunker || 0) > 0 ? "#fbbf24" : "#475569",
          totalFn: holes => holes.reduce((a, h) => a + (shd[h]?.bunker || 0), 0)
        }, {
          label: "ティショット",
          key: "teeEval",
          isEval: true,
          render: h => evalLabel(shd[h]?.teeEval),
          color: h => evalColor(shd[h]?.teeEval),
          totalFn: holes => successRate(holes, "teeEval")
        }];
        const front = r.frontCourse || "前半";
        const back = r.backCourse || "後半";
        // 内部キーは1〜18、表示番号はfrontHoleNums/backHoleNums
        const _fNums = r.frontHoleNums || Array.from({
          length: 9
        }, (_, i) => i + 1);
        const _bNums = r.backHoleNums || Array.from({
          length: 9
        }, (_, i) => i + 10);
        const sections = [{
          label: front,
        ...
        return /*#__PURE__*/React.createElement("div", {
        ...
        }, sec.label), /*#__PURE__*/React.createElement("table", {
          style: {
        ...
        }, "\u30DB\u30FC\u30EB"), sec.keys.map((h, i) => /*#__PURE__*/React.createElement("td", {
          key: h,
          style: {
            padding: "3px 4px",
            textAlign: "center",
        ...
        }, (sec.nums || sec.keys)[i])), /*#__PURE__*/React.createElement("td", {
          style: {
            padding: "3px 5px",
            textAlign: "center",
            color: "#94a3b8",
        ...
        }, "\u8A08"))), /*#__PURE__*/React.createElement("tbody", null, rows.map((row, ri) => {
          ...
          return /*#__PURE__*/React.createElement("tr", {
            ...
            onClick: isScoreRow ? () => setScoreDisplayMode(m => m === "number" ? "symbol" : "number") : undefined,
            title: isScoreRow ? "タップで数字↔記号を切替" : undefined,
            style: {
              padding: "3px 5px",
          ...
          }, row.label, isScoreRow && /*#__PURE__*/React.createElement("span", {
            style: {
              marginLeft: "3px",
              fontSize: "9px",
              opacity: 0.75
            }
          }, "\u21C4")), sec.keys.map(h => /*#__PURE__*/React.createElement("td", {
            key: h,
            style: {
              padding: "3px 4px",
              textAlign: "center",
          ...
          }, row.render(h))), /*#__PURE__*/React.createElement("td", {
            style: {
              padding: "3px 4px",
              textAlign: "center",
              color: totalColor,
              ...
              fontSize: row.isEval ? "9px" : "10px"
            }
          }, totalVal !== null ? totalVal === 0 ? "－" : totalVal : ""));
        }))))));
        ...
        }, "\u76EE\u6A19\u9054\u6210\u5EA6"), achieved && /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: "11px",
            fontWeight: "800",
            color: "#16a34a",
        ...
        }, "\u2705 \u9054\u6210\uFF01")), /*#__PURE__*/React.createElement("div", {
          style: {
            display: "flex",
            alignItems: "center",
            gap: "8px"
        ...
        }, goalDef.label), /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: "14px",
            fontWeight: "800",
            color: achieved ? "#16a34a" : "#0ea5e9"
          }
        }, progress, " / ", r.goal.target, "H")), /*#__PURE__*/React.createElement("div", {
          style: {
            marginTop: "8px",
            height: "5px",
            background: "#e2e8f0",
        ...
        }, pct, "%", !achieved && ` （目標まであと${r.goal.target - progress}H）`));
      })(), r.isComplete && sa && sa.holeCount >= 9 && (r.hcp != null ? /*#__PURE__*/React.createElement(AiDiagnosisPanel, {
        sa: sa,
      ...
      }, "HCP\u672A\u767B\u9332\u306E\u305F\u3081\u5206\u6790\u4E0D\u80FD\u3067\u3059\uFF08\u30DE\u30A4\u30DA\u30FC\u30B8\u3067HCP\u3092\u8A2D\u5B9A\u3057\u3066\u304B\u3089\u8A18\u9332\u3059\u308B\u3068\u5229\u7528\u3067\u304D\u307E\u3059\uFF09"))))));
    })(), !isCollapsed && holeStats.length > 0 && !sa && /*#__PURE__*/React.createElement("div", {
      style: {
        borderTop: "1px solid #e2e8f0",
        paddingTop: "10px",
        overflowX: "auto"
      }
    }, frontStats.length > 0 && /*#__PURE__*/React.createElement(ScoreRow, {
      stats: frontStats,
      label: "\u524D\u534A",
      halfStrk: frontStrk,
      halfPar: frontPar
    }), backStats.length > 0 && /*#__PURE__*/React.createElement(ScoreRow, {
      stats: backStats,
      label: "\u5F8C\u534A",
      halfStrk: backStrk,
      halfPar: backPar
      ...
      }, strk), " (", strk - totalPar >= 0 ? "+" : "", strk - totalPar, ")"), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "\u30D1\u30C3\u30C8 ", /*#__PURE__*/React.createElement("span", {
      ...
      }, totalPutts)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "OB ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: totalOb > 0 ? "#b91c1c" : "#e2e8f0",
          fontWeight: "700"
        }
      }, totalOb)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "\u30DA\u30CA\u30EB\u30C6\u30A3 ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: totalPond > 0 ? "#b91c1c" : "#e2e8f0",
          fontWeight: "700"
        }
      }, totalPond)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569"
        }
      }, "\u30D0\u30F3\u30AB\u30FC ", /*#__PURE__*/React.createElement("span", {
        style: {
          color: totalBunker > 0 ? "#fbbf24" : "#e2e8f0",
          fontWeight: "700"
        }
      }, totalBunker))), /*#__PURE__*/React.createElement("span", {
        style: S.pill(scoreColor(tot))
      }, fmt(tot), "pt"));
    })()), !isCollapsed && /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: "10px",
    ...
    }, "\u524A\u9664")), editingRoundId === r.id && /*#__PURE__*/React.createElement("div", {
      style: {
        position: "fixed",
        inset: 0,
        background: "rgba(0,0,0,0.50)",
    ...
    }, "\u65E5\u4ED8\u30FB\u5929\u6C17\u30FB\u98A8\u3092\u5909\u66F4"), /*#__PURE__*/React.createElement("label", {
      style: S.lbl
    }, "\u65E5\u4ED8"), /*#__PURE__*/React.createElement("input", {
      type: "date",
    ...
    }, /*#__PURE__*/React.createElement("span", null, w.v), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "600",
        opacity: 0.75,
    ...
    }, w.s, /*#__PURE__*/React.createElement("br", null), "m/s")))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "10px"
      }
      ...
      onClick: () => {
        setRounds(prev => prev.map(x => x.id === r.id ? {
          ...x,
          date: editDate,
          weather: editWeather,
          wind: editWind
    ...
    }, "\u4FDD\u5B58")))), confirmDeleteId === r.id && /*#__PURE__*/React.createElement("div", {
      style: {
        position: "fixed",
        inset: 0,
        background: "rgba(0,0,0,0.50)",
    ...
    }, r.course), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "12px",
        color: "#94a3b8",
        marginBottom: "6px"
    ...
    }, "\u767B\u9332\u3055\u308C\u305F\u30B9\u30B3\u30A2\u306F\u5143\u306B\u623B\u305B\u307E\u305B\u3093"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "10px"
      }
  ...
  const AnalyticsAIDiagnosis = () => {
    ...
    if (completed.length === 0) {
      ...
      }, "\u76F4\u8FD120\u30E9\u30A6\u30F3\u30C9 \u5E73\u5747AI\u8A3A\u65AD"), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "11px",
          color: "#64748b"
        }
      }, "HCP\u3092\u8A2D\u5B9A\u3057\u3066\u304B\u3089\u30E9\u30A6\u30F3\u30C9\u3092\u8A18\u9332\u3059\u308B\u3068\u5229\u7528\u3067\u304D\u307E\u3059")));
    }

    // 直近20ラウンド（対象）・直近5ラウンド（比較）
    ...
    const buildAvgSa = rList => {
      const sList = rList.map(r => calcAnalytics(r, r.hcp ?? null, teeRatesAna)).filter(Boolean);
      if (!sList.length) return null;

      // ── ショートゲーム：詳細と簡易で計算式・値域が異なるため混在平均すると歪む。
      //    直近ラウンドのうち登録の多い方のモードのみを使用（同数は詳細優先）。
      const detailList = sList.filter(s => s.isDetailMode);
      const simpleList = sList.filter(s => !s.isDetailMode);
      const shortUseDetail = detailList.length >= simpleList.length; // 同数は詳細優先
    ...
    // 対象ラウンドのHCP：現在の算出値（effectiveHcp）を使用
    const diagHcp = effectiveHcp ?? 0;
    return /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "16px"
      }
    }, /*#__PURE__*/React.createElement(AiDiagnosisPanel, {
      sa: avgSa,
      sa5: avgSa5,
      shd: flatShd,
      hcp: diagHcp,
  ...
  const AnalyticsScoreChart = () => {
    ...
    const recent = simpleCompleted.slice(0, 5).reverse(); // 古い順に並べる
    const teeRatesAna = calcHistoricalTeeRates(rounds);
    const saList = recent.map(r => {
      // 9Hラウンドは18H換算（×2）して推移グラフに表示
      const is9H = (r.holePars?.length ?? 18) <= 12;
      const mult = is9H ? 2 : 1;
      return {
        date: r.date,
        sa: calcAnalytics(r, r.hcp ?? null, teeRatesAna),
        is9H,
    ...
    // 直近20Rの平均スコア・パット数・評価合計（参照用）
    // 平均スコア・パット数は加重平均（9H=0.5回・18H=1.0回）で集計
    ...
    const weightOf = r => (r.holePars?.length ?? 18) <= 12 ? 0.5 : 1.0;
    ...
    const saList20 = sc20all.map(r => ({
      sa: calcAnalytics(r, r.hcp ?? null, teeRatesAna20)
    })).filter(x => x.sa);
    ...
    // カテゴリ別20R平均（評価スコア推移グラフ用）
    const avg20ByKey = saList20.length ? {
      teeScore: Math.round(saList20.reduce((a, x) => a + (x.sa.teeScore ?? 0), 0) / saList20.length * 100) / 100,
      longScore: Math.round(saList20.reduce((a, x) => a + (x.sa.longScore ?? 0), 0) / saList20.length * 100) / 100,
      ...
      puttScore: Math.round(saList20.reduce((a, x) => a + (x.sa.puttScore ?? 0), 0) / saList20.length * 100) / 100,
      bunkerScore: saList20.some(x => x.sa.bunkerScore != null) ? Math.round(saList20.filter(x => x.sa.bunkerScore != null).reduce((a, x) => a + (x.sa.bunkerScore ?? 0), 0) / saList20.filter(x => x.sa.bunkerScore != null).length * 100) / 100 : null
    } : null;
    const LineChart = ({
      title,
      series,
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        fill: "#475569",
        fontSize: "9"
      }, formatY ? formatY(v) : v))), yMin < 0 && yMax > 0 && /*#__PURE__*/React.createElement("line", {
        x1: padL,
        y1: yOf(0).toFixed(1),
        x2: W - padR,
        y2: yOf(0).toFixed(1),
        ...
        return /*#__PURE__*/React.createElement("g", {
          ...
          strokeLinecap: "round"
        }), pts.map(([x, y], i) => /*#__PURE__*/React.createElement("g", {
          key: i
          ...
          fontSize: "9",
          fontWeight: "700"
        }, s.format ? s.format(s.vals[i]) : s.vals[i]))));
      }), saList.map((x, i) => /*#__PURE__*/React.createElement("text", {
        key: i,
        x: xOf(i).toFixed(1),
        y: H - 4,
        textAnchor: "middle",
        fill: "#475569",
        fontSize: "8"
      }, x.date.replace(/\d{4}\//, "").replace(/\/(\d)$/, "/0$1")))), /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          gap: "12px",
          flexWrap: "wrap",
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, (() => {
      // ComboChart: 総打数・パット数・評価合計（棒グラフ右軸）
      const W = 320,
        H = 180,
        padL = 62,
        padR = 28,
      ...
      const latestPar = saList[saList.length - 1]?.totalPar || 72;
      const hcpVal = effectiveHcp ?? 0;
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: S.lbl
      }, "\u30B9\u30B3\u30A2\u30FB\u30D1\u30C3\u30C8\u6570\u30FB\u8A55\u4FA1\u5408\u8A08\u63A8\u79FB\uFF08\u76F4\u8FD15R\uFF09"), /*#__PURE__*/React.createElement("svg", {
        width: "100%",
        viewBox: `0 0 ${W} ${H}`,
        style: {
          display: "block"
        ...
        x: "2",
        y: (yOfL(v) + 4).toFixed(1),
        textAnchor: "start",
        fill: "#475569",
        fontSize: "9"
      }, v))), evalTotals.map((v, i) => {
        const x = xOf(i);
        const y0 = yOfR(0);
        const y1 = yOfR(-v);
        ...
        fill: "#475569",
        fontSize: "8"
      }, x.date.replace(/\d{4}\//, "").replace(/\/(\d)$/, "/0$1"))), avg20Score != null && avg20Score >= combinedMin && avg20Score <= combinedMax && (() => {
      ...
      }, s.label))), (avg20Score != null || avg20Putts != null) && /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          alignItems: "center",
          gap: "4px",
      ...
      }, "20R\u5E73\u5747"))), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "9px",
          color: "#94a3b8",
          marginTop: "4px"
        }
      }, "\u203B\u53F3\u8EF80 = Par", saList[saList.length - 1]?.totalPar, "+HCP", Math.round(hcpVal * 10) / 10, " = ", Math.round(zeroAnchorL)));
    })(), (() => {
      // 積み上げ棒グラフ（評価スコア推移）：＋は0の下、－は0の上
      const W = 320,
        H = 210,
        padL = 62,
        padR = 12,
      ...
      const seriesDef = [{
        ...
        color: "#0ea5e9",
        key: "teeScore",
        vals: saList.map(x => x.sa.teeScore ?? 0)
      }, {
        label: "B ロングゲーム",
        color: "#16a34a",
        key: "longScore",
        vals: saList.map(x => x.sa.longScore ?? 0)
      }, {
        label: "C ショートゲーム",
        color: "#fbbf24",
        key: "shortScore",
        vals: saList.map(x => x.sa.shortScore ?? 0)
      }, {
        label: "D パット",
        color: "#f472b6",
        key: "puttScore",
        vals: saList.map(x => x.sa.puttScore ?? 0)
      }, ...(saList.some(x => x.sa.bunkerScore != null) ? [{
        label: "E バンカー",
        color: "#fb923c",
        key: "bunkerScore",
        vals: saList.map(x => x.sa.bunkerScore ?? 0)
      }] : [])];
      // 20R平均列のデータ
      const avg20Vals = avg20ByKey ? seriesDef.map(s => avg20ByKey[s.key] ?? 0) : null;
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: S.lbl
      }, "\u8A55\u4FA1\u30B9\u30B3\u30A2\u63A8\u79FB\uFF08\u76F4\u8FD15R\uFF09"), /*#__PURE__*/React.createElement("svg", {
        width: "100%",
        viewBox: "0 0 " + W + " " + H,
        style: {
          display: "block"
        }
      }, gridLines.map((g, i) => /*#__PURE__*/React.createElement("g", {
        key: i
      }, /*#__PURE__*/React.createElement("line", {
        x1: padL,
        ...
        return /*#__PURE__*/React.createElement("g", {
          key: i
        }, bars.map((b, bi) => /*#__PURE__*/React.createElement("rect", {
          key: bi,
          x: (x - barW / 2).toFixed(1),
          y: b.y.toFixed(1),
          width: barW,
          ...
          fill: "#475569",
          fontSize: "8"
        }, saList[i].date.replace(/\d{4}\//, "").replace(/\/(\d)$/, "/0$1")));
      }), (() => {
        ...
        return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("path", {
          ...
          strokeWidth: "2.5",
          paintOrder: "stroke"
        }, (evalTotals[i] >= 0 ? "+" : "") + evalTotals[i]))));
      })(), avg20Vals && /*#__PURE__*/React.createElement("line", {
        x1: (sepX + 4).toFixed(1),
        y1: padT,
        x2: (sepX + 4).toFixed(1),
        y2: H - padB,
        ...
        const totalY = yOf(-(avg20Total ?? 0));
        return /*#__PURE__*/React.createElement("g", null, bars.map((b, bi) => /*#__PURE__*/React.createElement("rect", {
          key: bi,
          ...
          strokeWidth: "2.5",
          paintOrder: "stroke"
        }, (avg20Total >= 0 ? "+" : "") + avg20Total), /*#__PURE__*/React.createElement("text", {
          x: avgBarX.toFixed(1),
          y: H - 13,
          textAnchor: "middle",
          fill: "#f59e0b",
          fontSize: "7",
          fontWeight: "700"
        }, "20R"), /*#__PURE__*/React.createElement("text", {
          x: avgBarX.toFixed(1),
          y: H - 4,
          textAnchor: "middle",
          fill: "#f59e0b",
      ...
      }, seriesDef.map(s => /*#__PURE__*/React.createElement("div", {
        key: s.label,
        style: {
          display: "flex",
          alignItems: "center",
      ...
      }, "\u5408\u8A08")), avg20Vals && /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          alignItems: "center",
          gap: "4px",
      ...
      }, "20R\u5E73\u5747\u5408\u8A08"))), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "9px",
          color: "#94a3b8",
          marginTop: "3px"
  ...
  const AnalyticsRadarChart = () => {
    ...
    const buildRadarAvg = rList => {
      ...
      rList.forEach(r => {
        const sa = calcAnalytics(r, r.hcp ?? null, teeRatesAna);
        if (!sa) return;
    ...
    const cx = 175,
      ...
      R = 108;
    // HCPに応じてグリッド刻み幅を変える
    const hcpForGrid = effectiveHcp ?? 99;
    const gridStep = hcpForGrid <= 5 ? 1 : hcpForGrid < 10 ? 2 : hcpForGrid <= 18 ? 3 : 5;
    const MAX = gridStep;
    const MIN = -(gridStep * 3);
    const normalize = v => Math.max(0.0, Math.min(1, (v - MIN) / (MAX - MIN)));
    ...
    const gridPaths = gridLevels.map(lv => toPath(items.map((_, i) => pointOf(i, R * lv))));
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: S.lbl
    }, "\u8A55\u4FA1\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
    ...
    }, gridPaths.map((d, gi) => /*#__PURE__*/React.createElement("path", {
      key: gi,
      d: d,
      fill: "none",
      stroke: "#eef2f7",
      ...
      stroke: "#fbbf24",
      strokeWidth: "2"
    }), pts5.map(([x, y], i) => /*#__PURE__*/React.createElement("circle", {
      key: i,
      cx: x.toFixed(1),
      cy: y.toFixed(1),
      r: "4",
      ...
      return /*#__PURE__*/React.createElement("g", {
        key: i
      }, lines.map((line, li) => /*#__PURE__*/React.createElement("text", {
        key: li,
        x: it.x.toFixed(1),
        y: (it.y - 4 + li * 13).toFixed(1),
        textAnchor: anchor,
        fill: "#94a3b8",
        fontSize: "11",
        fontWeight: "700"
    ...
    }, "\u7406\u60F3\uFF08\xB10\uFF09")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "6px",
    ...
    }, "\u76F4\u8FD15R\u5E73\u5747"), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#475569",
        fontSize: "10px"
      }
    }, "(", recent5.length, "R)")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "6px",
    ...
    }, "\u76F4\u8FD120R\u5E73\u5747"), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#475569",
        fontSize: "10px"
      }
    }, "(", recent20.length, "R)")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "space-between",
        marginBottom: "4px",
    ...
    }, /*#__PURE__*/React.createElement("span", null, "\u8981\u7D20"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "8px"
      }
      ...
      // 矢印は表示列（toFixed(1)）と同一の丸め値から算出し、表の数値と完全一致させる（WYSIWYG）
      // ※ Math.round は負の.5を+∞側に丸めるため列のtoFixed(1)とズレる→parseFloat(toFixed(1))で統一
      ...
      if (!isNaN(diff) && Math.abs(diff) >= 0.05) {
        const improved = diff > 0; // 5要素は高いほど良い
        ...
        }, trendArrow(ac, !improved, 22), /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: "11px",
            fontWeight: "700",
            color: ac,
      ...
      return /*#__PURE__*/React.createElement("div", {
      ...
      }, it.key, " ", it.label), arrow), /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          gap: "8px"
        }
        ...
        style: {
          color: it.val < 0 ? "#dc2626" : "#d97706",
          fontWeight: "700",
          minWidth: "44px",
          textAlign: "right"
        }
      }, it.val >= 0 ? "+" : "", it.val.toFixed(1)), /*#__PURE__*/React.createElement("span", {
        style: {
          color: it.avgVal < 0 ? "#64748b" : "#0ea5e9",
          minWidth: "44px",
          textAlign: "right"
        }
      }, it.avgVal >= 0 ? "+" : "", it.avgVal.toFixed(1)))));
    }))));
  };

  // ── 傾向矢印（スウッシュ）：改善=緑の右肩上がり / 悪化=赤の右肩下がり ──────────
  ...
  const trendArrow = (color, down, size) => /*#__PURE__*/React.createElement("svg", {
    viewBox: down ? TREND_VB_DN : TREND_VB_UP,
    width: size,
    height: Math.round(size * 52.41 / 86.0),
    style: {
    ...
    d: TREND_ARROW_PATH,
    fill: color,
    transform: down ? "translate(0,100) scale(1,-1)" : undefined
  }));
  // 直近5R(n5)と直近20R(n20)の差から傾向バッジを返す。
  // betterLow=低いほど良い指標 / unit=表示単位 / mult=換算係数（OBは1OB=2打→mult:2）
  const renderTrend = (n5, n20, betterLow, unit, mult) => {
    ...
    }, trendArrow(c, !improved, 24), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "12px",
        fontWeight: "700",
        color: c,
  ...
  const AnalyticsStats = () => {
    ...
    const calcStats = list => {
      ...
      // 平均スコアは加重平均（9H=0.5回・18H=1.0回）。率系STATS(GIR%等)はホール単位で正規化済みのため換算不要
      const statsWeight = list.reduce((a, r) => a + ((r.holePars?.length ?? 18) <= 12 ? 0.5 : 1.0), 0);
      ...
      list.forEach(r => {
        ...
        Object.entries(hd).forEach(([hStr, h]) => {
          ...
          holes++;
          pbTot++;
          const isGIR = (h.score || 0) - (h.putts || 0) <= par - 2;
          if (isGIR) gir++;
          if (!isGIR) {
            recTot++;
      ...
      const girRate = holes ? Math.round(gir / holes * 1000) / 10 : 0;
      const recoveryRate = recTot ? Math.round(recOk / recTot * 1000) / 10 : null;
      const parBreakRate = pbTot ? Math.round(pbHoles / pbTot * 1000) / 10 : null;
      let putts = 0,
        puttHoles = 0;
      const roundPutts = [];
      list.forEach(r => {
    ...
    // ラベル / 直近5R値 / 直近20R値 / カラー / 単位
    const statRows = [{
      label: "平均スコア",
      v5: fmt(s5?.avgScore),
      v20: fmt(s20.avgScore),
      color: "#16a34a",
      unit: "",
      n5: s5?.avgScore,
      n20: s20.avgScore,
      ...
      label: "平均パット数（1H）",
      v5: fmt(s5?.avgPuttPerHole),
      v20: fmt(s20.avgPuttPerHole),
      color: "#94a3b8",
      unit: "打",
      n5: s5?.avgPuttPerHole,
      n20: s20.avgPuttPerHole,
      betterLow: true,
      diffUnit: "打"
    }, {
      label: "平均トータルパット数（18H）",
      v5: fmt(s5?.avgTotalPutts),
      v20: fmt(s20.avgTotalPutts),
      color: "#64748b",
      unit: "打",
      n5: s5?.avgTotalPutts,
      n20: s20.avgTotalPutts,
      betterLow: true,
      diffUnit: "打"
    }, {
      label: "パーオン率",
      v5: fmtPct(s5?.girRate),
      v20: fmtPct(s20.girRate),
      color: "#fbbf24",
      unit: "",
      n5: s5?.girRate,
      n20: s20.girRate,
      ...
      label: "リカバリー率",
      v5: fmtPct(s5?.recoveryRate),
      v20: fmtPct(s20.recoveryRate),
      color: "#fb923c",
      unit: "",
      n5: s5?.recoveryRate,
      n20: s20.recoveryRate,
      betterLow: false,
      diffUnit: "pt"
    }, {
      label: "パーブレイク率",
      v5: fmtPct(s5?.parBreakRate),
      v20: fmtPct(s20.parBreakRate),
      color: "#f472b6",
      unit: "",
      n5: s5?.parBreakRate,
      n20: s20.parBreakRate,
      betterLow: false,
      diffUnit: "pt"
    }, {
      label: "OB数（18H）",
      ...
      label: "ペナルティ数（18H）",
      v5: fmt(s5?.avgPenalty),
      v20: fmt(s20.avgPenalty),
      color: "#b91c1c",
      unit: "回",
      n5: s5?.avgPenalty,
      n20: s20.avgPenalty,
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: S.lbl
    }, "Stats"), hcpVal != null && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
    ...
    }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\uFF08\u53C2\u8003\u5024\uFF09"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "17px",
        fontWeight: "800",
        color: "#0ea5e9"
      }
    }, hcpVal)), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "flex-end",
        gap: "0",
    ...
    }, "\u76F4\u8FD120R")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        gap: "0px"
      }
    }, statRows.map((row, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      style: {
        display: "flex",
        justifyContent: "space-between",
    ...
    }, row.label), renderTrend(row.n5, row.n20, row.betterLow, row.diffUnit, row.mult)), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "15px",
        fontWeight: "700",
        color: "#f59e0b",
    ...
    }, row.v5, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "500",
        marginLeft: "1px",
    ...
    }, row.unit)), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "15px",
        fontWeight: "800",
        color: row.color,
    ...
    }, row.v20, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "500",
        marginLeft: "1px"
      }
    }, row.unit))))), (ddAvg5 != null || ddAvg20 != null) && /*#__PURE__*/React.createElement("div", {
      style: {
        borderTop: "1px solid #e2e8f0",
        paddingTop: "10px",
        marginTop: "10px"
    ...
    }, /*#__PURE__*/React.createElement("span", null, "\u5E73\u5747\u30C9\u30E9\u30A4\u30D3\u30F3\u30B0\u30C7\u30A3\u30B9\u30BF\u30F3\u30B9"), renderTrend(ddAvg5, ddAvg20, false, "Y", 1)), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px"
      }
    }, "\uFF08\u63A8\u5B9A\uFF0F1W\u3007\u306E\u307F\uFF09")), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "15px",
        fontWeight: "700",
        color: "#f59e0b",
    ...
    }, ddAvg5 != null ? ddAvg5 : "－", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "500",
        marginLeft: "1px",
    ...
    }, ddAvg20 != null ? ddAvg20 : "－", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "500",
        marginLeft: "2px",
    ...
    }, trendArrow("#16a34a", false, 20), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#16a34a",
        fontWeight: "700",
        marginLeft: "2px"
      }
    }, "\u6539\u5584")), /*#__PURE__*/React.createElement("span", {
      style: {
        display: "inline-flex",
        alignItems: "center",
        whiteSpace: "nowrap"
      }
    }, trendArrow("#b91c1c", true, 20), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#b91c1c",
        fontWeight: "700",
        marginLeft: "2px"
      }
    }, "\u60AA\u5316")), /*#__PURE__*/React.createElement("span", {
      style: {
        whiteSpace: "nowrap"
      }
    }, /*#__PURE__*/React.createElement("span", {
    ...
    }, "\u2014"), " \u6A2A\u3070\u3044")), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "9px",
        color: "#94a3b8",
        marginTop: "5px",
    ...
    }, "\u203B\u76F4\u8FD15R \u3068 \u76F4\u8FD120R \u306E\u5DEE\u3002OB\u6570\u306F\u300C1OB\uFF1D2\u6253\u300D\u63DB\u7B97\uFF08%\u7CFB\u306F\u30DD\u30A4\u30F3\u30C8\u5DEE\uFF09\u3002")), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "9px",
        color: "#94a3b8",
        marginTop: "8px",
    ...
    }, "\u203B\u30D1\u30FC\u30AA\u30F3\uFF1A\u30B9\u30B3\u30A2\uFF0D\u30D1\u30C3\u30C8\u6570 \u2264 Par\uFF0D2\u306E\u30DB\u30FC\u30EB", /*#__PURE__*/React.createElement("br", null), "\u203B\u30EA\u30AB\u30D0\u30EA\u30FC\u7387\uFF1A\u30D1\u30FC\u30AA\u30F3\u5931\u6557\u30DB\u30FC\u30EB\u3067Par\u4EE5\u4E0B\u3092\u53D6\u3063\u305F\u5272\u5408", /*#__PURE__*/React.createElement("br", null), "\u203B\u30D1\u30FC\u30D6\u30EC\u30A4\u30AF\u7387\uFF1A\u30B9\u30B3\u30A2\u304CPar\u4EE5\u4E0B\u306E\u30DB\u30FC\u30EB\u306E\u5272\u5408", /*#__PURE__*/React.createElement("br", null), "\u203B\u5E73\u5747\u30C9\u30E9\u30A4\u30D3\u30F3\u30B0\u30C7\u30A3\u30B9\u30BF\u30F3\u30B9\uFF1APar4/5\u306E\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u30671W\u4F7F\u7528\u25CB\u306E\u307F\uFF0F\u30DB\u30FC\u30EB\u8DDD\u96E2\uFF0D\u6B8B\u308A\u8DDD\u96E2"));
  };

  // ── AnalyticsPuttStats ─────────────────────────────────────────────────────
  const AnalyticsPuttStats = () => {
    // pinDist が記録されているホールを detail rounds から収集
    const distStats = {};
    ...
    const rows = PIN_DIST_OPTIONS.map(o => ({
      label: o.label,
      ...distStats[o.label]
    })).filter(r => r.total > 0);
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: S.lbl
    }, "\u8DDD\u96E2\u5225\u30D1\u30C3\u30C8\u7D71\u8A08\uFF08\u8A73\u7D30\u30E2\u30FC\u30C9\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        overflowX: "auto"
      }
    ...
    }, ["距離", "回数", "1パット率", "3パット率", "平均"].map(h => /*#__PURE__*/React.createElement("th", {
      key: h,
      style: {
        padding: "6px 4px",
        textAlign: "center",
      ...
      }, onePct, "%"), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "8px 4px",
          textAlign: "center",
          fontWeight: "700",
      ...
      }, thrPct, "%"), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "8px 4px",
          textAlign: "center",
          color: "#1e293b",
    ...
    }, "\u203B\u8A73\u7D30\u30E2\u30FC\u30C9\u3067\u8DDD\u96E2\u3092\u8A18\u9332\u3057\u305F\u30DB\u30FC\u30EB\u306E\u307F\u96C6\u8A08"));
  };

  // ── AnalyticsDistanceStats ─────────────────────────────────────────────────────
  const AnalyticsDistanceStats = () => {
    const allShots = rounds.filter(r => r.isComplete && r.inputMode === "detail" && r.holeData).flatMap(r => Object.values(r.holeData).flatMap(h => h.shots || [])).filter(s => s.quality !== undefined && s.categoryKey !== "putt");
    ...
    allShots.forEach(s => {
      ...
      if (s.quality === "×") distMap[d].bad += 1;
    });
    const rows = REMAIN_DIST_OPTIONS.map(o => ({
      label: o.label,
      ...distMap[o.label]
    })).filter(r => r.total > 0);
    if (rows.length === 0) return null;
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: S.lbl
    }, "\u6B8B\u308A\u8DDD\u96E2\u5225 \u25CB/\xD7\u7387\uFF08\u8A73\u7D30\u30E2\u30FC\u30C9\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        overflowX: "auto"
      }
    }, /*#__PURE__*/React.createElement("table", {
    ...
    }, ["距離", "回数", "○率", "×率"].map(h => /*#__PURE__*/React.createElement("th", {
      key: h,
      style: {
        padding: "6px 4px",
        textAlign: "center",
      ...
      }, gPct, "%"), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "8px 4px",
          textAlign: "center",
          fontWeight: "800",
  ...
  const AnalyticsClubStats = () => {
    const allShots = rounds.filter(r => r.isComplete && r.inputMode === "detail" && r.holeData).flatMap(r => Object.values(r.holeData).flatMap(h => h.shots || [])).filter(s => s.quality !== undefined && s.categoryKey !== "putt" && s.club);
    if (allShots.length === 0) return null;
    const clubMap = {};
    allShots.forEach(s => {
      const cid = normalizeClubId(s.club);
      if (!clubMap[cid]) clubMap[cid] = {
        total: 0,
        good: 0,
        bad: 0
      };
      ...
      if (s.quality === "×") clubMap[cid].bad += 1;
    });

    // クラブマスターの順番でソート
    const clubOrder = CLUB_MASTER.map(c => c.id);
    const rows = Object.entries(clubMap).map(([club, v]) => ({
      club,
      ...v
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: S.lbl
    }, "\u30AF\u30E9\u30D6\u5225 \u25CB/\xD7\u7387\uFF08\u8A73\u7D30\u30E2\u30FC\u30C9\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        overflowX: "auto"
      }
    }, /*#__PURE__*/React.createElement("table", {
    ...
    }, ["クラブ", "回数", "○率", "×率"].map(h => /*#__PURE__*/React.createElement("th", {
      key: h,
      style: {
        padding: "6px 4px",
        textAlign: "center",
      ...
      }, clubLabel(r.club)), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "8px 4px",
          textAlign: "center",
          color: "#64748b"
  ...
  const AnalyticsSegmentCharts = () => {
    ...
    // 区間ごとの傾向矢印：20R棒の頂点(x1,y1)から5R棒の頂点(x2,y2)へ伸びる細い矢印
    // 棒グラフから少し離すため全体を上方向に LIFT 分だけ浮かせる
    const ARROW_LIFT = 6;
    const SegTrendArrow = ({
      x1,
      y1,
      ...
      const ly1 = y1 - ARROW_LIFT,
        ly2 = y2 - ARROW_LIFT;
      const ang = Math.atan2(ly2 - ly1, x2 - x1);
      const head = 5; // 矢じりの長さ
      const spread = 0.5; // 矢じりの開き
      ...
      return /*#__PURE__*/React.createElement("g", null, /*#__PURE__*/React.createElement("line", {
        ...
        d: `M${x2},${ly2} L${hx1.toFixed(1)},${hy1.toFixed(1)} L${hx2.toFixed(1)},${hy2.toFixed(1)} Z`,
        fill: color
      }));
    };
    const SegBar = ({
      ...
      return /*#__PURE__*/React.createElement("div", {
      ...
      }, m.title), /*#__PURE__*/React.createElement("svg", {
        viewBox: `0 0 ${W} ${H}`,
        style: {
          width: "100%",
          height: "auto",
        ...
        return /*#__PURE__*/React.createElement("g", {
          ...
          fontSize: "8",
          fill: "#94a3b8"
        }, m.decimals === 1 ? val.toFixed(1) : Math.round(val)));
      }), zeroPx != null && /*#__PURE__*/React.createElement("line", {
        x1: padL,
        y1: zeroPx,
        x2: W - padR,
        y2: zeroPx,
        ...
        // 棒を少し離す：2本表示時はバー間に隙間を設け、その分やや細く
        const gap = show20 ? groupW * 0.10 : 0;
        ...
        const bars = [];
        let arrowEl = null;
        // 5Rのみ（show20でない）時は中央に1本
        const x5 = show20 ? startX + bw + gap : startX; // 直近5R（右・アンバー）
        const x20 = startX; // 直近20R（左・ブルー）
        // 直近5R（右・アンバー / 単独表示時は左端）
        if (v5) {
          const y5 = yToPx(v5[i]);
        ...
        if (show20) {
          ...
          bars.push(/*#__PURE__*/React.createElement("rect", {
            ...
            y: Math.min(y20, base),
            width: bw,
            height: Math.abs(base - y20),
            fill: C_BLUE,
            rx: "1.5"
          }));
          // 20R頂点 → 5R頂点 の傾向矢印（show20かつ5Rありのときのみ）
          if (v5) {
            const diff = v5[i] - v20[i];
            if (Math.abs(diff) >= (m.decimals === 0 ? 0.5 : 0.05)) {
              const improved = m.betterLow ? diff < 0 : diff > 0;
              const ac = improved ? "#16a34a" : "#b91c1c";
              const x20c = x20 + bw / 2; // 20R棒の中央（左）
              const x5c = x5 + bw / 2; // 5R棒の中央（右）
              const fromY = yToPx(v20[i]);
              const toY = yToPx(v5[i]);
              arrowEl = /*#__PURE__*/React.createElement(SegTrendArrow, {
                x1: x20c,
                y1: fromY,
                x2: x5c,
                y2: toY,
        ...
        return /*#__PURE__*/React.createElement("g", {
          key: i
        }, bars, arrowEl, /*#__PURE__*/React.createElement("text", {
          x: gx + groupW / 2,
          y: H - 8,
          textAnchor: "middle",
          fontSize: "8",
      ...
      }), show20 ? /*#__PURE__*/React.createElement("line", {
        x1: padL,
        y1: avg20Px,
        x2: W - padR,
        y2: avg20Px,
        ...
        strokeDasharray: "5 4",
        opacity: "0.8"
      }) : avg5Px != null && /*#__PURE__*/React.createElement("line", {
        x1: padL,
        y1: avg5Px,
        x2: W - padR,
        y2: avg5Px,
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "3\u30DB\u30FC\u30EB\u533A\u9593\u5206\u6790"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "11px",
        color: "#64748b",
        marginBottom: "10px"
      }
    }, "\u533A\u9593\u3054\u3068\u306B\u5F31\u70B9\u3092\u53EF\u8996\u5316\uFF0818H\u30E9\u30A6\u30F3\u30C9\u306E\u307F\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexWrap: "wrap",
        gap: "12px",
    ...
    }), "\u76F4\u8FD1", seg5 ? seg5.count : 0, "R"), show20 && /*#__PURE__*/React.createElement("span", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "5px"
    ...
    }), "\u76F4\u8FD120R"), show20 ? /*#__PURE__*/React.createElement("span", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "5px"
      ...
      style: {
        width: "16px",
        height: 0,
        borderTop: `2px dashed ${C_BLUE}`
      }
    }), "20R\u533A\u9593\u5E73\u5747") : /*#__PURE__*/React.createElement("span", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "5px"
      ...
      style: {
        width: "16px",
        height: 0,
        borderTop: `2px dashed ${C_AMBER}`
      }
    }), "\u533A\u9593\u5E73\u5747")), show20 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexWrap: "wrap",
        gap: "12px",
    ...
    }, "\u60AA\u5316")), /*#__PURE__*/React.createElement("span", null, "\u77E2\u5370\u306F 20R \u2192 \u76F4\u8FD15R \u306E\u5909\u5316")), metrics.map(m => /*#__PURE__*/React.createElement(SegBar, {
      key: m.key,
      m: m
    }))));
  };
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    alt: "icon"
  })), detailUnlocked ? /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "9px",
      fontWeight: "800",
      color: "#16a34a",
  ...
  }, "\u7121\u6599\u7248")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "3px"
    }
  }, currentRound && /*#__PURE__*/React.createElement("button", {
    style: S.navBtn(view === "round"),
    onClick: () => setView("round")
  }, "\u8A18\u9332\u4E2D"), /*#__PURE__*/React.createElement("button", {
    style: S.navBtn(view === "analytics"),
    onClick: () => setView("analytics")
  }, "\u5206\u6790"), /*#__PURE__*/React.createElement("button", {
    style: S.navBtn(view === "clubs"),
    onClick: () => setView("clubs")
  }, "\u30AF\u30E9\u30D6"), /*#__PURE__*/React.createElement("button", {
    style: S.navBtn(view === "manual"),
    ...
    }, "\uD83D\uDC64"), hcpUnset && /*#__PURE__*/React.createElement("span", {
      style: {
        position: "absolute",
        top: "-1px",
        right: "-1px",
    ...
    }), hcpUnset && showHcpHint && /*#__PURE__*/React.createElement("div", {
      className: "hcp-hint",
      onClick: () => {
        setShowHcpHint(false);
        setView("profile");
    ...
    style: S.page
  }, view === "home" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: "20px"
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "ScoRExolution\uFF08\u30B9\u30B3\u30EC\u30DC\uFF09"), detailUnlocked ? /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "10px",
      fontWeight: "800",
      color: "#16a34a",
  ...
  }, "PRO \u89E3\u9664\u6E08\u307F") : /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "10px",
      fontWeight: "800",
      color: "#0ea5e9",
    ...
    costume: isNightNow() ? "night" : "basic7",
    size: 64
  }, profile.nickname ? /*#__PURE__*/React.createElement(React.Fragment, null, "\u3053\u3093\u306B\u3061\u306F\u3001", profile.nickname, "\u3055\u3093\uFF01", /*#__PURE__*/React.createElement("br", null)) : null, "\u4ECA\u65E5\u3082\u4E00\u7DD2\u306B\u697D\u3057\u3082\u3046\uFF01")) : profile.nickname ? /*#__PURE__*/React.createElement("p", {
    style: {
      color: "#16a34a",
      fontSize: "13px",
      fontWeight: "600",
    ...
    // ベストスコア：全完了ラウンドの最小値 vs プロフィール自己申告 のうち小さい方
    ...
    const bestFromProfile = profile.bestScore ?? null;
    const best = bestFromData != null && bestFromProfile != null ? Math.min(bestFromData, bestFromProfile) : bestFromData ?? bestFromProfile;
    ...
    const items = [{
      label: "ハンディキャップ（参考値）",
      value: (handicap.hcp >= 0 ? "+" : "") + handicap.hcp,
      color: "#0ea5e9"
    }, {
      label: "ベストスコア",
      value: best,
  ...
  }, "\u203B\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\uFF1A\u5DEE\u5206\u5E73\u5747\u65B9\u5F0F\u306B\u3088\u308B\u53C2\u8003\u5024\u3002\u516C\u5F0F\u3068\u306F\u7570\u306A\u308A\u307E\u3059", /*#__PURE__*/React.createElement("br", null), "\u203B\u30D9\u30B9\u30C8\u30B9\u30B3\u30A2\uFF1A\u5168\u8A18\u9332\u30E9\u30A6\u30F3\u30C9\u3068\u30DE\u30A4\u30DA\u30FC\u30B8\u81EA\u5DF1\u7533\u544A\u306E\u3046\u3061\u5C0F\u3055\u3044\u65B9", /*#__PURE__*/React.createElement("br", null), "\u203B\u5E73\u5747\u30FB\u4E2D\u592E\u5024\uFF1A\u76F4\u8FD120\u30E9\u30A6\u30F3\u30C9\u304B\u3089\u7B97\u51FA")) : rounds.filter(r => r.isComplete).length > 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      ...S.card({
        marginBottom: "18px",
  ...
  }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\u7B97\u51FA\u306B\u306F18H\u5B8C\u4E86\u30E9\u30A6\u30F3\u30C9\u304C3\u4EF6\u4EE5\u4E0A\u5FC5\u8981\u3067\u3059\uFF08\u73FE\u5728 ", rounds.filter(r => r.isComplete).length, "\u4EF6\uFF09") : null, !showNewRound ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: "10px",
  ...
  }, "\uFF0B \u65B0\u3057\u3044\u30E9\u30A6\u30F3\u30C9\u3092\u958B\u59CB"), false && importedTestData.length > 0 && importedTestData.some(r => !rounds.some(x => x.rid === r.rid)) && /*#__PURE__*/React.createElement("button", {
    style: {
      ...S.btn("secondary"),
      width: "100%",
      padding: "10px",
    ...
    onClick: () => {
      setRounds(prev => {
        ...
        const newRounds = importedTestData.filter(r => !existingIds.has(r.rid)).map(r => {
          // 詳細モードで simpleHoleData が空の場合は後付け変換
          if (r.inputMode === "detail" && Object.keys(r.simpleHoleData || {}).length === 0 && r.holeData) {
            return {
              ...r,
              simpleHoleData: deriveSimpleHoleData(r.holeData, r.holePars)
            };
          }
          return r;
        });
  ...
  }, "\u30C6\u30B9\u30C8\u30C7\u30FC\u30BF\u3092\u8AAD\u307F\u8FBC\u3080\uFF08", importedTestData.length, "\u30E9\u30A6\u30F3\u30C9\uFF09"), false && rounds.length > 0 && /*#__PURE__*/React.createElement("button", {
    style: {
      ...S.btn("secondary"),
      width: "100%",
      padding: "10px",
    ...
    onClick: async () => {
      try {
        await window.storage.set("golf_test_data", JSON.stringify(rounds));
  ...
  }, "\u73FE\u5728\u306E", rounds.length, "\u4EF6\u3092\u30C6\u30B9\u30C8\u30C7\u30FC\u30BF\u3068\u3057\u3066\u4FDD\u5B58")) : RoundSetupForm(), rounds.length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: S.card({
      padding: "24px 18px"
    })
  }, REXY_IMAGES.basic10 ? /*#__PURE__*/React.createElement(RexyBubble, {
    costume: "basic10",
    size: 72
  }, "\u4E00\u7DD2\u306B\u30E9\u30A6\u30F3\u30C9\u3092\u958B\u59CB\u3057\u3088\u3046\uFF01") : /*#__PURE__*/React.createElement("p", {
    style: {
      fontWeight: "700",
      color: "#94a3b8",
      textAlign: "center"
    ...
    return [...rounds].sort((a, b) => {
      return dateToNum(b.date) - dateToNum(a.date);
    }).map(r => /*#__PURE__*/React.createElement(RoundCardErrorBoundary, {
      key: r.id
    }, /*#__PURE__*/React.createElement(RoundCard, {
      r: r,
      teeRates: teeRates
    })));
  })()), view === "round" && currentRound && inputMode === "simple" && (() => {
    const shd = simpleHoleData[currentHole] || {};
    const setShd = patch => setSimpleHoleData(prev => ({
      ...prev,
      [currentHole]: {
        ...(prev[currentHole] || {}),
        ...patch
    ...
    // ホール移動：現在ホールを保存してから指定ホールへ移動
    const moveToHole = nextHole => {
      setSimpleHoleData(prev => {
        const h = prev[currentHole] || {};
        const needsSave = h.score == null || h.putts === undefined;
        ...
        return {
          ...
          [currentHole]: {
            ...
            putts: h.putts === undefined ? 0 : h.putts
          }
        };
      });
    ...
    const simpleRecKeys = Object.keys(simpleHoleData).filter(k => simpleHoleData[k]?.score != null);
    ...
    const EvalBtn = ({
      ...
      key: l.v,
      onClick: () => onChange(current === l.v ? null : l.v),
      style: {
        width: "36px",
        height: "36px",
        borderRadius: "8px",
    ...
    const NumInput = ({
      ...
      onClick: () => onChange(Math.max(min, (val || 0) - 1)),
      style: {
        boxSizing: "border-box",
        width: "32px",
        height: "32px",
    ...
    }, val || 0), /*#__PURE__*/React.createElement("button", {
      onClick: () => onChange(Math.min(max, (val || 0) + 1)),
      style: {
        boxSizing: "border-box",
        width: "32px",
        height: "32px",
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, (currentRound.course || "").replace(/（[^）]*）\s*$/, "") || currentRound.course), /*#__PURE__*/React.createElement("p", {
      style: {
        color: "#94a3b8",
        fontSize: "11px",
        marginTop: "1px"
      }
    }, simpleRecCount, "\u30DB\u30FC\u30EB\u8A18\u9332\u6E08\u307F\u3000\u5408\u8A08 ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#1e293b",
        fontWeight: "700"
      }
    }, simpleTotalScore), "\u6253", simpleRecCount > 0 && /*#__PURE__*/React.createElement("span", {
      style: {
        color: scoreDiffSymbol(simpleRecDiff).color
      }
    }, "\uFF08", simpleRecDiff >= 0 ? "+" : "", simpleRecDiff, "\uFF09"))), /*#__PURE__*/React.createElement("button", {
      style: S.btn("danger"),
      onClick: () => {
        // 現在ホールを先に保存してから確認ダイアログ
        setSimpleHoleData(prev => {
          const h = prev[currentHole] || {};
          const needsSave = h.score == null || h.putts === undefined;
          if (!needsSave) return prev;
        ...
        setTimeout(() => {
          setSimpleHoleData(prev => {
            ...
            const incomplete = allHoles.filter(h => {
              const hd = prev[h];
              if (!hd || hd.score == null) return true;
              if (!hd.teeEval) return true;
              if (!hd.approachEval) return true;
              return false;
            });
            if (incomplete.length > 0) {
              setFinishConfirm({
      ...
      // 内部キーは1〜18、表示番号はマスタのhole値
      const frontLabel = currentRound?.frontCourse || "前半";
      const backLabel = currentRound?.backCourse || "後半";
      const frontNums = currentRound?.frontHoleNums || [1, 2, 3, 4, 5, 6, 7, 8, 9];
      const backNums = currentRound?.backHoleNums || [10, 11, 12, 13, 14, 15, 16, 17, 18];
      return [{
        label: frontLabel,
        ...
        label: backLabel,
        keys: [10, 11, 12, 13, 14, 15, 16, 17, 18],
        nums: backNums
      }].map((row, ri) => {
        // 入力済みホールの対パー小計
        const halfDiff = row.keys.reduce((a, h) => {
          const hd = simpleHoleData[h];
          if (hd?.score == null) return a;
          return a + (hd.score - (holePars[h - 1] || 4));
        }, 0);
        const halfDoneCount = row.keys.filter(h => simpleHoleData[h]?.score != null).length;
        return /*#__PURE__*/React.createElement("div", {
          key: ri,
          style: {
            marginBottom: "10px"
        ...
        }, row.label), halfDoneCount > 0 && /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: "13px",
            fontWeight: "900",
            color: halfDiff < 0 ? "#ef4444" : halfDiff === 0 ? "#16a34a" : "#f97316"
          }
        }, halfDiff > 0 ? "+" : "", halfDiff)), /*#__PURE__*/React.createElement("div", {
          style: {
            display: "flex",
            justifyContent: "space-between"
          }
          ...
          const done = hh?.score != null;
          const diff = done ? hh.score - (holePars[h - 1] || 4) : null;
          const {
            sym,
            color: symColor
          } = diff !== null ? scoreDiffSymbol(diff) : {
            sym: "",
            color: "#94a3b8"
          };
          const dispNum = row.nums[i] ?? h;
          return /*#__PURE__*/React.createElement("button", {
            key: h,
            ...
            style: {
              fontWeight: "700",
              fontSize: "11px",
              color: active ? "#16a34a" : done ? symColor : "#334155",
              lineHeight: 1
            }
          }, dispNum), done && /*#__PURE__*/React.createElement("span", {
            style: {
              fontSize: "11px",
              fontWeight: "800",
              color: symColor,
      ...
      const teeLabel = venue?.tees.find(t => t.id === currentRound.tee)?.label || "";
      const greenLabel = venue?.greens.find(g => g.id === currentRound.green)?.label || "";
      return /*#__PURE__*/React.createElement("div", {
        style: {
          marginBottom: "14px"
        }
      ...
      }, getDisplayHoleNum(currentHole), "\u756A"), /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          gap: "5px"
        }
      }, [3, 4, 5].map(p => /*#__PURE__*/React.createElement("button", {
        key: p,
      ...
      }, "Par", p))), hInfo && /*#__PURE__*/React.createElement("span", {
        style: {
          color: "#475569",
          fontWeight: "500",
          fontSize: "10px"
        }
      }, "HC", hInfo.hdcp)), yard && /*#__PURE__*/React.createElement("div", {
        style: {
          marginTop: "5px",
          fontSize: "10px"
        }
      ...
      }, [greenLabel, teeLabel].filter(Boolean).join("　"), " ", yard, "y")));
    })(), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
        gridTemplateColumns: "1fr 1fr",
      ...
      style: S.lbl
    }, "\u30B9\u30B3\u30A2\uFF08\u6253\u6570\uFF09"), /*#__PURE__*/React.createElement(NumInput, {
      val: shd.score ?? par,
      onChange: v => {
        const maxPutts = Math.max(0, v - 1);
        const currentPutts = shd.putts ?? 0;
        setShd({
          score: v,
          ...(currentPutts > maxPutts ? {
            putts: maxPutts
          } : {})
        });
      },
      ...
      style: S.lbl
    }, "\u30D1\u30C3\u30C8\u6570"), /*#__PURE__*/React.createElement(NumInput, {
      val: shd.putts,
      onChange: v => setShd({
        putts: v
      }),
      min: 0,
      max: Math.max(0, (shd.score ?? par) - 1)
    }))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
      ...
      style: S.lbl
    }, label), /*#__PURE__*/React.createElement(NumInput, {
      val: shd[key],
      onChange: v => setShd({
        [key]: v
      }),
      ...
      style: S.lbl
    }, "\u30B7\u30E7\u30C3\u30C8\u8A55\u4FA1"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        gap: "10px"
      ...
      style: {
        ...
        width: "104px",
        whiteSpace: "nowrap",
        color: shd.teeEval ? "#94a3b8" : "#dc2626"
      }
    }, "\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8 ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "9px"
      }
    }, "*")), /*#__PURE__*/React.createElement(EvalBtn, {
      val: shd.teeEval,
      current: shd.teeEval,
      onChange: v => setShd({
        teeEval: v
      ...
      labels: OX
    })), par >= 4 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center"
    ...
    }, "\u30BB\u30AB\u30F3\u30C9\u30B7\u30E7\u30C3\u30C8"), /*#__PURE__*/React.createElement(EvalBtn, {
      val: shd.secondEval,
      current: shd.secondEval,
      onChange: v => setShd({
        secondEval: v
      }),
      labels: OX
    })), par >= 5 && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center"
    ...
    }, "\u30B5\u30FC\u30C9\u30B7\u30E7\u30C3\u30C8"), /*#__PURE__*/React.createElement(EvalBtn, {
      val: shd.thirdEval,
      current: shd.thirdEval,
      onChange: v => setShd({
        thirdEval: v
      }),
      labels: OX
      ...
      style: {
        ...
        width: "104px",
        whiteSpace: "nowrap",
        color: shd.approachEval ? "#94a3b8" : "#dc2626"
      }
    }, "\u30A2\u30D7\u30ED\u30FC\u30C1 ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "9px"
      }
    }, "*")), /*#__PURE__*/React.createElement(EvalBtn, {
      val: shd.approachEval,
      current: shd.approachEval,
      onChange: v => setShd({
        approachEval: v
      }),
      labels: OXN
      ...
      // 後半の最終ホールを判定：frontがIN系なら後半はOUT(1〜9)なので9番が最終、それ以外は18番が最終
      // 内部は常に前半=1〜9、後半=10〜18
      const firstHole = 1;
      const lastHole = 18;
      ...
      return /*#__PURE__*/React.createElement("div", {
      ...
      }, currentHole !== firstHole && /*#__PURE__*/React.createElement("button", {
        style: {
          ...S.btn("secondary"),
          flex: 1
        },
        onClick: () => {
          // 後半先頭→前半最終への折り返し（純粋移動・自動記録なし）
      ...
      }, "\u2039 \u524D\u306E\u30DB\u30FC\u30EB"), currentHole !== lastHole ? /*#__PURE__*/React.createElement("button", {
        style: {
          ...S.btn("primary"),
          flex: 1
        },
        onClick: () => {
          ...
          // スコア種別トースト
          const sc = shd.score ?? par;
          scoreToast(sc, par);
          moveToHole(nextHole);
        }
      }, "\u6B21\u306E\u30DB\u30FC\u30EB \u203A") : /*#__PURE__*/React.createElement("button", {
        style: {
          ...S.btn("danger"),
          flex: 1
        },
        onClick: () => {
          ...
          setTimeout(() => {
            setSimpleHoleData(prev => {
              ...
              const incomplete = allHoles.filter(h => {
                const hd = prev[h];
                if (!hd || hd.score == null) return true; // スコア未入力
                if (!hd.teeEval) return true; // ティ評価未入力
                if (!hd.approachEval) return true; // アプローチ評価未入力
                return false;
              });
              if (incomplete.length > 0) {
                setFinishConfirm({
      ...
      "aria-hidden": "true"
    }), skipConfirm && /*#__PURE__*/React.createElement("div", {
      style: {
        position: "fixed",
        inset: 0,
        background: "rgba(0,0,0,0.50)",
    ...
    }, "\u30B7\u30E7\u30C3\u30C8\u8A55\u4FA1\u304C\u672A\u5165\u529B\u3067\u3059"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "12px",
        color: "#64748b",
        lineHeight: 1.6,
    ...
    }, "\u3053\u306E\u30DB\u30FC\u30EB\u306E\u30C6\u30A3\uFF0F\u30A2\u30D7\u30ED\u30FC\u30C1\u8A55\u4FA1\u304C\u672A\u5165\u529B\u3067\u3059\u3002\u5165\u529B\u305B\u305A\u306B\u6B21\u306E\u30DB\u30FC\u30EB\u3078\u9032\u307F\u307E\u3059\u304B\uFF1F"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "10px"
      }
  ...
  })(), view === "round" && currentRound && inputMode === "detail" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      justifyContent: "space-between",
      alignItems: "center",
    ...
    const doneEntries = Object.entries(holeData).filter(([, hd]) => hd.done);
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, doneCount, "\u30DB\u30FC\u30EB\u8A18\u9332\u6E08\u307F\u3000\u5408\u8A08 ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#1e293b",
        fontWeight: "700"
      }
    }, totalStrk), "\u6253", doneCount > 0 && totalParDone > 0 && /*#__PURE__*/React.createElement("span", {
      style: {
        color: diff < 0 ? "#ef4444" : diff === 0 ? "#16a34a" : "#f97316",
        fontWeight: "700"
      }
    }, "\uFF08", diff > 0 ? "+" : "", diff, "\uFF09"));
  })()))), /*#__PURE__*/React.createElement("div", {
  ...
  })(), currentRound.venueId && /*#__PURE__*/React.createElement("button", {
    style: S.btn("secondary"),
    onClick: () => setShowYardage(v => !v)
  }, "\u8DDD\u96E2"), /*#__PURE__*/React.createElement("button", {
    style: S.btn("danger"),
    onClick: handleDetailFinishClick
  }, "\u7D42\u4E86"))), (() => {
    const venue = VENUES.find(v => v.id === currentRound.venueId);
    if (!venue) return null;
    const greenLabel = venue.greens.find(g => g.id === currentRound.green)?.label || currentRound.green;
    const teeLabel = venue.tees.find(t => t.id === currentRound.tee)?.label || currentRound.tee;
    return /*#__PURE__*/React.createElement("div", {
      style: S.card({
        padding: "10px 14px",
        marginBottom: "10px"
    ...
    }, "\u524D\u534A: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#16a34a",
        fontWeight: "700"
      }
    }, currentRound.frontCourse)), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#64748b"
      }
    }, "\u5F8C\u534A: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#0ea5e9",
        fontWeight: "700"
      }
    }, currentRound.backCourse)), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#64748b"
      }
    }, "\u30B0\u30EA\u30FC\u30F3: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#d97706",
        fontWeight: "700"
      }
    }, greenLabel)), /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#64748b"
      }
    }, "\u30C6\u30A3\u30FC: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#1e293b",
        fontWeight: "700"
      }
    ...
    const teeLabel = venue.tees.find(t => t.id === tee)?.label || tee;
    return /*#__PURE__*/React.createElement("div", {
      style: S.card({
        padding: "12px 10px",
        marginBottom: "10px"
    ...
    }, "\u30E4\u30FC\u30C7\u30FC\u30B8\u8868\uFF08", teeLabel, "\uFF09"), /*#__PURE__*/React.createElement("button", {
      style: S.btn("ghost"),
      onClick: () => setShowYardage(false)
    }, "\u2715")), /*#__PURE__*/React.createElement("table", {
      style: {
        width: "100%",
        borderCollapse: "collapse",
    ...
    }, ["H", "Par", "HDCP", "距離(Y)"].map(h => /*#__PURE__*/React.createElement("th", {
      key: h,
      style: {
        padding: "5px 4px",
        textAlign: "center",
    ...
    }, h)))), /*#__PURE__*/React.createElement("tbody", null, allHoles.map((h, i) => {
      ...
      return /*#__PURE__*/React.createElement("tr", {
        ...
        style: {
          background: isActive ? "rgba(22,163,74,0.10)" : i % 2 === 0 ? "rgba(255,255,255,0.015)" : "transparent",
          cursor: "pointer"
        }
      }, /*#__PURE__*/React.createElement("td", {
        style: {
          ...
          color: isActive ? "#16a34a" : isFront ? "#0ea5e9" : "#a78bfa"
        }
      }, dispHoleNum), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "5px 4px",
          textAlign: "center",
          color: "#1e293b",
      ...
      }, h.hdcp), /*#__PURE__*/React.createElement("td", {
        style: {
          padding: "5px 4px",
          textAlign: "center",
          color: "#fbbf24",
    ...
    }, allHoles.reduce((s, h) => s + (venue.getYardage(h, green, tee) || 0), 0), "Y")))), /*#__PURE__*/React.createElement("p", {
      style: {
        fontSize: "9px",
        color: "#94a3b8",
        marginTop: "6px"
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("button", {
        ...
        style: {
          fontWeight: "700",
          fontSize: "11px",
          color: active ? "#16a34a" : hasDone ? symColor : hasAny ? "#fbbf24" : "#334155",
          lineHeight: 1
        }
      }, getDisplayHoleNum(h)), hasDone && /*#__PURE__*/React.createElement("span", {
        style: {
          fontSize: "11px",
          fontWeight: "800",
          color: symColor,
    ...
    style: S.card({
      border: `1px solid ${hd.done ? "rgba(22,163,74,0.25)" : "rgba(245,158,11,0.35)"}`,
      background: hd.done ? "#f0fdf4" : "#ffffff"
    })
  }, (() => {
    const venue = currentRound?.venueId ? VENUES.find(v => v.id === currentRound.venueId) : null;
    const allHoles = venue ? getRoundHoles(currentRound) : [];
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "HC", hInfo.hdcp)), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "6px",
        alignItems: "center"
      ...
      }, "\u6253")), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "13px",
          fontWeight: "800",
          color: symColor,
    ...
    }, "\u6253"))), hd.done && /*#__PURE__*/React.createElement("span", {
      style: S.pill("#16a34a")
    }, "\u2713 \u5B8C\u4E86"), hd.done && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        // 最後のショットを取り消してホールを未完了に戻す
        const shots = hd.shots;
        if (!shots.length) return;
        const prev = shots.slice(0, -1);
        let state = initialCategory(par);
        ...
        for (const sh of prev) {
          ...
          const opt = cat?.options.find(o => o.id === sh.optionId);
          if (!opt) continue;
    ...
    }, "\u4FEE\u6B63"))), yard && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "10px",
        marginTop: "5px",
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, hd.shots.slice(0, i).reduce((a, sh) => a + sh.shotCount, i > 0 ? 1 + (hd.extraPenalty || 0) : 1)), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        minWidth: 0
      }
    ...
    }, s.optionLabel), s.remainDist && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "700",
        color: "#fbbf24",
    ...
    }, s.remainDist), s.club && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        fontWeight: "700",
        color: "#0ea5e9",
    ...
    }, clubLabel(s.club))), s.penaltyCount > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        color: "#b91c1c",
        fontSize: "10px"
      }
    }, "\uFF08+", s.penaltyCount, "\u7F70\u30FB\u8A08", s.shotCount, "\u6253\u6D88\u8CBB\uFF09"), s.shotCount > 1 && !s.penaltyCount && s.categoryKey === "putt" && /*#__PURE__*/React.createElement("div", {
      style: {
        color: "#475569",
        fontSize: "10px"
      }
    }, "\uFF08", s.shotCount, "\u6253\u5206\uFF09"), s.note && /*#__PURE__*/React.createElement("div", {
      style: {
        color: "#94a3b8",
        fontSize: "10px"
      }
      ...
      style: {
        ...
        fontSize: "13px",
        flexShrink: 0,
        color: s.quality !== undefined ? s.quality === "○" ? "#16a34a" : s.quality === "△" ? "#fbbf24" : "#94a3b8" : scoreColor(s.score)
      }
    }, s.quality !== undefined ? s.quality : fmt(s.score)));
  }), hd.shots.length === 0 && /*#__PURE__*/React.createElement("p", {
    style: {
      color: "#1e293b",
      fontSize: "12px",
      textAlign: "center",
  ...
  }, "\u3053\u306E\u30DB\u30FC\u30EB\u306F\u307E\u3060\u8A18\u9332\u304C\u3042\u308A\u307E\u305B\u3093"), (hd.extraPenalty || 0) > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "9px",
  ...
  }, "\u305D\u306E\u4ED6\u30DA\u30CA\u30EB\u30C6\u30A3"), /*#__PURE__*/React.createElement("div", {
    style: {
      color: "#b91c1c",
      fontSize: "10px"
    }
  }, "\uFF08+", hd.extraPenalty, "\u7F70\u6253\uFF09")), /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: "800",
      fontSize: "13px",
      color: "#b91c1c"
  ...
  }, !hd.done && currentCatDef ? /*#__PURE__*/React.createElement("button", {
  ...
  }, "\u6B21: ", currentCatDef.label), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "12px",
      color: "#94a3b8",
      fontWeight: "600"
    }
  }, totalStrokes + 1, "\u6253\u76EE \u203A")) : hd.done ? /*#__PURE__*/React.createElement(React.Fragment, null, currentHole > 1 && /*#__PURE__*/React.createElement("button", {
    style: {
      ...S.btn("secondary"),
      padding: "10px 14px",
      fontSize: "12px"
  ...
  }, "\u2039 \u524D\u306E\u30DB\u30FC\u30EB"), /*#__PURE__*/React.createElement("div", {
    style: {
      flex: 1,
      textAlign: "center",
      padding: "12px",
  ...
  }, "\u30DB\u30FC\u30EB\u5B8C\u4E86", hd.pinDist && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "10px",
      color: "#0ea5e9",
      fontWeight: "600",
  ...
  }, hd.pinDist)), currentHole < holePars.length ? /*#__PURE__*/React.createElement("button", {
    style: {
      ...S.btn("primary"),
      padding: "10px 14px",
      fontSize: "12px"
    ...
    onClick: handleDetailFinishClick
  }, "\u30E9\u30A6\u30F3\u30C9\u7D42\u4E86")) : null, !hd.done && hd.shots.length > 0 && /*#__PURE__*/React.createElement("button", {
    style: S.btn("undo"),
    onClick: undoLastShot
  }, "\u21A9 \u53D6\u6D88"))), showExtraPenaltyModal && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      top: 0,
      left: 0,
  ...
  }, "\u26A0\uFE0F \u305D\u306E\u4ED6\u30DA\u30CA\u30EB\u30C6\u30A3"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      marginTop: "4px"
    }
  }, "Hole ", getDisplayHoleNum(currentHole), "\uFF08Par ", par, "\uFF09\u306B\u52A0\u7B97\u3059\u308B\u7F70\u6253\u6570")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
    ...
    style: {
      fontSize: "36px",
      fontWeight: "800",
      color: extraPenaltyDraft > 0 ? "#b91c1c" : "#94a3b8",
      lineHeight: 1
    }
  }, extraPenaltyDraft > 0 ? `+${extraPenaltyDraft}` : "0"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "10px",
      color: "#94a3b8",
      marginTop: "4px"
    }
  }, "\u6253\u7F70")), /*#__PURE__*/React.createElement("button", {
    style: {
      background: "#f1f5f9",
      border: "1.5px solid #cbd5e1",
      borderRadius: "50%",
  ...
  }, "\u9023\u7D9AOB\u30FB\u7A7A\u632F\u308A\u30FB\u8AA4\u7403\u306A\u3069\u3001\u901A\u5E38\u306E\u30B7\u30E7\u30C3\u30C8\u8A18\u9332\u306B\u542B\u307E\u308C\u306A\u3044\u7F70\u6253\u3092\u52A0\u7B97\u3057\u307E\u3059\u30020\u306B\u3059\u308B\u3068\u53D6\u308A\u6D88\u3057\u306B\u306A\u308A\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "8px"
    }
  ...
  }, "\u78BA\u5B9A"))))), view === "round" && currentRound && inputMode === "detail" && currentRound.goal && (() => {
    ...
    const completed = Object.values(holeData).filter(hd => hd.done).length;
    ...
    }, "\u4ECA\u65E5\u306E\u76EE\u6A19"), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        height: "1px",
        background: "#e2e8f0"
    ...
    }, completed, "H\u5B8C\u4E86")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "10px",
    ...
    }, goalDef.label), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "11px",
        color: "#64748b",
        marginTop: "1px"
      }
    }, "\u76EE\u6A19 ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontWeight: "800",
        color: "#1e293b"
      }
    }, goal.target), " \u30DB\u30FC\u30EB")), /*#__PURE__*/React.createElement("div", {
      style: {
        textAlign: "right"
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "/", goal.target)), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        marginTop: "2px"
      }
    }, achieved ? /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#16a34a",
        fontWeight: "800"
      }
    }, "\u2705 \u9054\u6210\uFF01") : /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#64748b"
      }
    ...
    }, remaining), "H")))), /*#__PURE__*/React.createElement("div", {
      style: {
        height: "6px",
        background: "#f1f5f9",
        borderRadius: "3px",
  ...
  })(), showHcpWarning && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
  ...
  }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\u304C\u672A\u8A2D\u5B9A\u3067\u3059"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
      color: "#64748b",
      marginBottom: "20px",
  ...
  }, "HCP\u3092\u8A2D\u5B9A\u3057\u3066\u304B\u3089\u30E9\u30A6\u30F3\u30C9\u3092\u8A18\u9332\u3059\u308B\u3068", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#0ea5e9",
      fontWeight: "700"
    }
  }, "AI\u8A3A\u65AD\u30FB5\u8981\u7D20\u5206\u6790"), "\u304C\u5229\u7528\u3067\u304D\u307E\u3059\u3002", /*#__PURE__*/React.createElement("br", null), "\u3053\u306E\u307E\u307E\u958B\u59CB\u3057\u305F\u5834\u5408\u3001\u4ECA\u56DE\u306E\u30E9\u30A6\u30F3\u30C9\u306F", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#dc2626",
      fontWeight: "700"
    }
  }, "\u5206\u6790\u306A\u3057"), "\u3067\u8A18\u9332\u3055\u308C\u307E\u3059\u3002"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: "8px"
  ...
  }, "\u30DE\u30A4\u30DA\u30FC\u30B8\u3067HCP\u3092\u8A2D\u5B9A\u3059\u308B"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setShowHcpWarning(false);
      if (currentRound) {
        setShowDiscardWarning(true);
  ...
  }, "\u5206\u6790\u306A\u3057\u3067\u3053\u306E\u307E\u307E\u958B\u59CB\u3059\u308B")))), showDiscardWarning && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
      background: "rgba(0,0,0,0.55)",
  ...
  }, "\u5165\u529B\u4E2D\u306E\u30E9\u30A6\u30F3\u30C9\u304C\u3042\u308A\u307E\u3059"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "13px",
      color: "#dc2626",
      fontWeight: "700",
  ...
  }, currentRound?.course || "入力中のラウンド"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
      color: "#64748b",
      marginBottom: "20px"
    }
  }, "\u7834\u68C4\u3057\u3066\u65B0\u3057\u3044\u30E9\u30A6\u30F3\u30C9\u3092\u958B\u59CB\u3057\u307E\u3059\u304B\uFF1F", /*#__PURE__*/React.createElement("br", null), "\u5165\u529B\u6E08\u307F\u306E\u30C7\u30FC\u30BF\u306F\u5931\u308F\u308C\u307E\u3059\u3002"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "10px"
    }
  ...
  }, "\u7834\u68C4\u3057\u3066\u958B\u59CB")))), showDetailPaywall && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
      background: "rgba(0,0,0,0.55)",
  ...
  }, "\u8A73\u7D30\u30E2\u30FC\u30C9\u3092\u3082\u3063\u3068\u4F7F\u3046\u306B\u306F")), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
      color: "#64748b",
      lineHeight: 1.7,
  ...
  }, "\u8A73\u7D30\u30E2\u30FC\u30C9\u306F\u7121\u6599\u3067", DETAIL_FREE_LIMIT, "\u30E9\u30A6\u30F3\u30C9\u307E\u3067\u304A\u8A66\u3057\u3067\u304D\u3001", DETAIL_FREE_LIMIT + 1, "\u30E9\u30A6\u30F3\u30C9\u76EE\u4EE5\u964D\u306F", /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#d97706",
      fontWeight: "800"
    }
  }, DETAIL_PRICE_LABEL), "\u3067\u4F7F\u3048\u308B\u3088\u3046\u306B\u306A\u308A\u307E\u3059\uFF08\u304A\u8A66\u3057\u8CA9\u58F2\u4E2D\uFF09\u3002\u7C21\u6613\u30E2\u30FC\u30C9\u306F\u7121\u6599\u3067\u4F7F\u3044\u653E\u984C\u3067\u3059\u3002"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      try {
  ...
  }, DETAIL_PRICE_LABEL, "\u3067\u89E3\u9664\u3059\u308B\uFF08\u8CFC\u5165\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      borderTop: "1px solid #e2e8f0",
      paddingTop: "14px",
      marginBottom: "12px"
  ...
  }, "\u8CFC\u5165\u6E08\u307F\u306E\u65B9\uFF1A\u89E3\u9664\u30B3\u30FC\u30C9\u3092\u5165\u529B"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "8px"
    }
    ...
    onClick: applyUnlockCode,
    style: {
      ...S.btn("secondary"),
      whiteSpace: "nowrap"
    }
  }, "\u9069\u7528")), unlockError && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#dc2626",
      fontWeight: "700",
  ...
  }, unlockError)), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setShowDetailPaywall(false);
      setUnlockInput("");
      setUnlockError("");
  ...
  }, "\u9589\u3058\u308B"))), finishConfirm && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
      background: "rgba(0,0,0,0.50)",
    ...
    style: {
      background: "#ffffff",
      border: `1px solid ${finishConfirm.type === "ok" ? "rgba(52,211,153,0.4)" : "rgba(251,191,36,0.4)"}`,
      borderRadius: "16px",
      padding: "24px 20px",
      maxWidth: "320px",
      width: "100%",
  ...
  }, finishConfirm.type === "ok" ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontWeight: "800",
      fontSize: "15px",
      color: "#e2e8f0",
  ...
  }, "\u30B9\u30B3\u30A2\u3092\u767B\u9332\u3057\u307E\u3059"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "10px"
    }
  ...
  }, "\u672A\u5165\u529B\u306E\u30DB\u30FC\u30EB\u304C\u3042\u308A\u307E\u3059"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexWrap: "wrap",
      gap: "6px",
  ...
  }, finishConfirm.incompleteHoles.map(h => /*#__PURE__*/React.createElement("span", {
    key: h,
    style: {
      padding: "4px 10px",
      borderRadius: "20px",
  ...
  }, getDisplayHoleNum(h), "\u756A"))), /*#__PURE__*/React.createElement("button", {
    onClick: () => setFinishConfirm(null),
    style: {
      width: "100%",
      padding: "11px",
  ...
  }, "OK")))), view === "analytics" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: "18px"
    }
  }, /*#__PURE__*/React.createElement("h2", {
  ...
  }, "\u30AF\u30E9\u30D6\u8A2D\u5B9A")), REXY_IMAGES.basic5 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: "18px"
    }
  }, /*#__PURE__*/React.createElement(RexyBubble, {
    costume: "basic5",
    size: 64
  }, "\u4F7F\u7528\u3059\u308B\u30AF\u30E9\u30D6\u3092\u6559\u3048\u3066\u306D\u3002")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      justifyContent: "space-between",
      alignItems: "center",
  ...
  }, savedClubs.length), "\u672C"), savedClubs.length > 0 && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setSavedClubs([]);
      try {
        (() => {
      ...
      if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
        window.storage.set('golf_clubs', '[]').catch(() => {});
      }
    },
    style: {
      fontSize: "11px",
  ...
  }, "\u3059\u3079\u3066\u30AF\u30EA\u30A2")), CLUB_CATEGORIES.map(cat => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, selectedInCat, "/", clubs.length)), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "6px",
        flexWrap: "wrap"
      ...
      return /*#__PURE__*/React.createElement("button", {
        ...
        onClick: () => {
          ...
          if (typeof window !== 'undefined' && window.storage && typeof window.storage.set === 'function') {
            window.storage.set('golf_clubs', JSON.stringify(next)).catch(() => {});
          }
        },
        style: {
          padding: "7px 11px",
  ...
  }), savedClubs.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: S.card({
      border: "1px solid rgba(52,211,153,0.2)"
    })
  }, /*#__PURE__*/React.createElement("label", {
    style: S.lbl
  }, "\u9078\u629E\u4E2D\u306E\u30AF\u30E9\u30D6\u4E00\u89A7"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "5px",
      flexWrap: "wrap"
  ...
  }, "\u30C7\u30FC\u30BF\u7BA1\u7406"), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "#64748b",
      fontSize: "11px"
    }
  }, "\u30C7\u30FC\u30BF\u306E\u66F8\u304D\u51FA\u3057\u30FB\u30A4\u30F3\u30DD\u30FC\u30C8\uFF08\u5FA9\u5143\uFF09\u203B\u6A5F\u7A2E\u5909\u66F4\u6642\u306E\u30C7\u30FC\u30BF\u5F15\u304D\u7D99\u304E\u306B\u4F7F\u7528")), /*#__PURE__*/React.createElement("span", {
    "aria-hidden": "true",
    style: {
      color: "#64748b",
      fontSize: "13px",
  ...
  }, "\u25B6"))), dataMgmtOpen && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("p", {
    style: {
      color: "#475569",
      fontSize: "12px",
      marginBottom: "8px"
    }
  }, "\u30A2\u30D7\u30EA\u66F4\u65B0\u6642\u306F\u30D0\u30C3\u30AF\u30A2\u30C3\u30D7\u3092\u53D6\u3063\u3066\u304B\u3089\u3001\u30A4\u30F3\u30DD\u30FC\u30C8\u3067\u5FA9\u5143\u3057\u3066\u304F\u3060\u3055\u3044"), /*#__PURE__*/React.createElement("div", {
    style: S.card()
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
  ...
  }, "\u30C7\u30FC\u30BF\u66F8\u304D\u51FA\u3057"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      marginBottom: "12px",
  ...
  }, "\u30E9\u30A6\u30F3\u30C9\u30C7\u30FC\u30BF\u3092JSON\u3068\u3057\u3066\u8868\u793A\u3057\u307E\u3059\u3002\u30B3\u30D4\u30FC\u3057\u3066\u30C6\u30B9\u30C8\u30C7\u30FC\u30BF\u6295\u5165\u30C4\u30FC\u30EB\u306B\u8CBC\u308A\u4ED8\u3051\u3066\u304F\u3060\u3055\u3044\u3002"), /*#__PURE__*/React.createElement("button", {
  ...
  }, "JSON\u3092\u8868\u793A\u3059\u308B\uFF08", rounds.length, "\u4EF6\uFF09")), /*#__PURE__*/React.createElement("div", {
    style: S.card()
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
  ...
  }, "\u30A4\u30F3\u30DD\u30FC\u30C8\uFF08\u5FA9\u5143\u30FB\u30C6\u30B9\u30C8\u30C7\u30FC\u30BF\u8AAD\u307F\u8FBC\u307F\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      marginBottom: "12px",
  ...
  }, "\u30C6\u30B9\u30C8\u30C7\u30FC\u30BF\u6295\u5165\u30C4\u30FC\u30EB\u3067\u751F\u6210\u3057\u305FJSON\u3092\u8CBC\u308A\u4ED8\u3051\u308B\u304B\u3001\u30D0\u30C3\u30AF\u30A2\u30C3\u30D7\u30D5\u30A1\u30A4\u30EB\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044\u3002"), /*#__PURE__*/React.createElement("textarea", {
  ...
  }, "\u8CBC\u308A\u4ED8\u3051\u305FJSON\u3092\u8AAD\u307F\u8FBC\u3080"), /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      textAlign: "center",
      padding: "10px",
  ...
  }, "\u307E\u305F\u306F\u30D5\u30A1\u30A4\u30EB\u3092\u9078\u629E\u3057\u3066\u30A4\u30F3\u30DD\u30FC\u30C8", /*#__PURE__*/React.createElement("input", {
    type: "file",
    accept: ".json",
    ...
    // ── モックアップコンポーネント ──
    const MockHome = () => /*#__PURE__*/React.createElement("div", {
      style: M.mockFrame
    }, /*#__PURE__*/React.createElement("div", {
      style: M.mockHeader
    }, /*#__PURE__*/React.createElement("span", {
    ...
    }, "ScoRExolution"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "3px"
      }
    }, ["ホーム", "分析", "クラブ", "使い方"].map(t => /*#__PURE__*/React.createElement("span", {
      key: t,
      style: M.mockNavBtn(t === "ホーム")
    }, t)))), /*#__PURE__*/React.createElement("div", {
      style: M.mockBody
    }, /*#__PURE__*/React.createElement("div", {
      style: M.mockBtn
    }, "\uFF0B \u65B0\u3057\u3044\u30E9\u30A6\u30F3\u30C9\u3092\u958B\u59CB"), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: "10px",
        fontSize: "9px",
        fontWeight: "700",
    ...
    const MockSetup = ({
    ...
    }, "\u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "9px",
        fontWeight: "700",
        color: "#475569",
      ...
      style: {
        ...
        padding: "8px",
        borderRadius: "8px",
        border: active === "detail" ? "2px solid #0ea5e9" : "1px solid #e2e8f0",
        background: active === "detail" ? "rgba(14,165,233,0.10)" : "#f8fafc",
        textAlign: "center"
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        fontWeight: "800",
        color: active === "detail" ? "#0ea5e9" : "#64748b"
      }
    }, "\u8A73\u7D30\u30E2\u30FC\u30C9"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "8px",
        color: active === "detail" ? "#475569" : "#94a3b8"
      }
    }, "\u30B7\u30E7\u30C3\u30C8\u5358\u4F4D\u3067\u8A18\u9332")), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        padding: "8px",
        borderRadius: "8px",
        border: active === "simple" ? "2px solid #16a34a" : "1px solid #e2e8f0",
        background: active === "simple" ? "rgba(22,163,74,0.10)" : "#f8fafc",
        textAlign: "center"
      }
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        fontWeight: "800",
        color: active === "simple" ? "#16a34a" : "#64748b"
      }
    }, "\u7C21\u6613\u30E2\u30FC\u30C9"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "8px",
        color: active === "simple" ? "#475569" : "#94a3b8"
      }
    }, "\u30B9\u30B3\u30A2\u30FB\u8A55\u4FA1\u306E\u307F"))), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "9px",
        fontWeight: "700",
        color: "#475569",
    ...
    }, "\u30B4\u30EB\u30D5\u5834"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.mockInput,
        marginBottom: "10px"
      }
    }, "\u30C0\u30C3\u30D5\u30A1\u30FC\u30B4\u30EB\u30D5\u30AF\u30E9\u30D6 \u25BC"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "5px",
        marginBottom: "10px"
      }
    }, ["中コース", "西コース", "東コース"].map((c, i) => /*#__PURE__*/React.createElement("div", {
      key: c,
      style: {
        padding: "5px 8px",
        borderRadius: "6px",
        fontSize: "9px",
        fontWeight: "700",
        border: i === 0 ? "2px solid #16a34a" : "1px solid #e2e8f0",
        background: i === 0 ? "rgba(22,163,74,0.10)" : "#f8fafc",
        color: i === 0 ? "#16a34a" : "#475569"
      }
    }, c))), /*#__PURE__*/React.createElement("div", {
      style: M.mockBtn
    }, "\u30B9\u30BF\u30FC\u30C8\uFF01")));
    const MockRecord = () => /*#__PURE__*/React.createElement("div", {
      style: M.mockFrame
    }, /*#__PURE__*/React.createElement("div", {
      style: M.mockHeader
    }, /*#__PURE__*/React.createElement("div", {
      ...
      length: 9
    }, (_, i) => i + 1).map(h => /*#__PURE__*/React.createElement("div", {
      key: h,
      style: {
        width: "28px",
        height: "28px",
        borderRadius: "6px",
        background: h === 3 ? "rgba(22,163,74,0.18)" : h < 3 ? "rgba(22,163,74,0.06)" : "#f8fafc",
        border: h === 3 ? "2px solid #16a34a" : h < 3 ? "1px solid rgba(22,163,74,0.3)" : "1px solid #e2e8f0",
        color: h === 3 ? "#16a34a" : h < 3 ? "#94a3b8" : "#334155",
        fontSize: "10px",
        fontWeight: "700",
        display: "flex",
        alignItems: "center",
    ...
    }, "3\u756A\u30DB\u30FC\u30EB\uFF08Par4\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
        gridTemplateColumns: "1fr 1fr",
        gap: "8px",
    ...
    }, "\u30D1\u30C3\u30C8\u6570"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "6px"
    ...
    }, [["OB", "0"], ["ペナルティ", "0"], ["バンカー", "1"]].map(([lb, v]) => /*#__PURE__*/React.createElement("div", {
      key: lb
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "8px",
    ...
    }, "\u30B7\u30E7\u30C3\u30C8\u8A55\u4FA1"), [["ティショット", "good"], ["セカンドショット", "fair"], ["アプローチ", "bad"]].map(([nm, ev]) => /*#__PURE__*/React.createElement("div", {
      key: nm,
      style: {
        display: "flex",
        justifyContent: "space-between",
    ...
    }, [["#16a34a", "○"], ["#fbbf24", "△"], ["#dc2626", "×"]].map(([c, l]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: {
        width: "26px",
        height: "26px",
        borderRadius: "6px",
        background: ev === "good" && l === "○" || ev === "fair" && l === "△" || ev === "bad" && l === "×" ? `${c}20` : "#f8fafc",
        border: ev === "good" && l === "○" || ev === "fair" && l === "△" || ev === "bad" && l === "×" ? `2px solid ${c}` : "1px solid #e2e8f0",
        color: ev === "good" && l === "○" || ev === "fair" && l === "△" || ev === "bad" && l === "×" ? c : "#475569",
        fontWeight: "800",
        fontSize: "12px",
        display: "flex",
        alignItems: "center",
    ...
    // 詳細モード：ホール入力画面（ショットログ＋次の一手）
    const MockRecordDetail = () => /*#__PURE__*/React.createElement("div", {
      style: M.mockFrame
    }, /*#__PURE__*/React.createElement("div", {
      style: M.mockHeader
    }, /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        height: "26px",
        borderRadius: "6px",
        background: h === 3 ? "rgba(14,165,233,0.18)" : h < 3 ? "rgba(22,163,74,0.06)" : "#f8fafc",
        border: h === 3 ? "2px solid #0ea5e9" : h < 3 ? "1px solid rgba(22,163,74,0.3)" : "1px solid #e2e8f0",
        color: h === 3 ? "#0ea5e9" : h < 3 ? "#16a34a" : "#334155",
        fontSize: "10px",
        fontWeight: "700",
        display: "flex",
        alignItems: "center",
    ...
    }, "3\u756A ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: "#94a3b8",
        fontWeight: "400",
        fontSize: "10px"
      }
    }, "Par4")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "4px"
      }
    ...
    }, s.club))))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
    ...
    }, "\u6B21: \u30D1\u30C3\u30C8"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        color: "#94a3b8",
        fontWeight: "700"
    ...
    // 詳細モード：ショット入力モーダル
    const MockShotForm = () => /*#__PURE__*/React.createElement("div", {
      style: M.mockFrame
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.mockHeader,
    ...
    }, "3\u756A\u30DB\u30FC\u30EB\uFF08Par4\uFF09"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        color: "#94a3b8",
        fontWeight: "700"
    ...
    }, "2\u6253\u76EE \u2014 \u30A2\u30D7\u30ED\u30FC\u30C1"), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "10px"
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u6B8B\u308A\u8DDD\u96E2 ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "8px",
        color: "#f87171",
        fontWeight: "700",
    ...
    }, "\u5FC5\u9808")), /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#ffffff",
    ...
    }, "\u4F7F\u7528\u30AF\u30E9\u30D6 ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "8px",
        color: "#f87171",
        fontWeight: "700",
    ...
    }, ["6I", "7I", "8I", "9I", "PW"].map((c, i) => /*#__PURE__*/React.createElement("div", {
      key: c,
      style: {
        padding: "6px 10px",
        borderRadius: "6px",
        fontSize: "10px",
        fontWeight: "800",
        border: i === 1 ? "1px solid rgba(14,165,233,0.6)" : "1px solid rgba(14,165,233,0.2)",
        background: i === 1 ? "rgba(14,165,233,0.18)" : "#ffffff",
        color: i === 1 ? "#0ea5e9" : "#93c5fd"
      }
    }, c)))), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "4px"
    ...
    }, "\u7D50\u679C ", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "8px",
        color: "#f87171",
        fontWeight: "700",
    ...
    }, [["○", "#16a34a", true], ["△", "#a16207", false], ["×", "#64748b", false], ["グリーンオン", "#16a34a", false], ["カップイン", "#16a34a", false]].map(([l, c, sel]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: {
        padding: "7px 10px",
        borderRadius: "7px",
    ...
    }, sel ? "✓ " : "", l))))), /*#__PURE__*/React.createElement("div", {
      style: {
    ...
    const MockAnalytics = () => {
      ...
      const gridLvls = [0.25, 0.5, 0.75, 1.0];
      return /*#__PURE__*/React.createElement("div", {
        style: M.mockFrame
      }, /*#__PURE__*/React.createElement("div", {
        style: M.mockHeader
      ...
      }, "\u30D7\u30EC\u30FC\u5206\u6790")), /*#__PURE__*/React.createElement("div", {
        style: M.mockBody
      }, /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "9px",
      ...
      }, "\u8A55\u4FA1\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8"), /*#__PURE__*/React.createElement("svg", {
        width: "100%",
        viewBox: "0 0 180 170",
        style: {
          display: "block"
        ...
        d: poly(labels.map((_, i) => pt(i, R * 0.75))),
        ...
        stroke: "#fbbf24",
        strokeWidth: "2"
      }), latestPts.map(([x, y], i) => /*#__PURE__*/React.createElement("circle", {
        key: i,
        cx: x,
        cy: y,
        r: "2.5",
      ...
      }), " \u76F4\u8FD15R\u5E73\u5747"), /*#__PURE__*/React.createElement("div", {
        style: {
          display: "flex",
          alignItems: "center",
          gap: "3px",
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      ...
      style: M.lbl
    }, "2\u3064\u306E\u5165\u529B\u30E2\u30FC\u30C9\u3092\u4F7F\u3044\u5206\u3051"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
        gridTemplateColumns: "1fr 1fr",
        gap: "8px"
    ...
    }, "1\u30DB\u30FC\u30EB30\u79D2\u3002\u30B9\u30B3\u30A2\u30FB\u30D1\u30C3\u30C8\u30FB\u30B7\u30E7\u30C3\u30C8\u8A55\u4FA1\u306E\u307F\u3002", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("b", null, "\u30E9\u30A6\u30F3\u30C9\u4E2D\u306B\u624B\u65E9\u304F"), "\u8A18\u9332\u3057\u305F\u3044\u4EBA\u5411\u3051\u3002")), /*#__PURE__*/React.createElement("div", {
      style: {
        padding: "10px",
        borderRadius: "8px",
        border: "1px solid rgba(14,165,233,0.2)",
    ...
    }, "1\u6253\u305A\u3064\u8DDD\u96E2\u30FB\u30AF\u30E9\u30D6\u30FB\u7D50\u679C\u3092\u8A18\u9332\u3002", /*#__PURE__*/React.createElement("br", null), /*#__PURE__*/React.createElement("b", null, "\u8DDD\u96E2\u5225\uFF0F\u30AF\u30E9\u30D6\u5225\u306E\u5206\u6790"), "\u304C\u898B\u305F\u3044\u4EBA\u5411\u3051\u3002"))), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#64748b",
        marginTop: "8px",
    ...
    }, "\u3069\u3061\u3089\u306E\u30E2\u30FC\u30C9\u3067\u3082\u57FA\u672C\u5206\u6790\uFF08\u63A8\u79FB\u30B0\u30E9\u30D5\u30FB\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8\uFF09\u306F\u540C\u3058\u3088\u3046\u306B\u4F7F\u3048\u307E\u3059\u3002\u8A73\u7D30\u30E2\u30FC\u30C9\u306F\u3055\u3089\u306B\u4E0A\u7D1A\u5206\u6790\u304C\u8FFD\u52A0\u3067\u898B\u3089\u308C\u307E\u3059\u3002"), /*#__PURE__*/React.createElement("div", {
      style: {
        marginTop: "10px",
        padding: "10px",
        borderRadius: "8px",
    ...
    }, "\u6599\u91D1\u306B\u3064\u3044\u3066"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#475569",
        lineHeight: 1.6
    ...
    }, "\u7C21\u6613\u30E2\u30FC\u30C9\u306F\u7121\u6599\u3067\u4F7F\u3044\u653E\u984C"), "\u3067\u3059\u3002", /*#__PURE__*/React.createElement("b", {
      style: {
        color: "#0ea5e9"
      }
    }, "\u8A73\u7D30\u30E2\u30FC\u30C9"), "\u306F\u7121\u6599\u3067", DETAIL_FREE_LIMIT, "\u30E9\u30A6\u30F3\u30C9\u307E\u3067\u304A\u8A66\u3057\u3067\u304D\u3001", DETAIL_FREE_LIMIT + 1, "\u30E9\u30A6\u30F3\u30C9\u76EE\u4EE5\u964D\u306F", DETAIL_PRICE_LABEL, "\u3067\u4F7F\u3048\u308B\u3088\u3046\u306B\u306A\u308A\u307E\u3059\uFF08\u304A\u8A66\u3057\u8CA9\u58F2\u4E2D\uFF09\u3002"))), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "8px"
      }
    }, /*#__PURE__*/React.createElement("div", {
      ...
      style: M.chBadge("#16a34a")
    }, "1"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      style: M.secH
    }, "\u7C21\u6613\u30E2\u30FC\u30C9\u306E\u5165\u529B\u65B9\u6CD5"), /*#__PURE__*/React.createElement("span", {
      style: M.secS
    }, "1\u30DB\u30FC\u30EB30\u79D2\u3067\u3067\u304D\u308B\u3001\u304B\u3093\u305F\u3093\u306A\u8A18\u9332\u65B9\u6CD5")))), /*#__PURE__*/React.createElement("div", {
      style: M.accentG
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 1 \u2014 \u30E9\u30A6\u30F3\u30C9\u3092\u59CB\u3081\u308B"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30DB\u30FC\u30E0\u753B\u9762\u306E\u300C\uFF0B \u65B0\u3057\u3044\u30E9\u30A6\u30F3\u30C9\u3092\u958B\u59CB\u300D\u3092\u30BF\u30C3\u30D7"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u8A2D\u5B9A\u753B\u9762\u304C\u958B\u3044\u305F\u3089\u3001\u5165\u529B\u30E2\u30FC\u30C9\u3067\u300C\u7C21\u6613\u30E2\u30FC\u30C9\u300D\u3092\u9078\u629E\u3057\u307E\u3059\u3002")), /*#__PURE__*/React.createElement(MockHome, null), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#475569",
        textAlign: "center",
    ...
    }, "\u2191 \u30DB\u30FC\u30E0\u753B\u9762\u3002\u30E9\u30A6\u30F3\u30C9\u5C65\u6B74\u3068\u30CF\u30F3\u30C7\u30A3\u304C\u78BA\u8A8D\u3067\u304D\u307E\u3059"), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement(Step, {
      n: "1",
      title: "\u7C21\u6613\u30E2\u30FC\u30C9 \u3092\u9078\u3076",
      body: "\u30B9\u30B3\u30A2\u3068\u8A55\u4FA1\u3092\u30B5\u30AF\u30C3\u3068\u5165\u529B\u3057\u305F\u3044\u306A\u3089\u3053\u3061\u3089\u3002"
    }), /*#__PURE__*/React.createElement(Step, {
      n: "2",
      title: "\u30B4\u30EB\u30D5\u5834\u3092\u9078\u3076",
      body: "\u30D7\u30EB\u30C0\u30A6\u30F3\u304B\u3089\u9078\u629E\u3002\u5BFE\u5FDC\u30B4\u30EB\u30D5\u5834\u62E1\u5927\u4E2D\u3002"
    }), /*#__PURE__*/React.createElement(Step, {
      n: "3",
      title: "\u30B3\u30FC\u30B9\u30FB\u30B0\u30EA\u30FC\u30F3\u30FB\u30C6\u30A3\u30FC\u3092\u8A2D\u5B9A",
      body: "\u300C\u4E2D\u2192\u897F\u300D\u300C\u30D9\u30F3\u30C8\u300D\u300C\u30EC\u30AE\u30E5\u30E9\u30FC\u300D\u306E\u3088\u3046\u306B\u6307\u5B9A\u3002"
    }), /*#__PURE__*/React.createElement(Step, {
      n: "4",
      title: "\u65E5\u4ED8\u30FB\u5929\u6C17\u30FB\u98A8\u3092\u5165\u529B\u3057\u3066\u30B9\u30BF\u30FC\u30C8\uFF01",
      body: "\u5929\u6C17\u30FB\u98A8\u306F\u3056\u3063\u304F\u308A\u3067OK\u3002\u5F8C\u304B\u3089\u4FEE\u6B63\u3082\u3067\u304D\u307E\u3059\u3002"
    })), /*#__PURE__*/React.createElement(MockSetup, {
      active: "simple"
    }), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
    ...
    }, "\u2191 \u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A\u753B\u9762\u3002\u7C21\u6613\u30E2\u30FC\u30C9\u3092\u9078\u3093\u3067\u30B9\u30BF\u30FC\u30C8\uFF01"), /*#__PURE__*/React.createElement("div", {
      style: M.accentG
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 2 \u2014 \u30DB\u30FC\u30EB\u3054\u3068\u306B\u5165\u529B\u3059\u308B"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u5404\u30DB\u30FC\u30EB\u30676\u9805\u76EE\u3092\u5165\u529B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u4E0A\u306E\u30DB\u30FC\u30EB\u756A\u53F7\u30DC\u30BF\u30F3\u3067\u79FB\u52D5\u3067\u304D\u307E\u3059\u3002\u300C\u2039 \u524D\u306E\u30DB\u30FC\u30EB\u300D\u300C\u6B21\u306E\u30DB\u30FC\u30EB \u203A\u300D\u30DC\u30BF\u30F3\u3067\u3082\u79FB\u52D5\u53EF\u80FD\u3067\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
    ...
    }, n, req && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "9px",
        color: "#dc2626",
        marginLeft: "4px"
    ...
    }, "\u30D0\u30F3\u30AB\u30FC\u306B\u5165\u3063\u305F\u56DE\u6570\u30020\u306E\u307E\u307E\u3067\u3082OK\u3067\u3059\u3002"))), /*#__PURE__*/React.createElement("div", {
      style: M.accentG
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 3 \u2014 \u30B7\u30E7\u30C3\u30C8\u8A55\u4FA1\u304C\u8D85\u91CD\u8981\uFF01"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u5404\u30B7\u30E7\u30C3\u30C8\u306E\u51FA\u6765\u3092 \u25CB \u25B3 \xD7 \u306E3\u6BB5\u968E\u3067\u5165\u529B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
      ...
      when: "パーオンなら「無」を選択",
      req: true,
      btns: [["#16a34a", "○"], ["#fbbf24", "△"], ["#dc2626", "×"], ["#64748b", "無"]]
    }].map(({
      ic,
      nm,
      when,
    ...
    }, /*#__PURE__*/React.createElement("span", null, ic), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "12px",
        fontWeight: "700",
        color: "#1e293b"
      }
    }, nm), req && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "9px",
        color: "#dc2626",
        marginLeft: "5px"
    ...
    }, when))), /*#__PURE__*/React.createElement(EvalBtns, {
      items: btns
    }))), /*#__PURE__*/React.createElement("div", {
      style: M.tipBox
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u8A55\u4FA1\u306E\u76EE\u5B89"), [["#16a34a", "○（良い）", "ねらい通りに打てた"], ["#fbbf24", "△（普通）", "少しミス、次が狙いづらい"], ["#dc2626", "×（悪い）", "バンカー・OB・池"]].map(([c, l, d]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: {
        display: "flex",
        alignItems: "center",
    ...
    }, d))))), /*#__PURE__*/React.createElement(MockRecord, null), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#475569",
        textAlign: "center",
    ...
    }, "\u2191 \u7C21\u6613\u30E2\u30FC\u30C9\u306E\u30DB\u30FC\u30EB\u5165\u529B\u753B\u9762"), /*#__PURE__*/React.createElement("div", {
      style: M.accentG
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 4 \u2014 \u30DB\u30FC\u30EB\u79FB\u52D5 & \u7D42\u4E86"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "18\u30DB\u30FC\u30EB\u5165\u529B\u3057\u305F\u3089\u300C\u7D42\u4E86\u300D\u30DC\u30BF\u30F3"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u30DB\u30FC\u30EB\u756A\u53F7\u30DC\u30BF\u30F3\u306F\u5165\u529B\u72B6\u6CC1\u306B\u5FDC\u3058\u3066\u8272\u304C\u5909\u308F\u308A\u307E\u3059\u3002\u30B9\u30B3\u30A2\u8A18\u53F7\u306E\u610F\u5473\u306F\u6B21\u306E\u901A\u308A\u3067\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, [["◎", "#dc2626", "イーグル以上", "パー -2以下"], ["○", "#ef4444", "バーディ", "パー -1"], ["－", "#16a34a", "パー", "基準通り"], ["△", "#60a5fa", "ボギー", "パー +1"], ["□", "#3b82f6", "ダブルボギー", "パー +2"], ["■", "#1d4ed8", "トリプル以上", "パー +3以上"]].map(([s, c, l, d]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: M.scoreRow
    }, /*#__PURE__*/React.createElement("span", {
      style: {
    ...
    }, "\u8A18\u9332\u306E\u30B3\u30C4"), ["ティショット・アプローチは必須。この2つがないと分析ができません。", "分析には3ラウンド以上必要。まず3回記録してみましょう！"].map(t => /*#__PURE__*/React.createElement("div", {
      key: t,
      style: {
        display: "flex",
        gap: "7px",
      ...
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 1 \u2014 \u8A73\u7D30\u30E2\u30FC\u30C9\u3067\u958B\u59CB"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A\u3067\u300C\u8A73\u7D30\u30E2\u30FC\u30C9\u300D\u3092\u9078\u629E"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u305D\u306E\u5F8C\u306E\u6D41\u308C\uFF08\u30B4\u30EB\u30D5\u5834\u30FB\u30B3\u30FC\u30B9\u30FB\u30B0\u30EA\u30FC\u30F3\u30FB\u30C6\u30A3\u30FC\u30FB\u65E5\u4ED8\uFF09\u306F\u7C21\u6613\u30E2\u30FC\u30C9\u3068\u540C\u3058\u3067\u3059\u3002")), /*#__PURE__*/React.createElement(MockSetup, {
      active: "detail"
    }), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
    ...
    }, "\u2191 \u300C\u8A73\u7D30\u30E2\u30FC\u30C9\u300D\u3092\u30BF\u30C3\u30D7\u3057\u3066\u9078\u629E"), /*#__PURE__*/React.createElement("div", {
      style: M.accentB
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 2 \u2014 \u30DB\u30FC\u30EB\u3092\u9078\u3093\u3067\u30B7\u30E7\u30C3\u30C8\u8A18\u9332\u958B\u59CB"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30DB\u30FC\u30EB\u756A\u53F7 \u2192 Par\u9078\u629E \u2192 \u300C\u6B21:\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u300D\u3092\u30BF\u30C3\u30D7"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u30DB\u30FC\u30EB\u3054\u3068\u306B\u6700\u521D\u306F\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u304B\u3089\u8A18\u9332\u304C\u59CB\u307E\u308A\u307E\u3059\u3002\u30A2\u30F3\u30D0\u30FC\u8272\u306E\u30DC\u30BF\u30F3\u304C\u300C\u6B21\u306B\u5165\u529B\u3059\u3079\u304D\u30B7\u30E7\u30C3\u30C8\u300D\u3092\u793A\u3057\u3066\u3044\u307E\u3059\u3002")), /*#__PURE__*/React.createElement(MockRecordDetail, null), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#475569",
        textAlign: "center",
    ...
    }, "\u2191 \u8A73\u7D30\u30E2\u30FC\u30C9\u306E\u30DB\u30FC\u30EB\u5165\u529B\u753B\u9762\uFF083\u6253\u76EE\u307E\u3067\u30ED\u30B0\u8868\u793A\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: M.accentB
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 3 \u2014 1\u30B7\u30E7\u30C3\u30C8\u305A\u3064\u5165\u529B"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30B7\u30E7\u30C3\u30C8\u5165\u529B\u30E2\u30FC\u30C0\u30EB\u304C\u958B\u304F"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "4\u9805\u76EE\u3092\u9806\u306B\u5165\u529B\u3057\u307E\u3059\u3002\u3059\u3079\u3066\u306E\u5FC5\u9808\u9805\u76EE\u3092\u5165\u308C\u308B\u3068\u300C\u8A18\u9332\u3059\u308B\u300D\u30DC\u30BF\u30F3\u304C\u7DD1\u8272\u306B\u306A\u308A\u3001\u62BC\u3059\u3068\u81EA\u52D5\u3067\u6B21\u306E\u30B7\u30E7\u30C3\u30C8\u3078\u9032\u307F\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement(Step, {
      n: "1",
      color: "#0ea5e9",
      ...
      }, "\xD7\u3092\u9078\u3076\u3068\u30DF\u30B9\u306E\u7A2E\u985E"), "\uFF08\u30D0\u30F3\u30AB\u30FC\u30FB1\u30DA\u30CA\u30FBOB\uFF09\u306E\u9078\u629E\u304C\u8FFD\u52A0\u3067\u51FA\u307E\u3059\u3002OB\u306F\u30C6\u30A3\u30B7\u30E7\u30C3\u30C8\u30672\u6253\u7F70\uFF08\u524D\u90324\u6253\u6271\u3044\uFF09\u3001\u30A2\u30D7\u30ED\u30FC\u30C1\u4EE5\u964D\u30671\u6253\u7F70\u3068\u3057\u3066\u8A08\u7B97\u3055\u308C\u307E\u3059\u3002")
    }), /*#__PURE__*/React.createElement(Step, {
      n: "4",
      color: "#0ea5e9",
      title: "\u30E1\u30E2\uFF08\u4EFB\u610F\uFF09",
    ...
    }, "\u2191 \u30B7\u30E7\u30C3\u30C8\u5165\u529B\u30E2\u30FC\u30C0\u30EB\uFF08\u30A2\u30D7\u30ED\u30FC\u30C1\u30FB150Y\u30FB7I\u30FB\u25CB\u306E\u4F8B\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: M.accentB
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 4 \u2014 \u30B0\u30EA\u30FC\u30F3\u5230\u9054\u5F8C\u306F\u30D1\u30C3\u30C8\u5165\u529B"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u300C\u30B0\u30EA\u30FC\u30F3\u30AA\u30F3\u300D\u3092\u9078\u3076\u3068\u30D1\u30C3\u30C8\u5165\u529B\u3078\u81EA\u52D5\u5207\u66FF"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "1\u30D1\u30C3\u30C8\u3067\u30AB\u30C3\u30D7\u30A4\u30F3\u306A\u3089\u300C1\u30D1\u30C3\u30C8 \u25CB\u300D\u30012\u30D1\u30C3\u30C8\u306A\u3089\u300C2\u30D1\u30C3\u30C8 \u25B3\u300D\u3001\u3068", /*#__PURE__*/React.createElement("b", null, "\u30D1\u30C3\u30C8\u6570\u307E\u3068\u3081\u30661\u56DE\u3067\u8A18\u9332"), "\u3067\u304D\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
    ...
    }, [["1パット", "#16a34a"], ["2パット", "#fbbf24"], ["3パット", "#64748b"], ["4パット", "#dc2626"], ["5パット", "#b91c1c"]].map(([l, c]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: {
        padding: "8px 4px",
        borderRadius: "7px",
    ...
    }, "\u30D1\u30C3\u30C8\u306E\u5F8C\u306F\u300C\u30AB\u30C3\u30D7\u307E\u3067\u306E\u8DDD\u96E2\u300D\u3092\u805E\u304B\u308C\u308B\u306E\u3067\u30BF\u30C3\u30D7\u3057\u3066\u8A18\u9332\uFF08\u4EFB\u610F\uFF09\u3002\u3053\u308C\u3067\u5206\u6790\u306E", /*#__PURE__*/React.createElement("b", {
      style: {
        color: "#0ea5e9"
      }
    }, "\u8DDD\u96E2\u5225\u30D1\u30C3\u30C8\u7D71\u8A08"), "\u304C\u3088\u308A\u6B63\u78BA\u306B\u306A\u308A\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.accentB
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30B9\u30C6\u30C3\u30D7 5 \u2014 \u30DB\u30FC\u30EB\u5B8C\u4E86 \u2192 \u6B21\u306E\u30DB\u30FC\u30EB\u3078"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30AB\u30C3\u30D7\u30A4\u30F3\u5F8C\u306F\u300C\u30DB\u30FC\u30EB\u5B8C\u4E86\u300D\u304C\u8868\u793A"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u300C\u6B21\u306E\u30DB\u30FC\u30EB \u203A\u300D\u3067\u6B21\u3078\u3002\u9593\u9055\u3048\u305F\u3089\u300C\u21A9 \u53D6\u6D88\u300D\u3067\u6700\u5F8C\u306E\u30B7\u30E7\u30C3\u30C8\u3092\u53D6\u308A\u6D88\u3057\u3001\u30DB\u30FC\u30EB\u5B8C\u4E86\u5F8C\u306F\u300C\u4FEE\u6B63\u300D\u30DC\u30BF\u30F3\u3067\u623B\u305B\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement("div", {
      style: M.tipBoxB
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u8A73\u7D30\u30E2\u30FC\u30C9\u306E\u30B3\u30C4"), ["ティの残り距離はコース距離から自動入力。違うティを使った時だけ手動修正。", "クラブセットを事前に登録しておくとクラブ選択がスムーズ。", "ピン距離（カップまでの距離）も入れておくと分析がより正確に。", "ラウンド中は簡易、後でじっくり詳細、と使い分けるのも◎"].map(t => /*#__PURE__*/React.createElement("div", {
      key: t,
      style: {
        display: "flex",
        gap: "7px",
      ...
      style: M.lbl
    }, "\u5206\u6790\u30DA\u30FC\u30B8\u3078"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u4E0A\u90E8\u30CA\u30D3\u306E\u300C\u5206\u6790\u300D\u30BF\u30D6\u3092\u30BF\u30C3\u30D7"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u5206\u6790\u30DA\u30FC\u30B8\u306F\u4E0A\u304B\u3089\u9806\u306B7\u3064\u306E\u30BB\u30AF\u30B7\u30E7\u30F3\u304C\u4E26\u3073\u307E\u3059\u3002\u30C7\u30FC\u30BF\u304C\u63C3\u3063\u305F\u30BB\u30AF\u30B7\u30E7\u30F3\u304B\u3089\u9806\u306B\u8868\u793A\u3055\u308C\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "11px",
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          padding: "2px 7px",
          borderRadius: "10px",
          background: isDetail ? "rgba(14,165,233,0.10)" : "rgba(22,163,74,0.10)",
          color: isDetail ? "#0ea5e9" : "#16a34a"
        }
      ...
      style: M.lbl
    }, "AI\u8A3A\u65AD\u306E\u8AAD\u307F\u65B9"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "1\u679A\u306E\u30AB\u30FC\u30C9\u3067\u300C\u73FE\u72B6\u2192\u5F31\u70B9\u2192\u3084\u308B\u3053\u3068\u300D\u304C\u5206\u304B\u308B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "AI\u8A3A\u65AD\u306F\u3072\u3068\u3064\u306E\u30AB\u30FC\u30C9\u306B4\u3064\u306E\u60C5\u5831\u304C\u307E\u3068\u307E\u3063\u3066\u3044\u307E\u3059\u3002\u4E0A\u304B\u3089\u9806\u306B\u8AAD\u3080\u3060\u3051\u3067\u3001\u6B21\u306B\u4F55\u3092\u7DF4\u7FD2\u3059\u3079\u304D\u304B\u304C\u898B\u3048\u3066\u304D\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, [{
      nm: "総合評価",
      d: "ハンディキャップから期待されるスコアと比べて、今が好調か不調かをひと言で。"
    }, {
      nm: "5要素分析",
      d: "ティショット／ロングゲーム／ショートゲーム／パット／バンカーを棒グラフとS〜Dのグレードで表示。"
    }, {
      nm: "改善課題（優先順）",
      d: "特に弱い要素を最大3つ、優先順位つきでピックアップ。"
    }, {
      nm: "改善アドバイス",
      d: "課題ごとに「何をすればいいか」を具体的に提案。"
    }].map(({
      nm,
      d
    }) => /*#__PURE__*/React.createElement("div", {
    ...
    }, "5\u8981\u7D20\u306E\u7528\u8A9E"), [["ティショット", "Par4/5の最初のティショット（Par3は除外）"], ["ロングゲーム", "100Y超のショット（Par3で100Y超のティショット含む）"], ["ショートゲーム", "100Y以内のショット（Par3で100Y以内のティショット含む）"], ["パット", "グリーン上のパット数で評価"], ["バンカー", "バンカーからの脱出・回避を評価"]].map(([term, desc]) => /*#__PURE__*/React.createElement("div", {
      key: term,
      style: {
        marginBottom: "4px",
        lineHeight: 1.55
    ...
    }, "\u203B \u30B0\u30EC\u30FC\u30C9\u3084\u6539\u5584\u8AB2\u984C\u306F\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\u3092\u57FA\u6E96\u306B\u5224\u5B9A\u3059\u308B\u306E\u3067\u3001\u30DE\u30A4\u30DA\u30FC\u30B8\u3067\u306E\u8A2D\u5B9A\u304C\u304A\u3059\u3059\u3081\u3067\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.accentA
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u8A55\u4FA1\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8\u306E\u8AAD\u307F\u65B9"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "3\u672C\u306E\u30E9\u30A4\u30F3\u3092\u6BD4\u3079\u3066\u5F31\u70B9\u3092\u898B\u3064\u3051\u308B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u7DD1\uFF08\u7406\u60F3\xB10\uFF09\u30FB\u9EC4\uFF08\u76F4\u8FD15R\u5E73\u5747\uFF09\u30FB\u9752\uFF08\u76F4\u8FD120R\u5E73\u5747\uFF09\u306E3\u672C\u3002", /*#__PURE__*/React.createElement("b", {
      style: {
        color: "#dc2626"
      }
    }, "\u4E2D\u5FC3\u306B\u8FD1\u3044\u8EF8\uFF1D\u82E6\u624B\u5206\u91CE"), "\u3067\u3059\u3002")), /*#__PURE__*/React.createElement(MockAnalytics, null), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "10px",
        color: "#475569",
        textAlign: "center",
    ...
    }, "\u2191 \u8A55\u4FA1\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8\u4F8B\u3002\u76F4\u8FD15R\uFF08\u9EC4\uFF09\u304C\u76F4\u8FD120R\u5E73\u5747\uFF08\u9752\uFF09\u3088\u308A\u5916\u5074\u306B\u51FA\u308C\u3070\u6700\u8FD1\u597D\u8ABF"), /*#__PURE__*/React.createElement("div", {
      style: M.cardG
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
      ...
      c: "#16a34a",
      l: "緑（理想）",
      d: "全要素 ±0 のライン。これより外側＝平均より上、内側＝平均より下。"
    }, {
      c: "#fbbf24",
      l: "黄（直近5R）",
      d: "直近5ラウンドの平均値。今の調子を表します。"
    }, {
      c: "#0ea5e9",
      l: "青（直近20R）",
      d: "直近20ラウンドの平均値。あなたの実力の基準線です。"
    }].map(({
      c,
      l,
      d
      ...
      style: M.lbl
    }, "\u5F31\u70B9 \u2192 \u7DF4\u7FD2\u30E1\u30CB\u30E5\u30FC"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u300C\u6700\u3082\u4F4E\u3044\u8981\u7D20\u300D\u3092\u96C6\u4E2D\u30BF\u30FC\u30B2\u30C3\u30C8\u306B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u30EC\u30FC\u30C0\u30FC\u30C1\u30E3\u30FC\u30C8\u3067\u4E00\u756A\u4E2D\u5FC3\u306B\u8FD1\u3044\u8981\u7D20\uFF1D\u3042\u306A\u305F\u306E\u30B9\u30B3\u30A2\u3092\u6700\u3082\u5F15\u304D\u4E0B\u3052\u3066\u3044\u308B\u5F31\u70B9\u3067\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
        gridTemplateColumns: "1fr 1fr",
        gap: "8px",
      ...
      style: M.lbl
    }, "\u8A73\u7D30\u30E2\u30FC\u30C9\u306A\u3089\u3067\u306F\u306E\u5206\u6790"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u300C\u3069\u306E\u8DDD\u96E2\u3067\uFF0F\u3069\u306E\u30AF\u30E9\u30D6\u3067\u300D\u5931\u6557\u3057\u3066\u3044\u308B\u304B\u304C\u5206\u304B\u308B"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u4F8B\u3048\u3070\u300C150Y\u5E2F\u306E\u6210\u529F\u7387\u304C30%\u300D\u3068\u5206\u304B\u308C\u3070\u3001\u305D\u306E\u8DDD\u96E2\u3067\u306E\u7DF4\u7FD2\u304C\u6700\u512A\u5148\u3068\u5224\u65AD\u3067\u304D\u307E\u3059\u3002\u300C9I\u3088\u308A8I\u306E\u65B9\u304C\u6210\u529F\u7387\u304C\u9AD8\u3044\u300D\u3068\u5206\u304B\u308C\u3070\u3001\u8FF7\u3063\u305F\u3068\u304D\u306E\u5224\u65AD\u6750\u6599\u306B\u306A\u308A\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.accentA
    }, /*#__PURE__*/React.createElement("span", {
      style: M.lbl
    }, "\u30DE\u30A4\u30DA\u30FC\u30B8\uFF08\uD83D\uDC64\u30BF\u30D6\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        ...M.cardH,
        marginBottom: "5px"
      }
    }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\u30FB\u304A\u6C17\u306B\u5165\u308A\u30B3\u30FC\u30B9\u3092\u8A2D\u5B9A"), /*#__PURE__*/React.createElement("div", {
      style: M.body
    }, "\u4E0A\u90E8\u30CA\u30D3\u53F3\u7AEF\u306E\uD83D\uDC64\u304B\u3089\u30DE\u30A4\u30DA\u30FC\u30B8\u3092\u958B\u3051\u307E\u3059\u3002", /*#__PURE__*/React.createElement("b", null, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7"), "\uFF08\u624B\u52D5\u5165\u529B\u307E\u305F\u306F\u81EA\u52D5\u7B97\u51FA\u3092\u9078\u629E\uFF09\u3001\u30CB\u30C3\u30AF\u30CD\u30FC\u30E0\u3001\u30D9\u30B9\u30C8\u30B9\u30B3\u30A2\u3001", /*#__PURE__*/React.createElement("b", null, "\u304A\u6C17\u306B\u5165\u308A\u30B4\u30EB\u30D5\u5834"), "\uFF08\u6700\u59275\u4EF6\u30FB\u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A\u3067\u4E0A\u306B\u8868\u793A\uFF09\u3092\u767B\u9332\u3067\u304D\u307E\u3059\u3002\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\u3092\u8A2D\u5B9A\u3059\u308B\u3068\u5206\u6790\u306E\u30B0\u30EC\u30FC\u30C9\u5224\u5B9A\u3084\u6539\u5584\u8AB2\u984C\u304C\u3088\u308A\u6B63\u78BA\u306B\u306A\u308A\u307E\u3059\u3002")), /*#__PURE__*/React.createElement("div", {
      style: M.tipBox
    }, /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "12px",
    ...
    }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7\uFF08\u53C2\u8003\u5024\uFF09\u3082\u81EA\u52D5\u8A08\u7B97\uFF01"), ["18H完了ラウンドが3件以上で、ホーム画面に参考ハンディが表示されます。", "直近20ラウンド上位40%の成績から算出（公式とは異なります）。", "ラウンドを重ねるほど精度アップ。記録を続けることが一番大事！"].map(t => /*#__PURE__*/React.createElement("div", {
      key: t,
      style: {
        display: "flex",
        gap: "7px",
  ...
  }, "\u30DE\u30A4\u30DA\u30FC\u30B8")), /*#__PURE__*/React.createElement("div", {
    style: S.card({
      marginBottom: "14px"
    })
  }, /*#__PURE__*/React.createElement("div", {
    ...
    style: S.lbl
  }, "\u30CB\u30C3\u30AF\u30CD\u30FC\u30E0"), /*#__PURE__*/React.createElement("input", {
    style: S.input,
    placeholder: "\u4F8B\uFF1A\u30BF\u30A4\u30AC\u30FC",
    value: profileEdit.nickname,
    maxLength: 20,
    onChange: e => setProfileEdit(p => ({
      ...p,
    ...
    style: S.lbl
  }, "\u30D9\u30B9\u30C8\u30B9\u30B3\u30A2\uFF08\u6253\u6570\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px"
    ...
    onClick: () => setProfileEdit(p => ({
      ...p,
      bestScore: Math.max(54, (p.bestScore || 72) - 1)
    })),
    style: {
      width: "36px",
      height: "36px",
  ...
  }, profileEdit.bestScore ?? "−"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setProfileEdit(p => ({
      ...p,
      bestScore: Math.min(200, (p.bestScore || 71) + 1)
    })),
    style: {
      width: "36px",
      height: "36px",
    ...
    style: S.lbl
  }, "\u76EE\u6A19\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px"
    ...
    onClick: () => setProfileEdit(p => ({
      ...p,
      targetHcp: Math.max(0, (p.targetHcp ?? 20) - 1)
    })),
    style: {
      width: "36px",
      height: "36px",
  ...
  }, profileEdit.targetHcp ?? "−"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setProfileEdit(p => ({
      ...p,
      targetHcp: Math.min(54, (p.targetHcp ?? 19) + 1)
    })),
    style: {
      width: "36px",
      height: "36px",
  ...
  }, "\uFF0B"), profileEdit.targetHcp != null && /*#__PURE__*/React.createElement("button", {
    onClick: () => setProfileEdit(p => ({
      ...p,
      targetHcp: null
    })),
  ...
  }, "\u30AF\u30EA\u30A2"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: S.lbl
  }, "\u30CF\u30F3\u30C7\u30A3\u30AD\u30E3\u30C3\u30D7"), handicap && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: "12px"
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "\u5206\u6790\u306B\u4F7F\u7528\u3059\u308B\u5024\u3092\u9078\u629E"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: "8px"
    }
    ...
    key: "auto",
    label: "自動算出",
    desc: `${handicap.hcp}（${handicap.roundCount}R）`
  }, {
    key: "manual",
    label: "✏️ 手入力",
    desc: profileEdit.manualHcp != null ? String(profileEdit.manualHcp) : "未設定"
  }].map(({
    key,
    label,
    desc
  }) => {
    const active = (profileEdit.hcpMode ?? "auto") === key;
    return /*#__PURE__*/React.createElement("button", {
      key: key,
      onClick: () => setProfileEdit(p => ({
        ...p,
  ...
  })), (profileEdit.hcpMode ?? "auto") === "auto" ? /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "10px",
      color: "#16a34a",
      marginTop: "6px"
    }
  }, "\u2713 \u904E\u53BB\u30E9\u30A6\u30F3\u30C9(", handicap.roundCount, "\u4EF6)\u304B\u3089\u81EA\u52D5\u7B97\u51FA\u3057\u305F ", /*#__PURE__*/React.createElement("b", null, handicap.hcp), " \u3092\u5206\u6790\u306B\u4F7F\u7528") : /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "10px",
      color: "#d97706",
      marginTop: "6px"
    }
  }, "\u2713 \u4E0B\u8A18\u3067\u8A2D\u5B9A\u3059\u308B\u624B\u5165\u529B\u5024\u3092\u5206\u6790\u306B\u4F7F\u7528")), /*#__PURE__*/React.createElement("div", {
    style: {
      opacity: handicap && (profileEdit.hcpMode ?? "auto") === "auto" ? 0.45 : 1
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
  ...
  }, handicap ? "手入力値（モードを「手入力」にすると分析に使用されます）" : "ラウンド3件未満の場合に分析で使用されます"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: "10px"
    ...
    onClick: () => setProfileEdit(p => ({
      ...p,
      manualHcp: Math.max(0, (p.manualHcp ?? 20) - 1)
    })),
    style: {
      width: "36px",
      height: "36px",
  ...
  }, profileEdit.manualHcp ?? "−"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: "9px",
      color: "#94a3b8"
    }
  }, "\u624B\u5165\u529B")), /*#__PURE__*/React.createElement("button", {
    onClick: () => setProfileEdit(p => ({
      ...p,
      manualHcp: Math.min(54, (p.manualHcp ?? 19) + 1)
    })),
    style: {
      width: "36px",
      height: "36px",
  ...
  }, "\uFF0B"), profileEdit.manualHcp != null && /*#__PURE__*/React.createElement("button", {
    onClick: () => setProfileEdit(p => ({
      ...p,
      manualHcp: null
    })),
    ...
    style: S.lbl
  }, "\u304A\u6C17\u306B\u5165\u308A\u30B4\u30EB\u30D5\u5834\uFF08\u6700\u59275\u4EF6\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#64748b",
      marginBottom: "10px"
    }
  }, "\u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A\u306E\u300C\u304A\u6C17\u306B\u5165\u308A\u300D\u30BF\u30D6\u306B\u8868\u793A\u3055\u308C\u307E\u3059"), (profileEdit.favoriteVenues || []).length === 0 ? /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "12px",
      borderRadius: "10px",
  ...
  }, "\u672A\u767B\u9332\u3067\u3059") : /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: "6px",
  ...
  }, (profileEdit.favoriteVenues || []).map((vid, idx) => {
    const v = VENUES.find(x => x.id === vid);
    if (!v) return null;
    return /*#__PURE__*/React.createElement("div", {
      key: vid,
    ...
    }, "\u2191"), idx < (profileEdit.favoriteVenues || []).length - 1 && /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        const arr = [...(profileEdit.favoriteVenues || [])];
      ...
      onClick: () => setProfileEdit(p => ({
        ...p,
        favoriteVenues: (p.favoriteVenues || []).filter(x => x !== vid)
      })),
      style: {
        padding: "3px 8px",
        borderRadius: "6px",
    ...
    }, sorted.map((v, i) => /*#__PURE__*/React.createElement("button", {
      key: v.id,
      onClick: () => setProfileEdit(p => ({
        ...p,
        favoriteVenues: [...(p.favoriteVenues || []), v.id]
      })),
      style: {
        width: "100%",
        textAlign: "left",
  ...
  }, "\u2705 \u4FDD\u5B58\u3059\u308B"))), showShotForm && currentCatDef && (() => {
    // ティ自動距離を取得
    const teeVenue = currentRound?.venueId ? VENUES.find(v => v.id === currentRound.venueId) : null;
    ...
    const teeDisplayY = teeDistOverride ?? teeAutoY;
    // アプローチスライダーMAX
    const approachMax = teeAutoY ?? 300;
    // 登録クラブ
    ...
    }, /*#__PURE__*/React.createElement("style", null, SLIDER_CSS), /*#__PURE__*/React.createElement("div", {
      style: {
        ...S.modalBox,
        padding: 0,
        display: "flex",
    ...
    }, getDisplayHoleNum(currentHole), "\u756A\u30DB\u30FC\u30EB\uFF08Par", par, "\uFF09"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        alignItems: "center",
        gap: "6px"
    ...
    })(), hd.shots.length === 0 ? /*#__PURE__*/React.createElement("button", {
      style: S.btn("ghost"),
      onClick: () => setShowShotForm(false)
    }, "\u30AD\u30E3\u30F3\u30BB\u30EB") : /*#__PURE__*/React.createElement("button", {
      style: S.btn("ghost"),
      onClick: () => {
        undoLastShot();
    ...
    }, totalStrokes + 1, "\u6253\u76EE \u2014 ", currentCatDef.label)), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1,
        overflowY: "auto",
    ...
    }, calcOpen ? "入力 → ✓確定 / ✕キャンセル" : teeDistOverride ? "手動入力済み ✏️" : "自動取得 ✏️ タップで修正"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: "24px",
        fontWeight: "800",
        color: "#60a5fa"
      }
    }, calcOpen ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("span", {
      style: {
        opacity: calcInput ? 1 : 0.35
      }
    }, calcInput || "0"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "13px",
        color: "#93c5fd"
      }
    }, " Y")) : /*#__PURE__*/React.createElement(React.Fragment, null, teeDisplayY ?? "−", /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "13px",
        color: "#93c5fd"
      }
    }, teeDisplayY ? " Y" : "")))), !calcOpen && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "16px",
        opacity: .35
      }
    }, "\u270F\uFE0F")), calcOpen && /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
    ...
    React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    }, approachDistVal, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "13px",
        marginLeft: "3px",
        color: "#93c5fd"
      ...
      style: {
        background: sliderBg(approachDistVal, 5, approachMax)
      }
    }), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
    ...
    }, "5Y"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        color: "#334155",
        fontWeight: "600"
    ...
    React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    }, puttDistVal, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "13px",
        marginLeft: "3px",
        color: "#93c5fd"
      ...
      style: {
        background: sliderBg(puttDistVal, 1, 30)
      }
    }), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
    ...
    }, "1m"), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: "10px",
        color: "#334155",
        fontWeight: "600"
      }
    }, "30m")))), currentCategory !== "putt" && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "14px"
      }
    }, /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("button", {
        key: c.id,
        onClick: () => setSelectedClub(sel ? null : c.id),
        style: {
          padding: "9px 13px",
          borderRadius: "8px",
          cursor: "pointer",
      ...
      onClick: () => setSelectedClub(selectedClub === "none" ? null : "none"),
      style: {
        padding: "9px 13px",
        borderRadius: "8px",
        cursor: "pointer",
    ...
    }, "\u30AF\u30E9\u30D6\u306A\u3057"))), /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "14px"
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u5FC5\u9808")), newResultId === "bad" && /*#__PURE__*/React.createElement("div", {
      style: {
        marginBottom: "8px",
    ...
    }, "\u25B6 \u30DF\u30B9\u306E\u7A2E\u985E"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: "6px"
      }
      ...
      return /*#__PURE__*/React.createElement("button", {
        key: b.id,
        onClick: () => setNewBadSubId(sel ? null : b.id),
        style: {
          flex: 1,
          padding: "10px 4px",
          borderRadius: "8px",
      ...
      }, /*#__PURE__*/React.createElement("div", null, b.label), b.penaltyCount > 0 && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: "9px",
          marginTop: "2px",
          opacity: .8
        }
      }, "+", b.penaltyCount, "\u7F70\u30FB\u8A08", totalPen, "\u6253"));
    }))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexWrap: "wrap",
    ...
    }, "\u30E1\u30E2\uFF08\u4EFB\u610F\uFF09"), /*#__PURE__*/React.createElement("input", {
      style: S.input,
      placeholder: "\u4F8B\uFF1A\u5DE6\u306B\u5F15\u3063\u639B\u3051\u305F...",
      value: shotNote,
      onChange: e => setShotNote(e.target.value)
    }))), /*#__PURE__*/React.createElement("div", {
      style: {
  ...
  })(), showPinDistStep && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
  ...
  }, "\u30AB\u30C3\u30D7\u307E\u3067\u306E\u8DDD\u96E2"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "11px",
      color: "#475569",
      marginTop: "2px"
    }
  }, getDisplayHoleNum(currentHole), "\u756A\u30DB\u30FC\u30EB")), /*#__PURE__*/React.createElement("button", {
    style: {
      padding: "5px 12px",
      borderRadius: "8px",
      border: "1px solid #e2e8f0",
  ...
  }, PIN_DIST_OPTIONS.map(opt => /*#__PURE__*/React.createElement("button", {
    key: opt.id,
  ...
  }, opt.label))))), showExportModal && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
  ...
  }, "\u30C7\u30FC\u30BF\u66F8\u304D\u51FA\u3057\uFF08", rounds.length, "\u4EF6\uFF09"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: "12px",
      color: "#16a34a",
      fontWeight: "700",
  ...
  }, "\u30C6\u30AD\u30B9\u30C8\u30A8\u30EA\u30A2\u3092\u30BF\u30C3\u30D7 \u2192 \u5168\u9078\u629E\u3055\u308C\u307E\u3059 \u2192 \u30B3\u30D4\u30FC\uFF08Ctrl+C / \u2318C\uFF09"), /*#__PURE__*/React.createElement("textarea", {
    readOnly: true,
    value: exportJson,
  ...
  }, "\u9589\u3058\u308B")))), toast.visible && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      top: "68px",
      left: "50%",
    ...
    return REXY_IMAGES[tc] ? /*#__PURE__*/React.createElement(RexyIcon, {
      costume: tc,
      size: 46,
      alt: ""
    }) : null;
...
// ─────────────────────────────────────────────────────────────────────────────
// 自動マウント（bootstrap）
//   #root に GolfTracker を描画する。
//   index.html 側のフォールバックと二重描画しないよう window.__SCRX_MOUNTED でガード。
// ─────────────────────────────────────────────────────────────────────────────
