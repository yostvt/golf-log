...
function ocrDetectFormat(opts) {
  ...
  var colHeaderText = opts.colHeaderText || "";
  var isTable = opts.isTable !== false;
  if (ratio > 1.3) return "F2";
  if (headerText.indexOf("\u30E9\u30A6\u30F3\u30C9\u5C65\u6B74") >= 0) return "F3";
  if (/FW\s*キープ|1\s*オン|クラブ/.test(colHeaderText)) return "F4";
  if (!isTable) return "UNSUPPORTED";
  return "F1";
}
...
async function ocrRunExtraction(file, handlers) {
  ...
  onProgress("\u5F62\u5F0F\u3092\u5224\u5225\u4E2D\u2026");
  var full = await ocrRecognize(ocrToCanvas(img, 1), { tessedit_pageseg_mode: "6" });
  var fullText = full && full.data && full.data.text || "";
  var words = full && full.data && full.data.words || [];
  var topText = ocrWordsInBand(words, 0, H * 0.13).map(function(w) {
    return w.text;
  }).join(" ");
  var fmt2 = ocrDetectFormat({
    ratio,
    headerText: topText + " " + fullText.slice(0, 200),
    colHeaderText: fullText,
    isTable: words.length > 20
  });
  if (fmt2 === "UNSUPPORTED") {
...
async function ocrExtractVertical(img, words, fmt2) {
  var anchors = ocrFindHoleRows(words, img.height);
  var cols = ocrEstimateVerticalColumns(words, img.width, fmt2);
  var front = [], back = [];
  for (var i = 0; i < anchors.length && i < 18; i++) {
    var a = anchors[i];
    var score = await ocrReadDigit(img, cols.score[0], a.y0, cols.score[1] - cols.score[0], a.y1 - a.y0);
    var putt = await ocrReadDigit(img, cols.putt[0], a.y0, cols.putt[1] - cols.putt[0], a.y1 - a.y0);
    var rec = { dispHole: a.hole, par: a.par || 4, score, putts: putt, teeEval: "\u25CB" };
  ...
  const ocrHandleFile = async (e) => {
    ...
    try {
      ...
      if (!ex || ex.fmt === "UNSUPPORTED") {
        setOcrError("\u5BFE\u5FDC\u5F62\u5F0F\u306E\u753B\u50CF\u3067\u306F\u3042\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002\u30B9\u30B3\u30A2\u30AB\u30FC\u30C9\uFF0F\u5C65\u6B74\u30B5\u30DE\u30EA\u30FC\u306E\u753B\u9762\u3092\u64AE\u5F71\u30FB\u30B9\u30AF\u30B7\u30E7\u3057\u305F\u753B\u50CF\u3092\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002");
        return;
      }
      ...
      setOcr({ built, setup, venueId, meta: { fmt: ex.fmt, courseName: ex.courseName, totalYard: ex.totalYard, teeText: ex.teeText, greenText: ex.greenText } });
      setOcrStep("setup");
    } catch (err) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: S.app }, /* @__PURE__ */ React.createElement("div", { style: S.header }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "7px" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setView("home"), style: { display: "inline-block", lineHeight: 0, verticalAlign: "middle", background: "none", border: "none", padding: 0, cursor: "pointer", borderRadius: "6px", outline: "none" }, title: "\u30DB\u30FC\u30E0" }, /* @__PURE__ */ React.createElement("img", { src: "data:image/png;base64,", style: { width: "28px", height: "28px", borderRadius: "6px", display: "block" }, alt: "icon" })), /* @__PURE__ */ React.createElement("button", { onClick: () => setView("home"), title: "\u30DB\u30FC\u30E0", style: { background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "15px", fontWeight: "800", color: "#16a34a", letterSpacing: "0.02em", lineHeight: 1, whiteSpace: "nowrap" } }, "\u30B9\u30B3\u30EC\u30DC"), detailUnlocked ? /* @__PURE__ */ React.createElement("span", { style: { fontSize: "9px", fontWeight: "800", color: "#16a34a", background: "rgba(22,163,74,0.10)", border: "1px solid rgba(22,163,74,0.30)", borderRadius: "20px", padding: "2px 7px" } }, "PRO") : /* @__PURE__ */ React.createElement("button", { onClick: () => {
    ...
    if (ocrStep === "setup") {
      ...
      return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: modeBadge }, "\u{1F4F7} \u30E9\u30A6\u30F3\u30C9\u8A2D\u5B9A\uFF08\u8AAD\u307F\u53D6\u308A\u5185\u5BB9\uFF09"), field("\u30B3\u30FC\u30B9", /* @__PURE__ */ React.createElement(React.Fragment, null, b.course || "\uFF08\u672A\u8A2D\u5B9A\uFF09", courseBadge), () => {
        ...
        setOcrStep("score");
        setOcrSel(null);
      } }, "\u6B21\u3078\uFF1A\u30B9\u30B3\u30A2\u767B\u9332"), /* @__PURE__ */ React.createElement("div", { style: { height: "10px" } }), /* @__PURE__ */ React.createElement("button", { style: __spreadProps(__spreadValues({}, S.btn("secondary")), { width: "100%" }), onClick: ocrCancel }, "\u6700\u521D\u306B\u623B\u308B"), sheet);
    }
    if (ocrStep === "score") {
