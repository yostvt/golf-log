...
function GolfTracker() {
  ...
  useEffect(() => {
    try {
      if (ScrxStorage.getItem("golf_detail_unlocked") === "1") setDetailUnlocked(true);
    } catch (_) {
    }
    try {
      if (ScrxStorage.getItem("golf_export_unlocked") === "1") setExportUnlocked(true);
    } catch (_) {
    }
  }, []);
  ...
  const readTrash = () => {
    try {
      const raw = ScrxStorage.getItem(TRASH_KEY);
  ...
  const writeTrash = (list) => {
    try {
      ScrxStorage.setItem(TRASH_KEY, JSON.stringify(list.slice(0, 5)));
    } catch (_) {
    }
  };
  const readDailyBackup = () => {
    try {
      const raw = ScrxStorage.getItem(DAILY_BACKUP_KEY);
  ...
  const readBackupGens = () => {
    try {
      const raw = ScrxStorage.getItem(BACKUP_GENS_KEY);
      const p = raw ? JSON.parse(raw) : null;
  ...
  const saveBackupSnapshot = (list) => {
    ...
    try {
      ScrxStorage.setItem(BACKUP_GENS_KEY, JSON.stringify(next));
      try {
        ScrxStorage.removeItem(DAILY_BACKUP_KEY);
      } catch (_) {
      }
      setBackupGens(next);
      ...
      try {
        ...
        ScrxStorage.setItem(BACKUP_GENS_KEY, JSON.stringify(min));
        setBackupGens(min);
  ...
  useEffect(() => {
    if (!storageLoaded) return;
    try {
      ScrxStorage.setItem("golf_profile", JSON.stringify(profile));
    } catch (_) {
    }
  ...
  const applyUnlockCode = () => {
    ...
    if (UNLOCK_CODES.map((c) => c.toUpperCase()).includes(code)) {
      ...
      try {
        ScrxStorage.setItem("golf_detail_unlocked", "1");
      } catch (_) {
      }
  ...
  useEffect(() => {
    if (!storageLoaded) return;
    try {
      ScrxStorage.setItem("golf_clubs", JSON.stringify(savedClubs));
    } catch (_) {
    }
  ...
  useEffect(() => {
    if (!storageLoaded) return;
    try {
      ScrxStorage.setItem("golf_rounds", JSON.stringify(rounds));
      saveBackupSnapshot(rounds);
    } catch (_) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      (() => {
        try {
          ScrxStorage.setItem("golf_current_round", JSON.stringify(payload));
        } catch (_) {
    ...
    "button",
    {
      onClick: () => {
        ...
        try {
          (() => {
            try {
              ScrxStorage.setItem("golf_clubs", "[]");
            } catch (_) {
            }
          })();
        } catch (_) {
      ...
      return /* @__PURE__ */ React.createElement(
        "button",
        {
          ...
          onClick: () => {
            ...
            (() => {
              try {
                ScrxStorage.setItem("golf_clubs", JSON.stringify(next));
              } catch (_) {
              }
            })();
          },
